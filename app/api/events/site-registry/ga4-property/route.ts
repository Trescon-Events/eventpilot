...
/* POST /api/events/site-registry/ga4-property?event_id=X
   Body: { accountId, displayName, liveUrl }
   ...
   free-typed account id. v1.4: auth is the shared service account, not a
   picked-per-request OAuth connection. Returns the created
   property/stream/measurement ID; the caller then saves the selection via
   /api/events/site-registry/connections, same as picking an existing
   property, so "created" and "selected" both end up going through the
   same fetch-and-select-shaped save step. */
...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => null) as { accountId?: string; displayName?: string; liveUrl?: string } | null
  if (!body?.accountId || !body?.displayName || !body?.liveUrl) {
    return NextResponse.json({ error: 'accountId, displayName, and liveUrl are required' }, { status: 400 })
  }

  const accessToken = await getGoogleServiceAccountToken()
  if (!accessToken) return NextResponse.json({ error: 'Google service account not configured.' }, { status: 400 })
