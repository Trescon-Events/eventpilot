...
import { resolveScope, hasScopePermission, ownerColumn, ownerFields, auditOwner } from '@/app/lib/ops/scope'
...
/* POST /api/events/operations/licenses/files   (multipart: event_id | umbrella_id, batch_ids (JSON array), file)
...
export async function POST(req: NextRequest) {
  ...
  const scope = await resolveScope({ eventId: form?.get('event_id')?.toString() || null, umbrellaId: form?.get('umbrella_id')?.toString() || null })
  const file = form?.get('file')
  let batchIds: unknown
  try { batchIds = JSON.parse(String(form?.get('batch_ids') ?? '[]')) } catch { batchIds = [] }
  if (!scope || !(file instanceof File) || !Array.isArray(batchIds) || batchIds.length === 0 || batchIds.some(b => typeof b !== 'string')) {
    return NextResponse.json({ error: 'event_id (or umbrella_id), at least one batch and a file are required.' }, { status: 400 })
  }
  ...
  const { data: batches } = await supabaseAdmin.from('ops_license_batches').select('id, vendor_id, status, batch_number').eq(ownerColumn(scope), scope.id).in('id', batchIds as string[])
  ...
  const storagePath = `${scope.id}/staff/${randomUUID()}.${LICENSE_ALLOWED[type]}`
  ...
  const { data: row, error } = await supabaseAdmin.from('ops_license_files').insert({
    ...ownerFields(scope), vendor_id: batches[0].vendor_id, storage_path: storagePath, file_name: cleanName, mime_type: type,
  ...
  await logOpsAccess({ ...auditOwner(scope), actorType: 'staff', actorId: session?.sid ?? null, action: 'license_uploaded_by_staff', targetType: 'license_file', targetId: row.id, meta: { batches: batches.map(b => b.batch_number) }, ip: clientIp(req) })
