...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  // Temporary verbose logging (2026-08-27) — see postiz-publish.ts's own
  // comment for why: real attempts were producing a fast, empty-body 502
  // with zero application log output at all, which this whole-handler
  // try/catch + entry marker exists to either explain or rule out. Remove
  // once resolved.
  console.log('[publish-now] route entered')
  try {
    ...
    try {
      ...
      console.error('[publish-now] publish failed:', e instanceof Error ? `${e.name}: ${e.message}\n${e.stack}` : String(e))
      return NextResponse.json({ error: message }, { status: 502 })
    }
  } catch (e) {
    console.error('[publish-now] UNEXPECTED top-level error:', e instanceof Error ? `${e.name}: ${e.message}\n${e.stack}` : String(e))
