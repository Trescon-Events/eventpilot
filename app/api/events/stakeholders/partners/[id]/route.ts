...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const formType: FormType = (req.nextUrl.searchParams.get('form_type') as FormType | null) ?? 'sponsor'
  const schema = await resolveFormSchema(data.event_id, formType)
  return NextResponse.json({ ...fromRow(data), fields: recordToFields(formType, schema, data) })
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Editing an already-approved partner (any data change) must force a
  // fresh review before it counts for announcements again — see the
  // matching comment in speakers/[id]/route.ts's PATCH for the precedent.
