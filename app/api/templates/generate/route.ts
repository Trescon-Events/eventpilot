...
/* POST /api/templates/generate
   ...
   Pulls event data from TAOS DB and generates a populated event.ts config
...
export async function POST(req: NextRequest) {
  try {
    ...
    const speakersBlock = speakers?.length
      ...
      : '    // No speakers yet — add from TAOS or directly here'
    ...
    const sponsorsBlock = sponsors?.length
      ...
      : '    // No sponsors yet — add from TAOS or directly here'

    const configTs = `// ── Generated by TAOS on ${new Date().toISOString().split('T')[0]} ────────────────────────────────────────
...
export const EVENT = {
  ...
  // ── Seeded Content (from TAOS DB — edit freely) ───────────────────────────
  speakers_seed: [
${speakersBlock}
  ],
