...
const DOCS = [
  /* ── 1. PLATFORM OVERVIEW ─────────────────────────────────────── */
  {
    slug: 'what-is-trescademy',
    category: 'Platform Overview',
    title: 'What is Trescademy?',
    content: `Trescademy is Trescon Global's internal AI learning and readiness platform. It serves all staff across the four Trescon offices — Dubai, Bangalore, Mangalore, and Manipal.
...
2. The system calculates their TAIRS score (AI Readiness Score out of 100)
...
Your score updates every time you retake the questionnaire. The way to move your score up is not to answer the questionnaire differently — it is to genuinely start using AI for more of your tasks. Take a course, apply it to real work, then retake the assessment. Your score should reflect your actual practice.`,
  ...
  /* ── 3. LEARNING TRACKS ────────────────────────────────────────── */
  {
    ...
    content: `Trescademy organises all courses into three tracks that align with your TAIRS score tier.
...
You can retake any assessment. Each attempt is recorded.`,
  ...
  /* ── 4. RECOMMENDATION ENGINE ──────────────────────────────────── */
  {
    ...
    content: `Every individual on Trescademy sees a personalised course list. The recommendation engine scores every available (uncompleted) course for each person using five signals, then ranks and labels them.
...
As staff complete courses, their completed courses are removed from the pool and the remaining courses re-rank. The list is always fresh and always moving forward.`,
  ...
  /* ── 5. STAFF: HOW TO USE ──────────────────────────────────────── */
  {
    ...
    category: 'User Guide',
    title: 'How to Use Trescademy as a Staff Member',
    content: `This guide is for all Trescon staff using their personal dashboard.

STEP 1: LOG IN
Go to www.trescademy.com (or localhost:3003 during development). Use your Trescon work email and the password provided by HR. You will land on your personal dashboard.
...
STEP 6: REPEAT
After completing a course, your "Next Up" updates to the next highest-priority recommendation. Work through your list. Your TAIRS score will rise as your actual AI usage increases — take the questionnaire again after 4–6 weeks of applying what you have learned.
...
If you cannot log in, contact your manager or the HR team. If a course is unclear or incorrect, flag it to your manager.`,
  ...
  /* ── 6. MANAGER GUIDE ──────────────────────────────────────────── */
  {
    ...
    category: 'User Guide',
    title: 'How to Use Trescademy as a Manager or Team Lead',
...
This is identical to every other staff member's dashboard. It shows your own TAIRS score, your own recommendations, and your own course completions. You are expected to lead by example — managers should be among the first to complete their own learning path.
...
- Their TAIRS score (AI Readiness Score)
...
The scope is determined automatically from the reporting hierarchy — it updates as staff are added, moved, or promoted.`,
  ...
  /* ── 7. ADMIN GUIDE ────────────────────────────────────────────── */
  {
    ...
    category: 'User Guide',
    title: 'How to Use Trescademy as an Administrator',
...
The Super Admin account (set via SUPER_ADMIN_EMAIL in environment variables) has full access to all data regardless of the reporting hierarchy. The Super Admin account exists even before HR data is imported — it uses a synthetic profile until a matching staff record is created.`,
  ...
  /* ── 8. HIERARCHY SYSTEM ───────────────────────────────────────── */
  {
    ...
    content: `Trescademy uses a self-referencing hierarchy — every staff member can have a manager, and every manager's scope on the Team Dashboard is automatically derived from the chain of reporting relationships.
...
The recommendation engine also uses job_level to boost management-relevant courses for anyone at team_lead and above.`,
  ...
  /* ── 9. TECHNICAL ARCHITECTURE ─────────────────────────────────── */
  {
    ...
    content: `Trescademy is built as a Next.js web application connected to a Supabase (PostgreSQL) database. This document describes what the platform is built on and how the data is organised — intended for technical staff and administrators.
...
staff_task_profiles
Stores each staff member's AI Readiness Questionnaire responses. Each response records a task description and an AI readiness rating from 1 (no AI use) to 5 (fully AI-integrated). These ratings are averaged to compute the TAIRS score.
...
2. TAIRS score is computed from task profile ratings (server-side calculation)
...
Individual TAIRS scores and task profiles are visible to the staff member and their direct manager. Department and office aggregate data is visible to department heads and office heads. All data is stored in Supabase with server-side access controls. No individual performance data is exposed publicly.
...
Your course list is personalised. The recommendation engine considers your TAIRS score (which determines your track), your department (which boosts relevant courses), whether any mandatory courses are outstanding, and your job level. You can browse all available courses in the Course Library.
...
Trescademy is designed for continuous learning, not one-time completion. Aim for one course per week. This means your full foundation track (if starting from AI-Unaware) is complete within 6 weeks. Adoption track takes another 6 weeks. You should be at AI-Ready or above within 90 days of starting.
...
Courses are authored by the Trescademy team. Course content is generated using AI tools (Google Gemini) and then reviewed and edited by subject matter experts before publication. All courses are reviewed for factual accuracy, practical applicability, and alignment with Trescon's specific context.`,
    order_index: 10,
  },
]
