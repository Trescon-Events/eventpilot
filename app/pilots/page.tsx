...
const ROLE_COLORS: Record<string, { bg: string; color: string }> = {
  pilot:      { bg: 'var(--info-light)',    color: 'var(--info)' },
  co_pilot:   { bg: '#420525',              color: '#F472B6' }, // pink has no dedicated token pair; dark pink-tinted swatch mirrors every other family's -light role
  consulting: { bg: 'var(--amber-light)',   color: 'var(--amber)' },
  tracking:   { bg: 'var(--success-light)', color: 'var(--success)' },
}
...
const STATUS_COLORS: Record<string, { bg: string; color: string; label: string }> = {
  active:   { bg: 'var(--info-light)',    color: 'var(--info)',    label: 'Active — Not Started' },
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: 'var(--surface)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 64px)' }}>
        <p style={{ color: 'var(--ink3)', fontSize: 15 }}>Loading your pilot projects…</p>
      </div>
    </div>
  )
  ...
  if (!projects.length) return (
    <div style={{ minHeight: '100vh', background: 'var(--surface)' }}>
      ...
      <div style={{ maxWidth: 600, margin: '80px auto', textAlign: 'center', padding: '0 24px' }}>
        ...
        <h2 style={{ fontSize: 22, fontWeight: 700, color: 'var(--ink)', margin: '0 0 8px' }}>No pilot projects yet</h2>
        <p style={{ color: 'var(--ink3)', fontSize: 15 }}>You haven't been assigned to any EventPilot pilot projects.</p>
      </div>
    </div>
  )
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 24px' }}>
        ...
        {projects.map(project => {
          ...
          const roleMeta   = myMember?.role_color
            ...
            : ROLE_COLORS[project.myRole ?? ''] ?? { bg: 'var(--border-light)', color: 'var(--ink2)' }
          ...
          return (
            <div key={project.id} style={{ background: 'var(--card)', borderRadius: 14, border: '1px solid var(--border)', marginBottom: 28, overflow: 'hidden' }}>

              {/* Project header */}
              <div style={{ padding: '24px 28px', borderBottom: '1px solid var(--border-light)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                  <div>
                    <h2 style={{ fontSize: 19, fontWeight: 700, color: 'var(--ink)', margin: '0 0 6px' }}>{project.name}</h2>
                    {project.description && <p style={{ color: 'var(--ink3)', fontSize: 14, margin: '0 0 12px', lineHeight: 1.6 }}>{project.description}</p>}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    {!isTrackerView && (
                      <div style={{ textAlign: 'center', minWidth: 64 }}>
                        <div style={{ fontSize: 26, fontWeight: 800, color: myPct === 100 ? 'var(--success)' : 'var(--ink)' }}>{myPct}%</div>
                        <div style={{ fontSize: 11, color: 'var(--ink3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>Complete</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 16, marginTop: 16, flexWrap: 'wrap' }}>
                  {project.members.map(m => {
                    const mRole = m.role_color
                      ...
                      : ROLE_COLORS[m.role] ?? { bg: 'var(--border-light)', color: 'var(--ink2)' }
                    return (
                      <div key={m.staff_id} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink)' }}>{m.staff?.name ?? 'Unknown'}</div>
                          <div style={{ fontSize: 11, color: 'var(--ink3)' }}>{m.role_label ?? ROLE_LABELS[m.role] ?? m.role}</div>
                        </div>
                      </div>
                    )
                  })}
              ...
              {tab === 'checklist' && (
                <div style={{ padding: '20px 28px' }}>
                  {!isTrackerView ? (
                    <>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 14 }}>
                        Your Checklist — {myItems.filter(i => i.completed).length} of {myItems.length} done
                      </div>
                      {myItems.length === 0
                        ? <p style={{ color: 'var(--ink3)', fontSize: 14 }}>No checklist items assigned to you yet.</p>
                    ...
                    <>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 16 }}>
                        ...
                        return (
                          <div key={sid} style={{ marginBottom: 20 }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                              <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink2)' }}>{name}</span>
                              <span style={{ fontSize: 12, color: pct === 100 ? 'var(--success)' : 'var(--ink3)', fontWeight: 600 }}>{pct}% complete</span>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                              {items.map((item, idx) => (
                                <ChecklistRow key={item.id} item={item} index={idx + 1}
              ...
              {tab === 'builds' && (
                <div style={{ padding: '20px 28px' }}>
                  ...
                  {showForm !== project.id && (
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
                      <button onClick={() => { setShowForm(project.id); setFormError('') }} style={{
                        background: 'var(--teal-mid)', color: 'var(--teal-light)', border: 'none', borderRadius: 8,
                  ...
                  {/* New request form */}
                  {showForm === project.id && (
                    <div style={{ background: 'var(--success-light)', border: '1px solid #1F8F66', borderRadius: 12, padding: '20px 24px', marginBottom: 20 }}>
                      <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)', margin: '0 0 16px' }}>New Build Request</h3>
                      <div style={{ marginBottom: 12 }}>
                        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink2)', display: 'block', marginBottom: 4 }}>TITLE *</label>
                        <input value={formTitle} onChange={e => setFormTitle(e.target.value)}
                          placeholder="Short summary of what you need built"
                          style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 14, boxSizing: 'border-box', outline: 'none' }} />
                      </div>
                      <div style={{ marginBottom: 12 }}>
                        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink2)', display: 'block', marginBottom: 4 }}>DETAILED INSTRUCTIONS *</label>
                        <textarea value={formMessage} onChange={e => setFormMessage(e.target.value)}
                          ...
                          rows={5} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 14, boxSizing: 'border-box', resize: 'vertical', outline: 'none', fontFamily: 'inherit' }} />
                      </div>
                      <div style={{ marginBottom: 16 }}>
                        <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink2)', display: 'block', marginBottom: 4 }}>
                          ATTACHMENTS <span style={{ fontWeight: 400, color: 'var(--ink3)' }}>(PDF, PNG, JPG — max 10 MB each, max 3 files)</span>
                        </label>
                        <div
                          ...
                          style={{
                            border: `1.5px dashed ${dragActive ? 'var(--teal-mid)' : 'var(--border)'}`, borderRadius: 8,
                            background: dragActive ? 'var(--teal-light)' : 'var(--card-hi)', padding: '16px 14px',
                        ...
                        {formFiles.length > 0 && (
                          <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            {formFiles.map((f, i) => (
                              <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, background: 'var(--card)', border: '1px solid var(--border)', padding: '3px 6px 3px 10px', borderRadius: 999, color: 'var(--ink2)' }}>
                                {fileIcon(f.type)} {f.name} ({fmtSize(f.size)})
                                <button type="button" onClick={() => setFormFiles(prev => prev.filter((_, j) => j !== i))}
                                  style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--ink4)', fontSize: 13, lineHeight: 1, padding: '0 2px' }}>✕</button>
                              </span>
                            ))}
                          </div>
                        )}
                      </div>
                      {formError && <p style={{ color: 'var(--red)', fontSize: 13, margin: '0 0 12px' }}>{formError}</p>}
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button onClick={() => handleSubmitRequest(project.id)} disabled={formSubmitting}
                          style={{ background: 'var(--teal-mid)', color: 'var(--teal-light)', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 14, fontWeight: 700, cursor: formSubmitting ? 'not-allowed' : 'pointer', opacity: formSubmitting ? 0.6 : 1 }}>
                  ...
                  {loadingBuilds[project.id] ? (
                    <p style={{ color: 'var(--ink3)', fontSize: 14, textAlign: 'center', padding: '24px 0' }}>Loading requests…</p>
                  ) : builds.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {builds.map(br => {
                        ...
                        return (
                          <div key={br.id} style={{ border: `1px solid ${br.status === 'needs_clarification' ? 'var(--purple-border)' : 'var(--border)'}`, borderRadius: 10, overflow: 'hidden', background: br.status === 'needs_clarification' ? 'var(--purple-light)' : 'var(--card)' }}>
                            {/* Request row */}
                            <div onClick={() => setExpandedBR(isExpanded ? null : br.id)}
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 12, color: 'var(--ink3)' }}>{fmtDate(br.created_at)}{br.files.length > 0 ? ` · ${br.files.length} file${br.files.length > 1 ? 's' : ''}` : ''}{br.replies.length > 0 ? ` · ${br.replies.length} repl${br.replies.length > 1 ? 'ies' : 'y'}` : ''}</div>
                              </div>
                            ...
                            {isExpanded && (
                              <div style={{ borderTop: '1px solid var(--border-light)', padding: '16px 20px' }}>
                                {/* Message */}
                                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Request Details</div>
                                ...
                                {canReply && (
                                  <div style={{ marginTop: 8 }}>
                                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--purple)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Reply to Durga</div>
                                    <textarea
                                      ...
                                      rows={3}
                                      style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid var(--purple-border)', fontSize: 14, boxSizing: 'border-box', resize: 'vertical', outline: 'none', fontFamily: 'inherit' }}
                                    />
                                    <button
                                      onClick={() => handleReply(br.id, project.id)}
                                      disabled={replySubmitting === br.id || !replyText[br.id]?.trim()}
                                      style={{ marginTop: 8, background: 'var(--purple)', color: 'var(--purple-light)', border: 'none', borderRadius: 8, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', opacity: replySubmitting === br.id ? 0.6 : 1 }}>
        ...
        {/* Guidance footer */}
        <div style={{ background: 'var(--amber-light)', border: '1px solid var(--amber-border)', borderRadius: 10, padding: '16px 20px', marginTop: 8 }}>
          <p style={{ margin: 0, fontSize: 14, color: 'var(--amber)', lineHeight: 1.7 }}>
...
function ChecklistRow({ item, index, toggling, onToggle, readonly = false }: {
  ...
  return (
    <div style={{
      ...
      background: item.completed ? 'var(--success-light)' : 'var(--card-hi)',
      border: `1px solid ${item.completed ? '#1F8F66' : 'var(--border)'}`,
      ...
      <div style={{
        ...
        background: item.completed ? 'var(--success)' : 'var(--card)',
        display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: readonly ? 'default' : 'pointer',
      }}>
        {item.completed && (
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
            <path d="M2 6l3 3 5-5" stroke="var(--success-light)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        )}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 13, color: 'var(--ink4)', fontWeight: 600, flexShrink: 0 }}>#{index}</span>
          <span style={{ fontSize: 14, fontWeight: 600, color: item.completed ? 'var(--ink3)' : 'var(--ink)', textDecoration: item.completed ? 'line-through' : 'none' }}>{item.title}</span>
          {catMeta && <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 999, background: 'var(--border-light)', color: 'var(--ink3)', fontWeight: 600, flexShrink: 0 }}>{catMeta}</span>}
        </div>
        {item.description && <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--ink3)', lineHeight: 1.5 }}>{item.description}</p>}
        {item.completed && item.completed_at && (
          <p style={{ margin: '3px 0 0', fontSize: 11, color: 'var(--success)' }}>
