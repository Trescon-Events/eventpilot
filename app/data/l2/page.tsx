  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8FAFB', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #DDE8EE', padding: '0 24px', height: '52px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span style={{ fontSize: '15px', fontWeight: 700, color: '#0F1923' }}>L2 Taxonomy</span>

        {/* Info bar */}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '16px', alignItems: 'center' }}>
          <span style={{ fontSize: '12px', color: '#9CA3AF' }}>
            <span style={{ color: '#00A5A3', fontWeight: 700 }}>{L1_TAXONOMY.length}</span> L1 categories
          </span>
          <span style={{ fontSize: '12px', color: '#9CA3AF' }}>
            <span style={{ color: '#00A5A3', fontWeight: 700 }}>{totalL2}</span> L2 sub-industries
          </span>
        </div>
      </div>
          ...
          <div style={{ maxWidth: '860px' }}>
            <div style={{ background: '#FFFFFF', border: '1px solid #DDE8EE', borderRadius: '14px', padding: '24px', marginBottom: '20px' }}>
              <div style={{ marginBottom: '16px' }}>
                <div style={{ fontSize: '14px', fontWeight: 700, color: '#0F1923', marginBottom: '6px' }}>Classify Industries</div>
                <div style={{ fontSize: '13px', color: '#6B7280' }}>Upload an Excel or CSV with "Company Name" and "Website" columns. Gemini AI will classify each company into L1 + L2 categories.</div>
              </div>
              ...
              <div
                style={{
                  border: `1px dashed ${dragging ? '#00A5A3' : '#DDE8EE'}`,
              ...
              >
                ...
                {file ? (
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: '#6B7280', marginTop: '4px' }}>Click to change</div>
                  </div>
                ) : (
                  <div>
                    <div style={{ fontSize: '14px', color: '#6B7280' }}>Drop Excel or CSV here, or click to browse</div>
                    <div style={{ fontSize: '12px', color: '#9CA3AF', marginTop: '4px' }}>Columns required: Company Name · Website</div>
                  </div>
                )}
              </div>
              ...
              <div style={{ marginTop: '14px', display: 'flex', justifyContent: 'flex-end' }}>
                <button
                  ...
                  style={{
                    ...
                    background: !file || running ? 'rgba(0,165,163,0.2)' : '#00A5A3',
                    color: !file || running ? '#9CA3AF' : '#FFFFFF',
            ...
            {results.length > 0 && (
              <div style={{ background: '#FFFFFF', border: '1px solid #DDE8EE', borderRadius: '14px', overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #DDE8EE', display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: '#0F1923' }}>Classification Results</span>
                  <span style={{ fontSize: '13px', color: '#6B7280' }}>{results.length} companies</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 140px 140px', padding: '10px 20px', background: '#FFFFFF', borderBottom: '1px solid #DDE8EE' }}>
                ...
                {results.map((r, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 140px 140px', padding: '11px 20px', borderBottom: i < results.length - 1 ? '1px solid #DDE8EE' : 'none' }}>
                    <div style={{ fontSize: '13px', color: '#0F1923', fontWeight: 500 }}>{r.company}</div>
                    <div style={{ fontSize: '13px', color: '#00A5A3' }}>{r.website || '—'}</div>
                    <div style={{ fontSize: '12px', color: '#6B7280' }}>{r.l1 || '—'}</div>
                    <div>
                      {r.l2 && (
                        <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '8px', background: 'rgba(0,165,163,0.08)', color: '#00A5A3', fontWeight: 600 }}>{r.l2}</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        {activeTab === 'manager' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap' }}>
              <div style={{ padding: '8px 16px', background: '#FFFFFF', border: '1px solid #DDE8EE', borderRadius: '10px', fontSize: '13px', color: '#0F1923' }}>
                <span style={{ color: '#00A5A3', fontWeight: 800 }}>{L1_TAXONOMY.length}</span> L1 Industries
              </div>
              <div style={{ padding: '8px 16px', background: '#FFFFFF', border: '1px solid #DDE8EE', borderRadius: '10px', fontSize: '13px', color: '#0F1923' }}>
                <span style={{ color: '#00A5A3', fontWeight: 800 }}>{totalL2}</span> L2 Sub-Industries
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '12px' }}>
              {filteredTaxonomy.map(item => (
                <div key={item.l1} style={{ background: '#FFFFFF', border: '1px solid #DDE8EE', borderRadius: '12px', overflow: 'hidden' }}>
                  <div style={{ padding: '10px 14px', background: '#FFFFFF', borderBottom: '1px solid #DDE8EE', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '13px', fontWeight: 700, color: '#0F1923' }}>{item.l1}</span>
                    <span style={{ fontSize: '11px', color: '#9CA3AF' }}>{item.l2s.length}</span>
                  </div>
                  <div style={{ padding: '10px 14px', display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                    {item.l2s.map(l2 => (
                      <span key={l2} style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '8px', background: 'rgba(59,130,246,0.08)', color: '#00A5A3', fontWeight: 500, border: '1px solid rgba(0,165,163,0.1)' }}>
                        {l2}
                      </span>
                    ))}
                  </div>
