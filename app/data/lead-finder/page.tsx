...
function IcpPreview({ icp }: { icp: any }) {
  ...
  return (
    <div style={{ background: 'rgba(0,165,163,0.06)', border: '1px solid rgba(0,165,163,0.2)', borderRadius: '12px', padding: '16px', margin: '12px 0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {fields.map(f => (
          <div key={f.label} style={{ display: 'flex', gap: '12px' }}>
            <span style={{ fontSize: '12px', color: '#9CA3AF', minWidth: '120px', flexShrink: 0 }}>{f.label}</span>
...
export default function LeadFinderPage() {
  ...
  return (
    <div style={{ height: '100vh', background: '#F8FAFB', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Page header */}
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #DDE8EE', padding: '0 24px', height: '52px', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
        <svg width="16" height="16" fill="none" stroke="#00A5A3" strokeWidth="2" strokeLinecap="round" viewBox="0 0 24 24">
          <circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/>
        </svg>
        <span style={{ fontSize: '15px', fontWeight: 700, color: '#0F1923' }}>Lead Finder AI</span>
        <span style={{ fontSize: '13px', color: '#9CA3AF' }}>Conversational ICP builder · Apollo execution</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {sidebarOpen && (
          <div style={{ width: '260px', borderRight: '1px solid #DDE8EE', background: '#FFFFFF', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid #DDE8EE', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#0F1923' }}>Search History</span>
              <button onClick={startNew} style={{ padding: '5px 10px', borderRadius: '7px', background: '#00A5A3', color: '#FFFFFF', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}>
                + New
              </button>
            </div>
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {searches.length === 0 ? (
                ...
                <div
                  ...
                  style={{
                    padding: '12px 16px', cursor: 'pointer', borderBottom: '1px solid #DDE8EE',
                    background: activeId === s.id ? 'rgba(0,165,163,0.08)' : 'transparent',
                    borderLeft: activeId === s.id ? '2px solid #00A5A3' : '2px solid transparent',
                  }}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {/* Chat header */}
          <div style={{ padding: '12px 24px', borderBottom: '1px solid #DDE8EE', background: '#FFFFFF', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <button onClick={() => setSidebarOpen(p => !p)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF', padding: '4px' }}>
              ...
              <div style={{ marginLeft: 'auto' }}>
                <button
                  ...
                  style={{
                    ...
                    background: executing ? 'rgba(0,165,163,0.2)' : '#00A5A3',
                    color: executing ? '#9CA3AF' : '#FFFFFF',
          ...
          {/* Messages */}
          <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', background: '#F8FAFB' }}>
            {transcript.length === 0 && (
              <div style={{ textAlign: 'center', padding: '48px 24px', maxWidth: '500px', margin: '0 auto' }}>
                <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'rgba(0,165,163,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                ...
                <div style={{ fontSize: '15px', color: '#6B7280', lineHeight: 1.65 }}>
                ...
                <div style={{ marginTop: '24px', display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
                  {[
                    ...
                    <button key={ex} onClick={() => setInput(ex)} style={{
                      padding: '8px 14px', borderRadius: '20px', border: '1px solid #DDE8EE',
                      background: '#FFFFFF', fontSize: '13px', color: '#0F1923', cursor: 'pointer',
              ...
              <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                ...
                <div style={{ maxWidth: '70%' }}>
                  <div style={{
                    ...
                    background: msg.role === 'user' ? '#00A5A3' : '#FFFFFF',
                    border: msg.role === 'user' ? 'none' : '1px solid #DDE8EE',
                    fontSize: '15px', color: '#0F1923', lineHeight: 1.65, whiteSpace: 'pre-wrap',
                  }}>
            ...
            {execResult && (
              <div style={{
                background: execResult.error ? 'rgba(248,113,113,0.06)' : 'rgba(52,211,153,0.06)',
                border: `1px solid ${execResult.error ? 'rgba(248,113,113,0.2)' : 'rgba(52,211,153,0.2)'}`,
                borderRadius: '12px', padding: '16px',
              }}>
                {execResult.error ? (
                  <>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#F87171', marginBottom: '4px' }}>Search Failed</div>
                    <div style={{ fontSize: '13px', color: '#6B7280' }}>{execResult.error}</div>
                    {execResult.setup_required && (
                      <div style={{ fontSize: '13px', color: '#FBBF24', marginTop: '8px', background: 'rgba(251,191,36,0.08)', padding: '8px 12px', borderRadius: '8px' }}>
          ...
          {/* Input */}
          <div style={{ padding: '14px 24px', borderTop: '1px solid #DDE8EE', background: '#FFFFFF' }}>
            <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
              <textarea
                ...
                style={{
                  ...
                  fontSize: '15px', color: '#0F1923', resize: 'none', outline: 'none',
                  background: '#FFFFFF', fontFamily: 'inherit', lineHeight: 1.5,
                }}
              />
              <button
                onClick={sendMessage}
                ...
                style={{
                  ...
                  background: !input.trim() || sending ? 'rgba(37,99,235,0.2)' : '#00A5A3',
