...
export function getModuleRegistry(): ModuleDef[] {
  return [
    ...
    {
      key: 'press-releases', label: 'Press Release Studio',
      description: 'Research a story with a grounded AI chat, then generate a press release draft — with full version history and an approval step, scoped to this event.',
      icon: I.docLines, color: '#E26262',
      href: ctx => `/admin/events/${ctx.eventId}/press-releases`,
      needsEvent: true,
      // Chains through the event workspace like every other pure event-
      // internal tool (Stakeholder Hub, Access, Content Check) — NOT
      // 'toolkit', which is only for tools also independently listed as
      // standalone Toolkit products (website-builder, market-intel, etc).
      breadcrumbPattern: '/admin/events/:eventId/press-releases', breadcrumbParent: 'admin-event-workspace',
      access: { kind: 'event_permission', permissionKey: 'sae.content_studio.press_release.view' },
      sidebar: { section: 'events', parent: 'events' },
      toolkitHub: {
        category: 'Events', badge: 'Event Tool',
        access: { kind: 'event_permission', permissionKey: 'sae.content_studio.press_release.view' },
        features: [
          { icon: '◈', label: 'Grounded research chat', detail: 'Ask clarifying questions before drafting, grounded in this event\'s style guide, messaging and facts' },
          { icon: '✎', label: 'Generate from research', detail: 'Turns the research conversation into a headline/dateline/body/boilerplate draft' },
          { icon: '◷', label: 'Version history', detail: 'Every generation is a new immutable version — nothing overwrites a prior draft' },
          { icon: '◉', label: 'Approval step', detail: 'A separate approve permission gates moving a draft to approved' },
        ],
      },
    },
    {
      key: 'operations', label: 'Operations',
      description: 'Event operations workspace — vendors today, with speaker licence procurement and other ops tools to follow.',
      icon: I.docLines, color: '#D9822B',
      href: ctx => `/admin/events/${ctx.eventId}/operations`,
      needsEvent: true,
      breadcrumbPattern: '/admin/events/:eventId/operations', breadcrumbParent: 'admin-event-workspace',
      access: { kind: 'event_permission', permissionKey: 'ops.view' },
      sidebar: { section: 'events', parent: 'events' },
      toolkitHub: {
        category: 'Events', badge: 'Event Tool',
        access: { kind: 'event_permission', permissionKey: 'ops.view' },
        features: [
          { icon: '◈', label: 'Vendor directory', detail: 'Shared list of outside vendors and their contacts, assigned per event' },
        ],
      },
    },
    {
