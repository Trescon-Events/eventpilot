...
export function mapFieldsToRecord(
  ...
  if (isSpeaker) {
    ...
    // bio_full_source/bio_full_text aren't real uploaded files — the
    // caller (the public form route's file-handling loop, or the Details
    // page's manual upload route) stashes 'pdf'|'docx_converted' and the
    // extracted plain text in this same fileUrls map alongside the real
    // bio_full URL, since all three are decided together at upload time
    // by toStoredBioPdf() and this function stays a plain synchronous
    // read of whatever the caller already resolved.
