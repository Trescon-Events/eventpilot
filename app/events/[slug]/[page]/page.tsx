...
type Website  = {
  event_id: string; slug: string; status: string
...
const LOGO_SIZES: Record<string, string> = { sm: '40px', md: '64px', lg: '96px' }

// ── Page ──────────────────────────────────────────────────────────────────────
export default async function EventSubPage({ params }: { params: Promise<{ slug: string; page: string }> }) {
  ...
  const ps = w.page_structure_full
  if (!ps) notFound()

  const currentPage = ps.pages.find(p => p.slug === pageSlug)
  ...
  const types = currentPage.sections.map(s => s.type)
  const needsSpeakers = types.includes('speakers')
  const needsAgenda   = types.some(t => t === 'agenda' || t === 'schedule')
  ...
  const css = `
    *,*::before,*::after{box-sizing:border-box}
    .ev-root{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,sans-serif;background:${P};color:rgba(240,237,232,.9);margin:0;min-height:100vh}
  ...
  function renderSection(sec: Section) {
    ...
    switch (sec.type) {
      ...
      case 'page_hero': {
        ...
        const logoH    = LOGO_SIZES[sec.logo_size ?? 'md']
        ...
        return (
          <section key={sec.id} className="ev-page-hero" style={ds}>
            ...
            <div className="ev-page-hero-inner" style={{ textAlign: ta }}>
              ...
              <h1 className="ev-h1" style={{ color: sec.design.text_light ? 'rgba(240,237,232,1)' : '#0F1923', textAlign: ta }}>{currentPage?.label}</h1>
          ...
          <div className="ev-nav-links">
            {navItems.map(item => {
              ...
              return item.type === 'cta'
                ...
                : <a key={item.id} href={href} className={`ev-nav-link${item.href === pageSlug ? ' ev-active' : ''}`}>{item.label}</a>
            })}
          </div>
        </nav>

        {currentPage.sections.map(sec => renderSection(sec))}

        {/* Footer from PageStructure */}
