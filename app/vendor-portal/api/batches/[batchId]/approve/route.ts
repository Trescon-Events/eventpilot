...
export async function POST(req: NextRequest, { params }: { params: Promise<{ batchId: string }> }) {
  ...
  await logOpsAccess({ eventId: batch.event_id, actorType: 'vendor', actorId: session.userId, action: 'vendor_batch_approved', targetType: 'license_batch', targetId: batch.id, ip })
  await notifyOps(batch.event_id, `Batch ${batch.batch_number} approved by ${session.vendorName}`, 'Batch approved',
