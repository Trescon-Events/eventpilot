  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '28px' }}>
        <div style={{ fontSize: '18px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: '12px' }}>Select Department</div>
      ...
      <div style={{ background: dc.bg, border: `1.5px solid ${dc.border}`, borderRadius: '16px', padding: '22px 28px', marginBottom: '28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: '18px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: dc.accent, marginBottom: '6px' }}>Department</div>
          <div style={{ fontSize: '20px', fontWeight: 800, color: dc.text }}>{dept}</div>
          <div style={{ fontSize: '20px', color: 'rgba(255,255,255,0.5)', marginTop: '4px' }}>{questions.length} questions — 2 shared openers · 3 dept-specific · 4 core closers</div>
        </div>
            ...
            <div key={q.id}>
              {sectionLabel && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: idx === 0 ? '0 0 12px' : '20px 0 12px' }}>
                  ...
                  <span style={{ fontSize: '18px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', whiteSpace: 'nowrap' }}>{sectionLabel}</span>
                  <div style={{ height: '1px', flex: 1, background: 'rgba(255,255,255,0.08)' }} />
                </div>
              )}
              ...
              <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '20px 24px', borderLeft: `3px solid ${tc.bg}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                  <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: dc.bg, border: `1.5px solid ${dc.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 800, color: dc.text, flexShrink: 0 }}>
                    {idx + 1}
                  </div>
                  <span style={{ fontSize: '18px', fontWeight: 700, padding: '3px 9px', borderRadius: '6px', background: tc.bg, color: tc.text, letterSpacing: '0.5px', textTransform: 'uppercase' }}>
                    {tc.label}
                  </span>
                  <span style={{ fontSize: '18px', color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace' }}>{q.id}</span>
                </div>

                <div style={{ fontSize: '18px', fontWeight: 700, color: 'white', lineHeight: 1.5, marginBottom: q.subtext ? '8px' : '0' }}>
                  {q.question}
                </div>
                {q.subtext && (
                  <div style={{ fontSize: '18px', color: 'rgba(255,255,255,0.55)', lineHeight: 1.6 }}>{q.subtext}</div>
                )}
                  ...
                  <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {q.options.map(opt => (
                      <div key={opt} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 14px', borderRadius: '9px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
                        ...
                        <span style={{ fontSize: '18px', color: 'rgba(255,255,255,0.75)' }}>{opt}</span>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    {q.options.map((opt, oi) => (
                      <div key={opt} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '9px 14px', borderRadius: '9px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
                        <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: `hsla(${oi * 30 + 160},55%,50%,0.15)`, border: `1.5px solid hsla(${oi * 30 + 160},55%,50%,0.4)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ fontSize: '18px', fontWeight: 800, color: `hsl(${oi * 30 + 160},60%,65%)` }}>{oi + 1}</span>
                        </div>
                        <span style={{ fontSize: '18px', color: 'rgba(255,255,255,0.75)' }}>{opt}</span>
                      </div>
                    ))}
                ...
                {q.type === 'textarea' && q.placeholder && (
                  <div style={{ marginTop: '14px', padding: '12px 16px', borderRadius: '9px', border: '1px dashed rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)' }}>
                    <span style={{ fontSize: '20px', color: 'rgba(255,255,255,0.3)', fontStyle: 'italic', lineHeight: 1.6 }}>{q.placeholder}</span>
                  </div>
                )}
              </div>
            </div>
