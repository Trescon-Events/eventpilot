...
const DEPT_COLORS: Record<string, { bg: string; text: string; border: string; accent: string }> = {
  'Events':               { bg: 'rgba(255,243,224,0.08)', text: '#FFB74D', border: 'rgba(255,183,77,0.3)',  accent: '#8B1A1A' },
...
const TYPE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  textarea: { label: 'Open text',     bg: '#00897B', text: 'white' },
  chips:    { label: 'Multi-select',  bg: '#C0F43C', text: '#1E2124' },
  scale:    { label: 'Scale 1–5',     bg: '#F4ED3C', text: '#1E2124' },
  select:   { label: 'Single choice', bg: '#8B1A1A', text: 'white' },
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '28px' }}>
        <div style={{ fontSize: '13px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: '12px' }}>Select Department</div>
          ...
          {ALL_DEPARTMENTS.map(d => {
            ...
            return (
              <button key={d} onClick={() => setDept(d)}
                style={{ padding: '7px 16px', borderRadius: '16px', border: `1.5px solid ${active ? c.border : 'rgba(255,255,255,0.1)'}`, background: active ? c.bg : 'transparent', color: active ? c.text : 'rgba(255,255,255,0.55)', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
                {d}
              </button>
            )
          })}
      ...
      <div style={{ background: dc.bg, border: `1.5px solid ${dc.border}`, borderRadius: '16px', padding: '22px 28px', marginBottom: '28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', marginTop: '4px' }}>{questions.length} questions — 2 shared openers · 3 dept-specific · 4 core closers</div>
        </div>
            ...
            <div key={q.id}>
              {sectionLabel && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: idx === 0 ? '0 0 12px' : '20px 0 12px' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,1)', whiteSpace: 'nowrap' }}>{sectionLabel}</span>
                  <div style={{ height: '1px', flex: 1, background: 'rgba(255,255,255,0.08)' }} />
                </div>
              )}
              ...
              <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '20px 24px', borderLeft: `3px solid ${tc.bg}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 700, padding: '3px 9px', borderRadius: '6px', background: tc.bg, color: tc.text, letterSpacing: '0.5px', textTransform: 'uppercase' }}>
                    {tc.label}
                  </span>
                  <span style={{ fontSize: '13px', color: 'rgba(255,255,255,1)', fontFamily: 'monospace' }}>{q.id}</span>
                </div>

                <div style={{ fontSize: '13px', fontWeight: 700, color: 'white', lineHeight: 1.5, marginBottom: q.subtext ? '8px' : '0' }}>
                  {q.question}
                </div>
                {q.subtext && (
                ...
                {q.type === 'chips' && q.options && q.options.length > 0 && (
                  <div style={{ marginTop: '14px', display: 'flex', flexWrap: 'wrap', gap: '7px' }}>
                    {q.options.map(opt => (
                      <span key={opt} style={{ padding: '5px 12px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.12)', fontSize: '13px', color: 'rgba(255,255,255,0.7)', background: 'rgba(255,255,255,0.04)', fontWeight: 600 }}>
                        {opt}
                      </span>
                    ))}
                  </div>
                    ...
                    {q.options.map((opt, oi) => (
                      <div key={opt} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '9px 14px', borderRadius: '9px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
                        <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: `hsla(${oi * 30 + 160},55%,50%,0.15)`, border: `1.5px solid hsla(${oi * 30 + 160},55%,50%,0.4)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ fontSize: '13px', fontWeight: 800, color: `hsl(${oi * 30 + 160},60%,65%)` }}>{oi + 1}</span>
                        </div>
                        <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)' }}>{opt}</span>
                      </div>
                    ))}
                ...
                {q.type === 'textarea' && q.placeholder && (
                  <div style={{ marginTop: '14px', padding: '12px 16px', borderRadius: '9px', border: '1px dashed rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)' }}>
                    <span style={{ fontSize: '13px', color: 'rgba(255,255,255,1)', fontStyle: 'italic', lineHeight: 1.6 }}>{q.placeholder}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <div style={{ marginTop: '32px', padding: '18px 20px', background: 'rgba(0,165,163,0.06)', border: '1px solid rgba(0,165,163,0.2)', borderRadius: '12px', fontSize: '13px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>
        <strong style={{ color: '#00897B' }}>Read-only preview.</strong> Staff go through this questionnaire once they join via the Trescademy portal. Answers are stored and visible in the Interview Answers tab of the admin dashboard.
      </div>
    </div>
  )
}
