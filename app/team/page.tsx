...
// Literal hex (not var) -- these get an alpha suffix concatenated at runtime
// (`${TIER_COLOR[tier]}12`, `${TIER_COLOR[tier]}40`), which CSS vars can't
// support. Brightened from the old light-theme values so each still clears
// ~4.5:1 against the new dark card (#142330).
...
function TeamContent() {
  ...
  if (loading) return (
    <div style={{ fontFamily: 'var(--font-manrope),Manrope,sans-serif', background: 'var(--surface)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ color: 'var(--ink2)', fontSize: '13px' }}>Loading team data…</div>
      </div>
  ...
  if (error) return (
    <div style={{ fontFamily: 'var(--font-manrope),Manrope,sans-serif', background: 'var(--surface)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--ink)' }}>
      <div style={{ textAlign: 'center' }}>
  ...
  if (!loading && members.length === 0) return (
    <div style={{ fontFamily: 'var(--font-manrope),Manrope,sans-serif', background: 'var(--surface)', minHeight: '100vh', color: 'var(--ink)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 64px)', padding: '40px 24px', textAlign: 'center' }}>
        {/* Icon */}
        <div style={{ width: '72px', height: '72px', borderRadius: '16px', background: 'var(--card)', border: '1px solid var(--ink4)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '24px' }}>
          <svg width="32" height="32" fill="none" stroke="var(--ink)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        ...
        <div style={{ fontSize: '13px', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--teal)', marginBottom: '12px' }}>Team Dashboard</div>
        <h2 style={{ fontSize: '36px', fontWeight: 800, color: 'var(--ink)', margin: '0 0 10px' }}>No team members assigned yet</h2>
        <p style={{ fontSize: '13px', color: 'var(--ink2)', maxWidth: '380px', lineHeight: 1.65, margin: '0 0 32px' }}>
    ...
    <div style={{ fontFamily: 'var(--font-manrope),Manrope,sans-serif', background: 'var(--surface)', minHeight: '100vh', color: 'var(--ink)' }}>
      ...
      <div style={{ maxWidth: '1040px', margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '28px' }}>
          {[
            { label: 'Team Size',          value: members.length,   sub: 'total reports',        accent: 'var(--teal-mid)' },
            { label: 'Avg AI Readiness Score',    value: avgScore,          sub: 'team average',          accent: 'var(--purple)' },
            { label: 'Courses Completed',  value: totalDone,         sub: 'across team',           accent: '#7DC520' },
            { label: 'Active Learners',    value: activeCount,       sub: 'completed a course',    accent: '#F5B94D' },
          ].map(({ label, value, sub, accent }) => (
            <div key={label} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderLeft: `4px solid ${accent}`, borderRadius: '14px', padding: '20px 22px', boxShadow: '0 2px 8px rgba(15,25,35,0.06)' }}>
              ...
              <div style={{ fontSize: '40px', fontWeight: 900, color: 'var(--ink)', marginBottom: '4px', lineHeight: 1 }}>{value}</div>
              <div style={{ fontSize: '13px', color: 'var(--ink3)', fontWeight: 600 }}>{sub}</div>
            </div>
          ))}
        </div>

        {/* Tier distribution */}
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '18px 24px', marginBottom: '20px', display: 'flex', gap: '8px', alignItems: 'stretch', flexWrap: 'wrap', boxShadow: '0 2px 8px rgba(15,25,35,0.06)' }}>
          <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '1.5px', flexShrink: 0, display: 'flex', alignItems: 'center', marginRight: '8px' }}>AI Readiness</div>
          {Object.entries(tierCounts).map(([tier, count]) => count > 0 && (
            <div key={tier} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '6px 12px', background: `${TIER_COLOR[tier]}12`, border: `1px solid ${TIER_COLOR[tier]}40`, borderRadius: '8px' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: 900, color: 'var(--ink)', marginLeft: '2px' }}>{count}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginBottom: '28px', background: 'rgba(0,165,163,0.05)', border: '1px solid rgba(0,165,163,0.18)', borderRadius: '16px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: briefState === 'ready' ? '1px solid rgba(0,165,163,0.15)' : 'none' }}>
            <div>
              <div style={{ fontSize: '13px', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--teal)', marginBottom: '4px' }}>AI-Powered</div>
              <div style={{ fontSize: '13px', fontWeight: 800, color: 'var(--ink)' }}>Team Health Brief</div>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              {briefState === 'ready' && (
                <button
                  ...
                  style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ink2)', background: 'none', border: '1px solid var(--ink4)', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  Regenerate
                </button>
              )}
              {(briefState === 'idle' || briefState === 'error') && (
                <button
                  onClick={generateBrief}
                  style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '13px', fontWeight: 800, color: 'var(--teal-light)', background: 'var(--teal-mid)', border: 'none', padding: '9px 20px', borderRadius: '10px', cursor: 'pointer', fontFamily: 'inherit' }}
                >
              ...
              {briefState === 'loading' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: 'var(--teal)', fontWeight: 700 }}>
                  <div style={{ width: '16px', height: '16px', border: '2px solid rgba(0,165,163,0.3)', borderTopColor: 'var(--teal-mid)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                  Analysing team data…
                </div>
              )}
            </div>
            ...
            <div style={{ padding: '24px' }}>
              ...
              {briefStats && (
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '20px' }}>
                  {[
                    { label: 'Avg AIRS', value: briefStats.avgScore, color: 'var(--teal)' },
                    { label: 'Team Size', value: briefStats.teamSize, color: 'var(--ink2)' },
                    { label: 'No Courses Yet', value: briefStats.zeroCourses, color: 'var(--red)' },
                    { label: 'Profile Pending', value: briefStats.noProfile, color: 'var(--purple)' },
                  ].map(({ label, value, color }) => (
                    <div key={label} style={{ background: 'var(--card)', border: '1px solid var(--ink4)', borderRadius: '10px', padding: '8px 14px', display: 'flex', gap: '6px', alignItems: 'baseline', boxShadow: '0 1px 4px rgba(0,165,163,0.06), 0 1px 2px rgba(0,0,0,0.04)' }}>
                      <span style={{ fontSize: '36px', fontWeight: 900, color }}>{value}</span>
                      <span style={{ fontSize: '13px', color: 'var(--ink2)', fontWeight: 600 }}>{label}</span>
                    </div>
                  ))}
                </div>
              )}
                  ...
                  if (line.trim().startsWith('- ') || line.trim().startsWith('• ')) {
                    return (
                      <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '6px' }}>
                        <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: 'var(--teal-mid)', marginTop: '9px', flexShrink: 0 }} />
                        <span>{line.replace(/^[-•]\s/, '')}</span>
                      </div>
                    )
                  }
                  if (/^\d+\./.test(line.trim())) {
                    return <p key={i} style={{ margin: '0 0 6px', fontWeight: 700, color: 'var(--ink)' }}>{line}</p>
                  }
                ...
                <div style={{ marginTop: '20px', padding: '18px 20px', background: 'rgba(164,120,255,0.08)', border: '1px solid rgba(164,120,255,0.2)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: '3px' }}>
                      Biggest gap: <span style={{ color: 'var(--purple)' }}>{gapDept}</span>
                    </div>
                  ...
                  <a
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '13px', fontWeight: 800, color: 'var(--purple)', background: 'var(--purple-light)', border: '1px solid var(--purple-border)', padding: '9px 18px', borderRadius: '10px', textDecoration: 'none', whiteSpace: 'nowrap' }}
                  >
              ...
              <div style={{ marginTop: '16px', fontSize: '13px', color: 'var(--ink)', lineHeight: 1.65, borderTop: '1px solid var(--border)', paddingTop: '14px' }}>
          ...
          {briefState === 'error' && (
            <div style={{ padding: '20px 24px', fontSize: '13px', color: 'var(--red)' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative', flex: '0 0 220px' }}>
            <svg width="12" height="12" fill="none" stroke="var(--ink)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" style={{ position: 'absolute', left: '11px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name or role…"
              style={{ width: '100%', paddingLeft: '32px', paddingRight: '12px', paddingTop: '7px', paddingBottom: '7px', borderRadius: '16px', border: '1px solid var(--ink4)', background: 'var(--card)', color: 'var(--ink)', fontSize: '13px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
          </div>

          {/* Dept filter */}
          {depts.length > 1 && (
            <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)}
              style={{ padding: '7px 12px', borderRadius: '16px', border: '1px solid var(--ink4)', background: 'var(--card)', color: 'var(--ink)', fontSize: '13px', fontFamily: 'inherit', outline: 'none', cursor: 'pointer' }}>
          ...
          <div style={{ marginLeft: 'auto', fontSize: '13px', color: 'var(--ink2)', fontWeight: 600 }}>
        ...
        {/* Team member list */}
        {filtered.length === 0 ? (
          <div style={{ padding: '60px', textAlign: 'center', color: 'var(--ink2)', fontSize: '13px' }}>
            No team members match your filters.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {/* Header row */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 100px 100px 100px 80px', gap: '12px', padding: '8px 20px', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', textTransform: 'uppercase', letterSpacing: '1px' }}>
              <span>Name / Role</span>
            ...
            {filtered.map(m => {
              const tc = TIER_COLOR[m.tier] ?? '#7E93A1'
              return (
                <div key={m.id} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 100px 100px 100px 80px', gap: '12px', alignItems: 'center', padding: '14px 20px', background: 'var(--card)', border: '1px solid var(--ink4)', borderRadius: '12px', boxShadow: '0 1px 4px rgba(0,165,163,0.06), 0 1px 2px rgba(0,0,0,0.04)' }}>
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: '2px' }}>{m.name}</div>
                  ...
                  <div style={{ fontSize: '13px', color: 'var(--ink2)' }}>{m.department ?? '—'}</div>
                  ...
                  <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink2)', textTransform: 'capitalize' }}>{m.track}</div>
                  <div style={{ fontSize: '13px', fontWeight: 800, color: m.completed_courses > 0 ? 'var(--teal)' : 'var(--ink)' }}>
                    {m.completed_courses}
                  </div>
                  <div style={{ fontSize: '13px', color: 'var(--ink2)' }}>
