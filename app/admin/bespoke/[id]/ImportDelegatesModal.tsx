 ...
 * inside the same card language (var(--card) bg, radius 12, var(--border))
    ...
    >
      ...
      >
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, fontSize: '17px', fontWeight: 800, color: 'var(--ink)' }}>Import Delegates from Spreadsheet</h3>
          <button
            onClick={onClose}
            style={{
              padding: '4px 10px', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--card)',
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--amber)', letterSpacing: '1px', marginBottom: '6px' }}>STEP 1</div>
            <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--ink)', marginBottom: '10px' }}>Pick a CSV or XLSX file</div>
            <input
              ref={fileInputRef}
              type="file"
            ...
            {error && (
              <div style={{ marginTop: '10px', padding: '10px 14px', background: 'var(--red-light)', border: '1px solid var(--red-border)', borderRadius: '8px', fontSize: '13px', color: 'var(--red)' }}>
                {error}
              </div>
            )}
          </div>
          ...
          {headers.length > 0 && !result && (
            <>
              <div style={{ marginBottom: '20px' }}>
                <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--amber)', letterSpacing: '1px', marginBottom: '6px' }}>STEP 2</div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--ink)', marginBottom: '10px' }}>Map columns to delegate fields</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  {headers.map(h => (
                    <div key={h} style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '10px 12px', border: '1px solid var(--border)', borderRadius: '8px', background: 'var(--card-hi)' }}>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Source column</div>
                      <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: '4px' }}>{h}</div>
                      <select
                        value={mapping[h] ?? 'ignore'}
                        onChange={e => setMapping(m => ({ ...m, [h]: e.target.value as TargetField }))}
                        style={{
                          padding: '6px 8px', borderRadius: '6px', border: '1px solid var(--border)', fontSize: '12px',
                          fontFamily: 'var(--font-manrope)', background: 'var(--card)', cursor: 'pointer',
                        }}
                      >
                        {TARGET_FIELDS.map(t => (
                ...
                {!Object.values(mapping).some(v => v === 'name') && (
                  <div style={{ marginTop: '10px', padding: '8px 12px', background: 'var(--amber-light)', border: '1px solid var(--amber-border)', borderRadius: '6px', fontSize: '12px', color: 'var(--amber)' }}>
              ...
              {/* Step 3: preview */}
              <div style={{ marginBottom: '20px' }}>
                <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--amber)', letterSpacing: '1px', marginBottom: '6px' }}>STEP 3</div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--ink)', marginBottom: '10px' }}>Preview (first {previewRows.length} of {rows.length})</div>
                <div style={{ overflow: 'auto', border: '1px solid var(--border)', borderRadius: '8px' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    <thead style={{ background: 'var(--card-hi)' }}>
                      <tr>
                        {headers.map(h => (
                          <th key={h} style={{ padding: '8px 10px', textAlign: 'left', fontWeight: 700, color: 'var(--ink3)', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {previewRows.map((r, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid var(--border-light)' }}>
                          {headers.map(h => (
                            <td key={h} style={{ padding: '8px 10px', color: 'var(--ink2)', whiteSpace: 'nowrap', maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                ...
                <div style={{ marginTop: '8px', fontSize: '12px', color: 'var(--ink3)' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', paddingTop: '12px', borderTop: '1px solid var(--border)' }}>
                <button
                  ...
                  disabled={importing}
                  style={{
                    padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--card)',
                    fontSize: '13px', fontWeight: 600, color: 'var(--ink3)', cursor: importing ? 'not-allowed' : 'pointer',
                ...
                <button
                  ...
                  style={{
                    ...
                    background: (canImport && !importing) ? 'var(--amber)' : 'var(--ink4)',
                    color: (canImport && !importing) ? 'var(--amber-light)' : 'var(--ink3)', fontSize: '13px', fontWeight: 700,
          ...
          {/* Result */}
          {result && (
            <div style={{ padding: '16px 20px', background: 'var(--success-light)', border: '1px solid var(--success)', borderRadius: '10px' }}>
              <div style={{ fontSize: '14px', fontWeight: 800, color: 'var(--success)', marginBottom: '8px' }}>Import complete</div>
              <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '13px', color: 'var(--success)', lineHeight: 1.8 }}>
              ...
              {result.errors.length > 0 && (
                <div style={{ marginTop: '12px', padding: '10px 12px', background: 'var(--amber-light)', border: '1px solid var(--amber-border)', borderRadius: '8px', fontSize: '12px', color: 'var(--amber)' }}>
              ...
              <div style={{ marginTop: '14px', display: 'flex', justifyContent: 'flex-end' }}>
                <button
                  onClick={onClose}
                  style={{
                    padding: '8px 20px', borderRadius: '8px', border: 'none', background: 'var(--success)', color: 'var(--success-light)',
