    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--ink)', letterSpacing: '0.2px' }}>{label}</span>
        </div>
...
function DelegateCard({
  ...
  return (
    ...
    >
      <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: '2px' }}>{delegate.name}</div>
      {delegate.company && (
        <div style={{ fontSize: '12px', color: 'var(--ink2)', marginBottom: '2px' }}>{delegate.company}</div>
      )}
      {delegate.title && (
        <div style={{ fontSize: '11px', color: 'var(--ink3)', marginBottom: '6px' }}>{delegate.title}</div>
      )}
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
        <span
          style={{
            fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '4px',
            background: 'var(--border-light)', color: 'var(--ink3)',
          }}
        >
