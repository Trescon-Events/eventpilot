...
const TYPE_META: Record<string, { label: string; color: string; bg: string }> = {
  bug:         { label: 'Bug',         color: 'var(--red)',    bg: '#F1667A15' },
  not_working: { label: 'Not Working', color: 'var(--orange)', bg: '#FB923C15' },
  suggestion:  { label: 'Suggestion',  color: 'var(--info)',   bg: '#5AA9F215' },
  improvement: { label: 'Improvement', color: 'var(--purple)', bg: '#A78BFA15' },
}

const SEVERITY_META: Record<string, { label: string; color: string; bg: string }> = {
  critical: { label: 'Critical', color: 'var(--red)',     bg: '#F1667A15' },
  high:     { label: 'High',     color: 'var(--orange)',  bg: '#FB923C15' },
  medium:   { label: 'Medium',   color: 'var(--amber)',   bg: '#F5B94D15' },
  low:      { label: 'Low',      color: 'var(--success)', bg: '#34D39915' },
}

const STATUS_META: Record<string, { label: string; color: string; bg: string }> = {
  new:          { label: 'New',          color: 'var(--red)',     bg: '#F1667A15' },
  acknowledged: { label: 'Acknowledged', color: 'var(--amber)',   bg: '#F5B94D15' },
  in_progress:  { label: 'In Progress',  color: 'var(--info)',    bg: '#5AA9F215' },
  resolved:     { label: 'Resolved',     color: 'var(--success)', bg: '#34D39915' },
  wont_fix:     { label: "Won't Fix",    color: 'var(--ink3)',    bg: '#7E93A115' },
}
...
function ReplyModal({ review, onClose, onUpdate }: {
  ...
  const typeMeta = TYPE_META[review.review_type] ?? { label: review.review_type, color: C.muted, bg: 'rgba(255,255,255,0.06)' }
  const sevMeta  = SEVERITY_META[review.severity] ?? { label: review.severity, color: C.muted, bg: 'rgba(255,255,255,0.06)' }
      ...
      <div style={{ background: C.surface, borderRadius: '16px', width: '100%', maxWidth: '680px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.2)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                <Badge meta={typeMeta} />
                <Badge meta={sevMeta} />
                <Badge meta={STATUS_META[review.status] ?? { label: review.status, color: C.muted, bg: 'rgba(255,255,255,0.06)' }} />
              </div>
            ...
            <button onClick={onClose} style={{ background: 'var(--border-light)', border: 'none', borderRadius: '8px', width: '32px', height: '32px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div ref={threadRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            ...
            <div style={{ maxWidth: '80%', padding: '10px 14px', borderRadius: '2px 12px 12px 12px', background: 'var(--border-light)', color: C.text, fontSize: '14px', lineHeight: 1.65, whiteSpace: 'pre-wrap' as const }}>
              {review.description}
            </div>
          </div>
            ...
            if (c.is_status_change) {
              ...
              return (
                <div key={c.id} style={{ display: 'flex', justifyContent: 'center' }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: sm?.bg ?? 'var(--border-light)', border: `1px solid ${sm?.color ? `color-mix(in srgb, ${sm.color} 40%, transparent)` : C.border}`, borderRadius: '20px', padding: '3px 12px' }}>
            ...
            return (
              <div key={c.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isAdmin ? 'flex-end' : 'flex-start' }}>
                ...
                <div style={{
                  ...
                  background: isAdmin ? C.teal : 'var(--border-light)',
                  color: isAdmin ? 'var(--teal-light)' : C.text,
        ...
        <div style={{ borderTop: `1px solid ${C.border}`, padding: '12px 20px 16px', flexShrink: 0, background: C.surface }}>
          {sendError && (
            <div style={{ fontSize: '12px', color: 'var(--red)', fontWeight: 600, marginBottom: '8px' }}>{sendError}</div>
          )}
          <textarea
            rows={3}
            value={response}
            ...
            style={{ width: '100%', background: 'var(--surface)', border: `1px solid ${C.border}`, borderRadius: '10px', padding: '10px 14px', fontSize: '14px', color: C.text, outline: 'none', fontFamily: 'inherit', resize: 'none' as const, boxSizing: 'border-box' as const, lineHeight: 1.6 }}
          />
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
            ...
            <button
              ...
              style={{
                ...
                background: response.trim() && !sending ? C.teal : 'var(--surface)',
                color: response.trim() && !sending ? 'var(--teal-light)' : C.muted,
...
function ReviewCard({ review, onReply, onUpdate }: {
  ...
  const typeMeta     = TYPE_META[review.review_type]  ?? { label: review.review_type, color: C.muted, bg: 'color-mix(in srgb, ' + C.muted + ' 8%, transparent)' }
  const severityMeta = SEVERITY_META[review.severity] ?? { label: review.severity,    color: C.muted, bg: 'color-mix(in srgb, ' + C.muted + ' 8%, transparent)' }
  const statusMeta   = STATUS_META[review.status]     ?? { label: review.status,      color: C.muted, bg: 'color-mix(in srgb, ' + C.muted + ' 8%, transparent)' }
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${review.status === 'new' ? '#F1667A30' : C.border}`,
      ...
      <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: C.teal, color: 'var(--teal-light)', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
          >
      ...
      {expanded && (
        <div style={{ borderTop: `1px solid ${C.border}`, padding: '0 18px 18px' }}>
          ...
          <div style={{ marginTop: '14px' }}>
            ...
            <div style={{ fontSize: '14px', color: C.text, lineHeight: 1.7, whiteSpace: 'pre-wrap' as const, background: 'var(--surface)', padding: '12px 14px', borderRadius: '10px', border: `1px solid ${C.border}` }}>
              {review.description}
            </div>
          </div>
              ...
              {STATUS_FLOW.map(st => {
                ...
                return (
                  <button key={st} onClick={() => !active && changeStatus(st)} disabled={active}
                    style={{ padding: '6px 14px', borderRadius: '8px', fontSize: '12px', fontWeight: 700, cursor: active ? 'default' : 'pointer', fontFamily: 'inherit', background: active ? m.bg : C.surface, color: active ? m.color : C.muted, border: `1px solid ${active ? `color-mix(in srgb, ${m.color} 60%, transparent)` : C.border}`, transition: 'all 0.15s' }}>
                    {m.label}
                  </button>
                )
              })}
          ...
          <div style={{ marginTop: '16px' }}>
            <div style={{ fontSize: '11px', fontWeight: 700, color: C.muted, letterSpacing: '0.8px', textTransform: 'uppercase' as const, marginBottom: '10px' }}>
              ...
              {comments.filter(c => !c.is_status_change).length > 0 && (
                <span style={{ marginLeft: '6px', background: C.teal, color: 'var(--teal-light)', borderRadius: '10px', padding: '1px 7px', fontSize: '10px', fontWeight: 800 }}>
                ...
                {comments.map(c => {
                  ...
                  return (
                    <div key={c.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isAdmin ? 'flex-end' : 'flex-start' }}>
                      ...
                      <div style={{ maxWidth: '85%', padding: '8px 12px', borderRadius: isAdmin ? '10px 2px 10px 10px' : '2px 10px 10px 10px', background: isAdmin ? C.teal : 'var(--border-light)', color: isAdmin ? 'var(--teal-light)' : C.text, fontSize: '13px', lineHeight: 1.6, whiteSpace: 'pre-wrap' as const }}>
                        {c.message}
                      </div>
                    </div>
                  )
          ...
          <div style={{ marginTop: '16px' }}>
            ...
            <textarea
              ...
              style={{ width: '100%', background: 'var(--surface)', border: `1px solid ${C.border}`, borderRadius: '10px', padding: '10px 14px', fontSize: '13px', color: C.text, outline: 'none', fontFamily: 'inherit', resize: 'vertical' as const, minHeight: '72px', boxSizing: 'border-box' as const, lineHeight: 1.6 }}
            />
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '8px' }}>
              <button onClick={saveNotes} disabled={savingNotes}
                style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: notesMsg === 'Saved' ? 'var(--success)' : 'var(--surface)', color: notesMsg === 'Saved' ? 'var(--success-light)' : C.text, border: `1px solid ${notesMsg === 'Saved' ? 'var(--success)' : C.border}`, borderRadius: '8px', padding: '7px 18px', fontSize: '13px', fontWeight: 700, cursor: savingNotes ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: savingNotes ? 0.6 : 1, transition: 'all 0.2s' }}>
              ...
              {notesMsg && notesMsg !== 'Saved' && <span style={{ fontSize: '12px', color: 'var(--red)', fontWeight: 600 }}>{notesMsg}</span>}
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '32px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '28px' }}>
          {[
            { label: 'New',          value: stats.new,         color: 'var(--red)',     action: () => setFilterStatus('new')         },
            { label: 'Acknowledged', value: stats.acknowledged, color: 'var(--amber)',   action: () => setFilterStatus('acknowledged') },
            { label: 'In Progress',  value: stats.in_progress,  color: 'var(--info)',    action: () => setFilterStatus('in_progress')  },
            { label: 'Resolved',     value: stats.resolved,     color: 'var(--success)', action: () => setFilterStatus('resolved')     },
            { label: 'Critical',     value: stats.critical,     color: 'var(--red)',     action: () => setFilterSeverity('critical')   },
          ].map(s => (
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' as const, marginBottom: '20px', alignItems: 'center' }}>
          ...
          {(filterTool !== 'all' || filterType !== 'all' || filterStatus !== 'all' || filterSeverity !== 'all' || search) && (
            <button onClick={() => { setFilterTool('all'); setFilterType('all'); setFilterStatus('all'); setFilterSeverity('all'); setSearch('') }}
              style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '9px', padding: '8px 14px', fontSize: '12px', color: 'var(--red)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700 }}>
              Clear
            </button>
          )}
        </div>
