...
export default function VendorsView() {
  ...
  const [scopeInfo, setScopeInfo] = useState<{ kind: string; id: string; name: string } | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      const vRes = await fetch(`/api/events/operations/vendors?${scope.query}`)
      ...
      if (cancelled) return
      if (vendorsBody) { setVendors(vendorsBody.vendors); setScopeInfo(vendorsBody.scope); setEventName(vendorsBody.scope?.name ?? null) }
      if (vendorsErr) setError(vendorsErr)
      setLoading(false)
    })()
    return () => { cancelled = true }
  }, [scope.query, reloadTick])
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface)' }}>
      <PageHeader
        eyebrow="Operations"
        title="Vendors"
        description={scopeInfo?.kind === 'umbrella' ? `A shared directory. Vendors assigned here handle licences for every event in ${scopeInfo.name}.` : 'A shared directory — the same vendor can serve many events. Assign the ones engaged on this event.'}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {vendors.map(v => (
              <div key={v.id} style={{ padding: '16px 18px', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--card)', opacity: v.active ? 1 : 0.6, display: 'flex', justifyContent: 'space-between', gap: '16px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                {v.assigned && v.active && <VendorLogins vendor={v} setError={setError} />}
              </div>
            ))}
          </div>
        )}
...
function VendorLogins({ vendor, setError }: { vendor: Vendor; setError: (e: string | null) => void }) {
  const scope = useOpsScope()
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      const res = await fetch(`/api/events/operations/vendor-users?${scope.query}&vendor_id=${vendor.id}`)
      const body = await res.json().catch(() => null)
      if (cancelled) return
      if (res.ok) setUsers(body); else setError(body?.error ?? 'Could not load logins.')
    })()
    return () => { cancelled = true }
  }, [open, tick, scope.query, vendor.id, setError])
  const reload = () => setTick(t => t + 1)

  async function create() {
    setBusy('create'); setError(null); setNote(null)
    const res = await fetch('/api/events/operations/vendor-users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...scope.body, vendor_id: vendor.id, name, email }) })
    setBusy(null)
    const body = await res.json().catch(() => null)
  ...
  async function act(u: PortalUser, action: 'send_link' | 'disable' | 'enable') {
    ...
    const res = await fetch(`/api/events/operations/vendor-users/${u.id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...scope.body, action }) })
    setBusy(null)
    const body = await res.json().catch(() => null)
...
function VendorForm({ vendor, onDone, onCancel, setError }: {
  vendor: Vendor | null; onDone: () => void; onCancel: () => void; setError: (e: string | null) => void
}) {
  const scope = useOpsScope()
  ...
  async function save() {
    ...
    const res = vendor
      ...
      : await fetch('/api/events/operations/vendors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...payload, ...scope.body, assign: true }) })
    setSaving(false)
