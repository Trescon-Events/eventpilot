...
/* Post-publish "please like and share" composer (2026-08-27) — same
   two-step compose/edit → send shape as SendForExternalApprovalComposer,
   built directly against that precedent, but a one-way FYI (no review
   token/link — the body already carries the real published links) rather
   than an approval request. Only handles the FIRST send; the "Send
   Reminder" action after that is a single click straight against
   notify-external/remind (no need to reopen this composer — see
   AnnouncementDetailPanel.tsx's After Publishing section).

   Additional Contacts (2026-09-20, Madhu; CC-only fixed 2026-09-22) —
   speaker-only (speakerId is undefined for a partner announcement, a
   no-op there). Same treatment as SendForExternalApprovalComposer.tsx/
   SendToSpeakerComposer.tsx: the speaker's saved Additional Contacts are
   never a selectable "To" — CC defaults to all of them on load, but the
   only quick-pick recipients are the speaker themselves or "someone
   else." Copied verbatim rather than re-derived, same precedent as the
   rest of this composer. */
...
export default function NotifyExternalComposer({
  ...
  async function startCompose() {
    ...
    try {
      const res = await fetch(`/api/events/stakeholders/announcements/${announcementId}/notify-external/compose`, {
  ...
  async function send() {
    ...
    try {
      const res = await fetch(`/api/events/stakeholders/announcements/${announcementId}/notify-external/send`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'color-mix(in srgb, black 60%, transparent)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '680px', maxWidth: '95%', maxHeight: '90vh', overflowY: 'auto', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ fontSize: '17px', fontWeight: 800, color: 'var(--ink)' }}>Notify {stakeholderName} — It&apos;s Live</div>
