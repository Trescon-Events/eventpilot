...
// ── Shared style tokens ────────────────────────────────────────────────────────
const S = {
  card:    { background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px' } as React.CSSProperties,
  label:   { fontSize: '11px', fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase' as const, letterSpacing: '1.5px', marginBottom: '8px', display: 'block' as const },
  input:   { width: '100%', padding: '10px 13px', borderRadius: '8px', border: '1px solid var(--border)', fontSize: '14px', fontFamily: 'inherit', color: 'var(--ink)', background: 'var(--border-light)', boxSizing: 'border-box' as const, outline: 'none' },
  textarea:{ width: '100%', padding: '10px 13px', borderRadius: '8px', border: '1px solid var(--border)', fontSize: '14px', fontFamily: 'inherit', color: 'var(--ink)', background: 'var(--border-light)', resize: 'vertical' as const, lineHeight: 1.6, boxSizing: 'border-box' as const, outline: 'none' },
  sectionTitle: { fontSize: '13px', fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase' as const, letterSpacing: '1.5px', marginBottom: '20px' },
  chip:    { display: 'inline-flex', alignItems: 'center' as const, gap: '6px', padding: '5px 12px', borderRadius: '20px', background: 'rgba(18,201,189,0.1)', border: '1px solid rgba(18,201,189,0.25)', fontSize: '13px', fontWeight: 700, color: 'var(--teal-mid)' },
  addBtn:  { padding: '5px 12px', borderRadius: '20px', border: '1px dashed var(--border)', background: 'transparent', fontSize: '13px', color: 'var(--ink3)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700 },
}
      ...
      {values.map(v => (
        <span key={v} style={S.chip}>
          ...
          <button onClick={() => onChange(values.filter(x => x !== v))}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--teal-mid)', padding: 0, fontSize: '14px', fontWeight: 900, lineHeight: 1 }}>×</button>
        </span>
      ))}
      {adding
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {values.map((v, i) => (
        <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <div style={{ width: '22px', height: '22px', borderRadius: '6px', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 800, color: 'var(--ink3)', flexShrink: 0 }}>{i + 1}</div>
          ...
          <button onClick={() => onChange(values.filter((_, j) => j !== i))}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink4)', fontSize: '18px', lineHeight: 1, padding: '4px', flexShrink: 0 }}>×</button>
        </div>
      ))}
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-manrope), Manrope, sans-serif' }}>
        <span style={{ fontSize: '15px', color: 'var(--ink3)' }}>Loading Brand Studio…</span>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface)', fontFamily: 'var(--font-manrope), Manrope, sans-serif', color: 'var(--ink)' }}>
      ...
      <nav style={{ background: 'var(--card)', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '0 32px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div>
            ...
            <div style={{ fontSize: '15px', fontWeight: 900, color: 'var(--ink)' }}>Brand Studio</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 14px', borderRadius: '20px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
            <div style={{ width: '60px', height: '4px', borderRadius: '2px', background: 'rgba(255,255,255,0.1)', overflow: 'hidden' }}>
              <div style={{ width: `${Math.round(completedSections / 9 * 100)}%`, height: '100%', background: 'var(--lime)', borderRadius: '2px', transition: 'width 0.3s' }} />
            </div>
          ...
          {guidelines?.build_mode && (
            <div style={{ padding: '5px 12px', borderRadius: '20px', background: guidelines.build_mode === 'pdf_extracted' ? 'rgba(18,201,189,0.2)' : 'rgba(255,255,255,0.08)', fontSize: '11px', fontWeight: 700, color: guidelines.build_mode === 'pdf_extracted' ? 'var(--teal-mid)' : 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '1px' }}>
          ...
          <button
            ...
            style={{ padding: '10px 20px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.08)', color: 'var(--ink2)', fontSize: '13px', fontWeight: 800, cursor: guidelines?.brand_name ? 'pointer' : 'not-allowed', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '7px', opacity: guidelines?.brand_name ? 1 : 0.4 }}>
          ...
          <button onClick={save} disabled={saving}
            style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', background: saving ? 'var(--lime-border)' : 'var(--lime)', color: 'var(--lime-dark)', fontSize: '13px', fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '7px' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 32px 80px' }}>
        ...
        {/* Message */}
        {msg && (
          <div style={{ marginBottom: '12px', padding: '12px 18px', borderRadius: '10px', background: msg.ok ? 'rgba(192,244,60,0.08)' : 'rgba(241,102,122,0.08)', border: `1px solid ${msg.ok ? 'rgba(192,244,60,0.3)' : 'rgba(241,102,122,0.3)'}`, color: msg.ok ? 'var(--lime)' : 'var(--red)', fontSize: '14px', fontWeight: 600, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            {msg.text}
        ...
        {msg?.ok && msg.text.includes('saved') && (
          <div style={{ marginBottom: '20px', padding: '14px 20px', borderRadius: '12px', background: 'rgba(14,167,157,0.05)', border: '1px solid rgba(14,167,157,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--teal-mid)' }}>Brand guidelines saved. Next: build the event website using these guidelines.</span>
            </div>
            <Link href={`/admin/events/${eventId}/website`}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '9px 18px', borderRadius: '9px', background: 'var(--teal)', color: 'var(--teal-light)', fontSize: '13px', fontWeight: 800, textDecoration: 'none', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '28px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '5px', overflowX: 'auto', flexWrap: 'nowrap' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ padding: '8px 16px', borderRadius: '9px', border: 'none', background: tab === t.id ? 'var(--surface)' : 'transparent', color: tab === t.id ? 'var(--lime)' : 'var(--ink3)', fontSize: '13px', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', transition: 'all 0.15s', flexShrink: 0 }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === 'upload' && (
          <div style={{ display: 'grid', gap: '24px' }}>
            <div style={S.card}>
              ...
              <p style={{ fontSize: '14px', color: 'var(--ink3)', lineHeight: 1.6, marginBottom: '24px', marginTop: 0 }}>
              ...
              <label
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '14px', padding: '40px 24px', border: `2px dashed ${pdfUploading ? 'var(--teal-mid)' : 'var(--border)'}`, borderRadius: '14px', background: pdfUploading ? 'rgba(18,201,189,0.04)' : 'var(--border-light)', cursor: pdfUploading ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}
                ...
                {pdfUploading ? (
                  <>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--teal-mid)', marginBottom: '6px' }}>
                        {extracting ? 'Gemini is analysing the brand book…' : extractProgress || 'Processing…'}
                      </div>
                      <div style={{ fontSize: '13px', color: 'var(--ink3)' }}>This can take 30–60 seconds for large PDFs</div>
                    </div>
                  ...
                  <>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--ink2)', marginBottom: '5px' }}>Drop brand PDF here or click to browse</div>
                      <div style={{ fontSize: '13px', color: 'var(--ink3)' }}>PDF only · Up to 50 MB · AI extracts all 9 brand sections</div>
                    </div>
                    {pdfFileName && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px', borderRadius: '20px', background: 'rgba(192,244,60,0.1)', border: '1px solid rgba(192,244,60,0.3)' }}>
                        <svg width="12" height="12" fill="none" stroke="var(--lime)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                        <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--lime)' }}>{pdfFileName}</span>
                      </div>
                    )}
                  </>
                )}
            ...
            <div style={S.card}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                {[
                  ...
                  <div key={section} style={{ padding: '14px 16px', borderRadius: '10px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                    <div style={{ fontSize: '12px', fontWeight: 800, color: 'var(--ink)', marginBottom: '8px' }}>{section}</div>
                    {items.map(item => (
                      <div key={item} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                        ...
                        <span style={{ fontSize: '12px', color: 'var(--ink3)' }}>{item}</span>
                      </div>
                    ))}
                  </div>
                ))}
                ...
                {archetypes.map((a, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr auto', gap: '10px', alignItems: 'center' }}>
                    ...
                    <button onClick={() => setArchetypes(archetypes.filter((_, j) => j !== i))}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink4)', fontSize: '18px', padding: '4px' }}>×</button>
                  </div>
                ))}
        ...
        {tab === 'colors' && (
          <div style={{ display: 'grid', gap: '20px' }}>
            <div style={S.card}>
              ...
              {colorPalette.length === 0 && (
                <div style={{ padding: '32px', textAlign: 'center', color: 'var(--ink3)', fontSize: '14px' }}>No colours yet — import a PDF or add manually.</div>
              )}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {colorPalette.map((c, i) => (
                  <div key={i} style={{ padding: '16px', borderRadius: '10px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '52px 1fr 1fr 140px auto', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
                      {/* Swatch + picker */}
                      <div style={{ position: 'relative' }}>
                        <div style={{ width: '48px', height: '48px', borderRadius: '10px', background: c.hex, border: '2px solid var(--border)', cursor: 'pointer' }} />
                      ...
                      <button onClick={() => setColorPalette(colorPalette.filter((_, j) => j !== i))}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink4)', fontSize: '18px', padding: '4px', alignSelf: 'center' }}>×</button>
                    </div>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {colorPalette.filter(c => /^#[0-9A-Fa-f]{6}$/.test(c.hex)).map((c, i) => (
                    <div key={i} style={{ textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink)', marginBottom: '2px' }}>{c.name || c.role}</div>
                      <div style={{ fontSize: '11px', fontFamily: 'monospace', color: 'var(--ink3)' }}>{c.hex.toUpperCase()}</div>
                      <div style={{ fontSize: '10px', color: 'var(--ink4)', textTransform: 'capitalize' }}>{c.role}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {tab === 'typography' && (
          <div style={{ display: 'grid', gap: '20px' }}>
            <div style={S.card}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '18px', marginBottom: '24px' }}>
                <Field label="Heading Font">
                  ...
                  <div style={{ marginTop: '10px', padding: '12px', borderRadius: '8px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                    <div style={{ fontSize: '20px', fontWeight: 900, fontFamily: headingFont + ', sans-serif' }}>The Future of AI</div>
                    <div style={{ fontSize: '11px', color: 'var(--ink4)', marginTop: '4px', fontWeight: 700 }}>{headingFont} — Heading</div>
                  </div>
                </Field>
                <Field label="Body Font">
                  ...
                  <div style={{ marginTop: '10px', padding: '12px', borderRadius: '8px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                    <div style={{ fontSize: '14px', fontFamily: bodyFont + ', sans-serif', color: 'var(--ink2)', lineHeight: 1.6 }}>Connecting global leaders to shape the next decade of technology and innovation.</div>
                    <div style={{ fontSize: '11px', color: 'var(--ink4)', marginTop: '4px', fontWeight: 700 }}>{bodyFont} — Body</div>
                  </div>
                </Field>
                <Field label="Modular Scale Ratio">
                  ...
                  <div style={{ marginTop: '10px', fontSize: '12px', color: 'var(--ink3)', lineHeight: 1.5 }}>Controls the proportional growth between type levels. 1.200 = Minor Third.</div>
                </Field>
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              {typeScale.length === 0 && (
                <div style={{ padding: '24px', textAlign: 'center', color: 'var(--ink3)', fontSize: '14px' }}>No type levels yet — import a PDF or add manually.</div>
              )}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {typeScale.map((t, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 100px 1fr auto', gap: '10px', alignItems: 'center', padding: '12px', borderRadius: '10px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                ...
                {typeScale.length > 0 && (
                  <div style={{ fontSize: '11px', color: 'var(--ink4)', fontWeight: 700 }}>Level · Size (px) · Weight · Line height · Usage</div>
                )}
              </div>
            </div>
        ...
        {tab === 'patterns' && (
          <div style={S.card}>
            ...
            {patternAssets.length === 0 && (
              <div style={{ padding: '32px', textAlign: 'center', color: 'var(--ink3)', fontSize: '14px' }}>No patterns yet — import a PDF to extract pattern info, then upload the pattern files here.</div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {patternAssets.map((p, i) => (
                <div key={i} style={{ padding: '16px', borderRadius: '10px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                  ...
                  {p.url && (
                    <div style={{ width: '100%', height: '80px', borderRadius: '8px', backgroundImage: `url(${p.url})`, backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid var(--surface)' }} />
                  )}
                  <div style={{ marginTop: '10px', fontSize: '12px', color: 'var(--ink3)' }}>Pattern file URL (upload via website builder patterns section or paste URL):</div>
          ...
          <div style={{ display: 'grid', gap: '20px' }}>
            ...
            <div style={S.card}>
              ...
              {imageryTreatments.map((t, i) => (
                <div key={i} style={{ padding: '16px', borderRadius: '10px', background: 'var(--card-hi)', border: '1px solid var(--surface)', marginBottom: '12px' }}>
                  ...
                  <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '6px' }}>Use Cases</div>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                {spacingTokens.map((t, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 12px', borderRadius: '10px', background: 'var(--card-hi)', border: '1px solid var(--surface)' }}>
                    <input value={t.name} onChange={e => { const a = [...spacingTokens]; a[i] = { ...t, name: e.target.value }; setSpacingTokens(a) }}
                      placeholder="XS" style={{ width: '50px', padding: '4px 6px', borderRadius: '6px', border: '1px solid var(--border)', fontSize: '12px', fontFamily: 'inherit', color: 'var(--ink)', fontWeight: 700, textAlign: 'center' }} />
                    <input type="number" value={t.value_px} onChange={e => { const a = [...spacingTokens]; a[i] = { ...t, value_px: Number(e.target.value) }; setSpacingTokens(a) }}
                      style={{ width: '50px', padding: '4px 6px', borderRadius: '6px', border: '1px solid var(--border)', fontSize: '12px', fontFamily: 'monospace', color: 'var(--ink3)', textAlign: 'center' }} />
                    <span style={{ fontSize: '11px', color: 'var(--ink4)' }}>px</span>
                    <button onClick={() => setSpacingTokens(spacingTokens.filter((_, j) => j !== i))}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink4)', fontSize: '14px', padding: '2px', lineHeight: 1 }}>×</button>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab === 'standards' && (
          <div style={{ display: 'grid', gap: '20px' }}>
            ...
            <div style={S.card}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--ink3)', marginBottom: '16px', marginTop: '-8px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '16px' }}>
                {['15 August 2025', 'August 15, 2025', 'Aug 15, 2025', '15.08.25', 'Custom'].map(opt => (
                  <button key={opt} onClick={() => setStdDateFormat(opt)}
                    style={{ padding: '12px 16px', borderRadius: '10px', border: `2px solid ${stdDateFormat === opt ? 'var(--teal-mid)' : 'var(--border)'}`, background: stdDateFormat === opt ? 'rgba(18,201,189,0.08)' : 'var(--border-light)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px', fontWeight: 700, color: stdDateFormat === opt ? 'var(--teal-mid)' : 'var(--ink3)', textAlign: 'left' as const, transition: 'all 0.15s' }}>
              ...
              <div style={{ padding: '12px 16px', borderRadius: '10px', background: 'var(--teal-light)', border: '1px solid var(--teal-border)', marginTop: '4px' }}>
                <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--teal-mid)', textTransform: 'uppercase' as const, letterSpacing: '0.5px' }}>Preview: </span>
                <span style={{ fontSize: '15px', fontWeight: 800, color: 'var(--teal-mid)' }}>{stdDateFormat === 'Custom' ? stdDateCustom || '—' : stdDateFormat}</span>
              </div>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '16px' }}>
                {['Venue name only', 'Venue name + City', 'Full address', 'City only', 'Custom'].map(opt => (
                  <button key={opt} onClick={() => setStdVenueFormat(opt)}
                    style={{ padding: '12px 16px', borderRadius: '10px', border: `2px solid ${stdVenueFormat === opt ? 'var(--teal-mid)' : 'var(--border)'}`, background: stdVenueFormat === opt ? 'rgba(18,201,189,0.08)' : 'var(--border-light)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px', fontWeight: 700, color: stdVenueFormat === opt ? 'var(--teal-mid)' : 'var(--ink3)', textAlign: 'left' as const, transition: 'all 0.15s' }}>
                    {opt}
                  </button>
                ))}
              </div>
            ...
            <div style={S.card}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--ink3)', marginBottom: '20px', marginTop: '-8px' }}>
                ...
                <Field label="Letter Case">
                  <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px' }}>
                    {['ALL CAPS', 'Title Case', 'Sentence case'].map(opt => (
                      <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: 600, color: stdTaglineCase === opt ? 'var(--ink)' : 'var(--ink3)' }}>
                        <input type="radio" name="tagline_case" checked={stdTaglineCase === opt} onChange={() => setStdTaglineCase(opt)}
                          style={{ accentColor: 'var(--teal-mid)', width: 16, height: 16 }} />
                        {opt}
                      </label>
                    ))}
                  </div>
                ...
                <Field label="Font Weight">
                  <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px' }}>
                    {['Black (900)', 'Bold (700)', 'SemiBold (600)', 'Regular (400)'].map(opt => (
                      <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: 600, color: stdTaglineWeight === opt ? 'var(--ink)' : 'var(--ink3)' }}>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
                {[
                  ...
                  <div key={key}>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink3)', textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: '8px' }}>{label}</div>
                    {value ? (
                      <div style={{ position: 'relative' as const, borderRadius: '10px', overflow: 'hidden', border: '1px solid var(--border)' }}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={value} alt={label} style={{ width: '100%', display: 'block', maxHeight: '160px', objectFit: 'cover', background: 'var(--surface)' }} />
                        <button onClick={() => set('')}
                          style={{ position: 'absolute' as const, top: 6, right: 6, width: 24, height: 24, borderRadius: '50%', background: 'rgba(15,25,35,0.7)', border: 'none', color: 'var(--ink)', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}>×</button>
                      </div>
                    ) : (
                      <div>
        ...
        {tab === 'assets' && (
          <div style={{ display: 'grid', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '24px' }}>
                {ASSET_TYPES.map(a => (
                  <button key={a.id} onClick={() => setSelectedAsset(a)}
                    style={{ padding: '12px 16px', borderRadius: '10px', border: `2px solid ${selectedAsset.id === a.id ? 'var(--teal-mid)' : 'var(--border)'}`, background: selectedAsset.id === a.id ? 'rgba(18,201,189,0.08)' : 'var(--border-light)', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
                    <div style={{ fontSize: '13px', fontWeight: 800, color: selectedAsset.id === a.id ? 'var(--teal-mid)' : 'var(--ink)', marginBottom: '2px' }}>{a.label}</div>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink4)' }}>{a.aspect}</div>
                  </button>
                ))}
              </div>
              <Field label="Image Prompt">
                <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={4} style={S.textarea} />
                <div style={{ fontSize: '12px', color: 'var(--ink4)', marginTop: '6px' }}>Imagen 3 · ~10–20 seconds</div>
              </Field>
              <button onClick={generateImage} disabled={generatingImg || !prompt.trim()}
                style={{ padding: '12px 24px', borderRadius: '10px', border: 'none', background: generatingImg ? 'var(--ink4)' : 'var(--teal-mid)', color: 'var(--teal-light)', fontSize: '14px', fontWeight: 800, cursor: generatingImg ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: '9px' }}>
                {generatingImg
            ...
            {imgError && <div style={{ padding: '14px 18px', borderRadius: '10px', background: 'rgba(241,102,122,0.08)', border: '1px solid rgba(241,102,122,0.3)', color: 'var(--red)', fontSize: '14px' }}>{imgError}</div>}
            {generatedImg && (
              <div style={S.card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                  <div style={{ fontSize: '13px', fontWeight: 800, color: 'var(--ink)' }}>{selectedAsset.label}</div>
                  <a href={generatedImg} download target="_blank" rel="noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '8px', background: 'var(--surface)', color: 'var(--ink)', fontSize: '13px', fontWeight: 700, textDecoration: 'none' }}>
                ...
                <img src={generatedImg} alt={selectedAsset.label} style={{ width: '100%', display: 'block', maxHeight: '560px', objectFit: 'contain', borderRadius: '8px', background: 'var(--surface)' }} />
              </div>
            )}
            {/* Gallery */}
            {assets.length > 0 && (
              <div style={S.card}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '16px' }}>
                  {assets.map(a => (
                    <div key={a.id} style={{ borderRadius: '10px', border: '1px solid var(--surface)', overflow: 'hidden', background: 'var(--card-hi)' }}>
                      <img src={a.image_url} alt={a.label ?? a.asset_type} style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover', display: 'block', background: 'var(--surface)' }} />
                      <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--ink)', textTransform: 'capitalize' }}>{(a.label ?? a.asset_type).replace(/_/g, ' ')}</div>
                        <a href={a.image_url} download target="_blank" rel="noreferrer"
                          style={{ fontSize: '11px', color: 'var(--ink3)', textDecoration: 'none', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
