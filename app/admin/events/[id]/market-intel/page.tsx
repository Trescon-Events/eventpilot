...
// ── Design tokens ─────────────────────────────────────────────────────────────
// Dark navy/teal theme — values are CSS var()s from app/globals.css so this file
// tracks the same tokens every other migrated page uses.
...
// Literal hex (NOT var()) — every usage site appends a runtime alpha suffix
// (e.g. `tc + '18'`) for badge backgrounds, which var() cannot support since it
// carries no alpha channel of its own. Values below are each family's actual
// CSS-var hex from globals.css, so despite being literal they still track the
// theme; 'other' and 'supporter' have no matching family (see comments).
const TYPE_COLOR: Record<string, string> = {
  sponsor: '#818CF8',            // var(--indigo)
  exhibitor: '#5AA9F2',          // var(--info)
  partner: '#34D399',            // var(--success)
  media_partner: '#F5B94D',      // var(--amber)
  knowledge_partner: '#A78BFA',  // var(--purple)
  supporter: '#F472B6',          // brightened pink — no token exists for this family
  technology_partner: '#12C9BD', // var(--teal-mid)
  other: '#94A3B8',              // already 6.2:1 on --card, no brightening needed
}
...
function tierBadge(tier: string | null) {
  ...
  // Each tier reuses an existing family's -light/main pair (rule 2: text on a
  // family's own -light tint uses that family's bright value) instead of the
  // old bespoke light-mode hex, which measured well under 4.5:1 on --card.
  const colors: Record<string, { bg: string; color: string }> = {
    platinum:  { bg: 'rgba(255,255,255,0.08)', color: 'var(--ink2)' },
    diamond:   { bg: 'var(--info-light)',       color: 'var(--info)' },
    gold:      { bg: 'var(--amber-light)',      color: 'var(--amber)' },
    silver:    { bg: 'rgba(255,255,255,0.06)',  color: 'var(--ink3)' },
    bronze:    { bg: 'var(--orange-light)',     color: 'var(--orange)' },
    strategic: { bg: 'var(--indigo-light)',     color: 'var(--indigo)' },
    associate: { bg: 'var(--info-light)',       color: 'var(--info)' },
    general:   { bg: 'rgba(255,255,255,0.06)',  color: 'var(--ink3)' },
  }
  const c = colors[tier] ?? { bg: 'var(--indigo-light)', color: 'var(--indigo)' }
  return (
...
function StatusDot({ status }: { status: string }) {
  const color = status === 'done' || status === 'complete' ? 'var(--success)'
    : status === 'failed' || status === 'cancelled' ? 'var(--red)'
...
export default function MarketIntelPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: SURFACE, borderBottom: `1px solid ${BORDER}`, padding: '0 32px', position: 'sticky', top: 0, zIndex: 200 }}>
        <div style={{ maxWidth: '1360px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '56px', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '8px', background: 'var(--indigo-light)', color: INDIGO }}>AEIE · Adaptive Event Intelligence Engine</div>
          </div>
          <button onClick={downloadAllExcel} disabled={downloading}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 14px', borderRadius: '8px', background: downloading ? BORDER : 'var(--success)', color: downloading ? MUTED : 'var(--success-light)', fontSize: '12px', fontWeight: 700, border: 'none', cursor: downloading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
      ...
      {/* ── Credit Counter Bar ── */}
      <div style={{ background: 'var(--sidebar-bg)', borderBottom: `1px solid ${BORDER}`, padding: '0 32px', position: 'sticky', top: '56px', zIndex: 150 }}>
        <div style={{ maxWidth: '1360px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '20px', height: '34px' }}>
          ...
          {[
            { label: 'Gemini calls',     value: sessionCr.gemini,    color: 'var(--indigo)' },
            { label: 'Firecrawl pages',  value: sessionCr.firecrawl, color: 'var(--amber)' },
            { label: 'Jina pages',       value: sessionCr.jina,      color: 'var(--success)' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: '24px', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'sticky', top: '100px' }}>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '20px' }}>
              ...
              {!slotsAvailable && (
                <div style={{ padding: '8px 12px', borderRadius: '8px', background: 'var(--amber-light)', border: '1px solid var(--amber-border)', fontSize: '11px', color: 'var(--amber)', marginBottom: '8px' }}>
              ...
              <button onClick={checkAndRun}
                ...
                style={{ width: '100%', padding: '11px', borderRadius: '10px', background: !slotsAvailable ? BORDER : INDIGO, color: !slotsAvailable ? MUTED : 'var(--indigo-light)', fontSize: '13px', fontWeight: 700, border: 'none', cursor: !slotsAvailable ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
            ...
            {activeJobs.length > 0 && (
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '16px 18px' }}>
                <div style={{ fontSize: '11px', fontWeight: 800, color: MUTED, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--success)', display: 'inline-block', boxShadow: '0 0 0 2px rgba(52,211,153,0.2)' }} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {activeJobs.map(job => {
                    ...
                    return (
                      <div key={job.jobId} style={{ borderRadius: '12px', border: `1px solid ${isRunning ? 'var(--indigo-border)' : BORDER}`, background: isRunning ? 'var(--indigo-light)' : 'var(--border-light)', padding: '12px 14px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '8px' }}>
                          {job.urlJobs.map((u, i) => {
                            ...
                            const statusColor = u.status === 'done' ? 'var(--success)' : u.status === 'failed' ? 'var(--red)' : u.status === 'running' ? INDIGO : MUTED
                            return (
                              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                ...
                                {u.attempts > 1 && u.status === 'running' && (
                                  <span style={{ fontSize: '9px', color: 'var(--amber)', fontWeight: 700 }}>retry {u.attempts}/3</span>
                                )}
                                {u.status === 'done' && (
                                  <span style={{ fontSize: '9px', color: 'var(--success)', fontWeight: 700 }}>
                                    {u.result?.participants?.length ?? 0}co +{u.result?.speakers?.length ?? 0}sp
                                  </span>
                                )}
                                {u.status === 'failed' && (
                                  <span style={{ fontSize: '9px', color: 'var(--red)', fontWeight: 700, textTransform: 'capitalize' }}>{u.failureReason ?? 'failed'}</span>
                                )}
                              </div>
                            )
                          })}
                        ...
                        {job.partialFailures.length > 0 && (
                          <div style={{ padding: '6px 9px', borderRadius: '6px', background: 'var(--amber-light)', border: '1px solid var(--amber-border)', fontSize: '10px', color: 'var(--amber)', marginBottom: '8px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px' }}>
                            {isRunning && (
                              <button onClick={() => scanManager.pauseJob(job.jobId)}
                                style={{ flex: 1, padding: '5px 0', borderRadius: '7px', border: '1px solid var(--amber-border)', background: 'var(--amber-light)', color: 'var(--amber)', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                                Pause
                              </button>
                            )}
                            {isPaused && (
                              <button onClick={() => scanManager.resumeJob(job.jobId)}
                                style={{ flex: 1, padding: '5px 0', borderRadius: '7px', border: '1px solid var(--success)', background: 'var(--success-light)', color: 'var(--success)', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                                Resume
                              </button>
                            )}
                            <button onClick={() => scanManager.cancelJob(job.jobId)}
                              style={{ flex: 1, padding: '5px 0', borderRadius: '7px', border: '1px solid var(--red-border)', background: 'var(--red-light)', color: 'var(--red)', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                              Cancel
                            </button>
                          </div>
                        )}
                        {isDone && (
                          <div style={{ padding: '4px 8px', borderRadius: '6px', background: job.status === 'complete' ? 'var(--success-light)' : 'var(--border-light)', textAlign: 'center', fontSize: '10px', fontWeight: 700, color: job.status === 'complete' ? 'var(--success)' : MUTED }}>
              ...
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '12px', padding: '16px 18px' }}>
                ...
                {[
                  ...
                  <div key={title} style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                    <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: 'var(--indigo-light)', border: '1px solid var(--indigo-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: 800, color: INDIGO, flexShrink: 0 }}>{title[0]}</div>
                    <div>
          ...
          <div>
            ...
            {activeJobs.length === 0 && allParticipants.length === 0 && (
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '60px 40px', textAlign: 'center' }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--indigo-light)', border: '1px solid var(--indigo-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {([
                    ...
                    <button key={t} onClick={() => setTab(t)}
                      style={{ padding: '8px 16px', borderRadius: '8px', border: `1px solid ${tab === t ? INDIGO : BORDER}`, background: tab === t ? INDIGO : SURFACE, color: tab === t ? 'var(--indigo-light)' : MUTED, fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                      {label}
                    </button>
                  ))}
                  {isAnyRunning && (
                  ...
                  <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', overflow: 'hidden' }}>
                    <div style={{ padding: '12px 20px', borderBottom: `1px solid ${BORDER}`, display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      {['all', ...allTypes].map(t => (
                        <button key={t} onClick={() => setFilterType(t)}
                          style={{ padding: '3px 11px', borderRadius: '12px', border: `1px solid ${filterType === t ? INDIGO : BORDER}`, background: filterType === t ? 'var(--indigo-light)' : 'transparent', color: filterType === t ? INDIGO : MUTED, fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>
                          {t.replace(/_/g, ' ')}
                        </button>
                      ))}
                      <div style={{ flex: 1 }} />
                      {(['confidence', 'tier', 'type'] as const).map(s => (
                        <button key={s} onClick={() => setSortBy(s)}
                          style={{ padding: '3px 9px', borderRadius: '6px', border: `1px solid ${sortBy === s ? INDIGO : BORDER}`, background: sortBy === s ? 'var(--indigo-light)' : 'transparent', color: sortBy === s ? INDIGO : MUTED, fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>
                          {s}
                        </button>
                      ))}
                    </div>
                    <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: '10px' }}>
                      {filteredCompanies.map((p, i) => {
                        ...
                        return (
                          <div key={i} style={{ background: 'var(--border-light)', border: `1px solid ${BORDER}`, borderRadius: '12px', padding: '14px 16px', borderLeft: `3px solid ${tc}` }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                              ...
                              {p.company_linkedin_url && (
                                <a href={String(p.company_linkedin_url)} target="_blank" rel="noreferrer"
                                  style={{ fontSize: '10px', color: 'var(--info)', fontWeight: 700, textDecoration: 'none', background: 'var(--info-light)', padding: '2px 7px', borderRadius: '4px' }}>
                                  in
                                </a>
                              )}
                            </div>
                            ...
                            <div style={{ marginTop: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                              <div style={{ width: '40px', height: '3px', background: 'var(--surface)', borderRadius: '2px', overflow: 'hidden' }}>
                                <div style={{ height: '100%', width: `${p.confidence * 100}%`, background: p.confidence >= 0.8 ? 'var(--success)' : p.confidence >= 0.6 ? '#F5B94D' : 'var(--red)', borderRadius: '2px' }} />
                              </div>
                  ...
                  <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', overflow: 'hidden' }}>
                    ...
                    {allSpeakers.length === 0 ? (
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: '10px', padding: '16px 20px' }}>
                        {allSpeakers.map((s, i) => (
                          <div key={i} style={{ background: 'var(--border-light)', border: `1px solid ${BORDER}`, borderRadius: '12px', padding: '14px 16px', borderLeft: '3px solid var(--purple)' }}>
                            ...
                            {s.linkedin_url && (
                              <a href={s.linkedin_url} target="_blank" rel="noreferrer"
                                style={{ display: 'inline-block', marginTop: '6px', fontSize: '10px', color: 'var(--info)', fontWeight: 700, textDecoration: 'none', background: 'var(--info-light)', padding: '2px 7px', borderRadius: '4px' }}>
                                LinkedIn Profile
                              </a>
                            )}
                            <div style={{ marginTop: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                              <div style={{ width: '40px', height: '3px', background: 'var(--surface)', borderRadius: '2px', overflow: 'hidden' }}>
                                <div style={{ height: '100%', width: `${(s.confidence ?? 0) * 100}%`, background: 'var(--purple)', borderRadius: '2px' }} />
                              </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px' }}>
                      {[
                        ...
                        { label: 'Speakers',    value: allSpeakers.length,     color: 'var(--purple)' },
                        { label: 'Events done', value: activeJobs.flatMap(j => j.urlJobs).filter(u => u.status === 'done').length, color: 'var(--success)' },
                        { label: 'Unique types', value: [...new Set(allParticipants.map(p => p.participant_type))].length, color: 'var(--amber)' },
                      ].map(s => (
                    ...
                    {activeJobs.flatMap(j => j.urlJobs).filter(u => u.status === 'done').map((u, i) => u.result && (
                      <div key={i} style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '12px', padding: '16px 20px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                          <div style={{ textAlign: 'center', background: 'var(--indigo-light)', border: '1px solid var(--indigo-border)', borderRadius: '8px', padding: '8px 14px' }}>
                          ...
                          <div style={{ textAlign: 'center', background: 'var(--purple-light)', border: '1px solid var(--purple-border)', borderRadius: '8px', padding: '8px 14px' }}>
                            <div style={{ fontSize: '20px', fontWeight: 900, color: 'var(--purple)' }}>{u.result.speakers?.length ?? 0}</div>
                            <div style={{ fontSize: '9px', color: MUTED, fontWeight: 700, textTransform: 'uppercase' }}>Speakers</div>
                          </div>
                          <div style={{ textAlign: 'center', background: 'var(--success-light)', border: '1px solid var(--success)', borderRadius: '8px', padding: '8px 14px' }}>
                            <div style={{ fontSize: '20px', fontWeight: 900, color: 'var(--success)' }}>{u.result.crawl_summary.sub_pages_fetched + 1}</div>
                            <div style={{ fontSize: '9px', color: MUTED, fontWeight: 700, textTransform: 'uppercase' }}>Pages</div>
                          </div>
                        </div>
                        <div style={{ fontSize: '12px', color: DARK, lineHeight: 1.6, padding: '10px 12px', background: 'var(--indigo-light)', borderRadius: '8px', borderLeft: `3px solid ${INDIGO}` }}>
                ...
                {tab === 'hypotheses' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {allHypotheses.map((h, i) => (
                      <div key={i} style={{ background: SURFACE, border: `1px solid ${h.validated ? 'rgba(52,211,153,0.25)' : BORDER}`, borderRadius: '12px', padding: '14px 18px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                          <span style={{ fontSize: '10px', fontWeight: 800, padding: '2px 8px', borderRadius: '5px', background: h.validated ? 'var(--success-light)' : 'var(--red-light)', color: h.validated ? 'var(--success)' : 'var(--red)' }}>
                        ...
                        <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '24px' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px', gap: '16px' }}>
                            ...
                            <button onClick={() => downloadJobExcel(selectedJob)}
                              style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '8px', background: 'var(--success)', color: 'var(--success-light)', fontSize: '12px', fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>
                              <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M6.5 1v7M3.5 5.5l3 3 3-3M2 10h9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                              Download CSV
                            </button>
                          </div>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '20px' }}>
                            {[
                              ...
                              { label: 'Companies',     value: selectedJob.job.participants_found, color: 'var(--success)' },
                              { label: 'Speakers',      value: selectedJob.job.speakers_found,     color: 'var(--purple)' },
                              { label: 'Failed URLs',   value: selectedJob.job.failed_urls,        color: selectedJob.job.failed_urls > 0 ? 'var(--red)' : MUTED },
                            ].map(s => (
                              <div key={s.label} style={{ background: 'var(--border-light)', border: `1px solid ${BORDER}`, borderRadius: '10px', padding: '14px', textAlign: 'center' }}>
                          ...
                          {/* Credits breakdown */}
                          <div style={{ background: 'var(--indigo-light)', border: '1px solid var(--indigo-border)', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '20px' }}>
                              {[
                                { label: 'Gemini calls',    value: selectedJob.job.credits_gemini_calls,    color: 'var(--indigo)' },
                                { label: 'Firecrawl pages', value: selectedJob.job.credits_firecrawl_pages, color: 'var(--amber)' },
                                { label: 'Jina pages',      value: selectedJob.job.credits_jina_pages,      color: 'var(--success)' },
                              ].map(({ label, value, color }) => (
                          ...
                          {Array.isArray(selectedJob.job.partial_failures) && selectedJob.job.partial_failures.length > 0 && (
                            <div style={{ background: 'var(--amber-light)', border: '1px solid var(--amber-border)', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px' }}>
                              <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--amber)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '1px' }}>Partial Extraction Issues</div>
                              {(selectedJob.job.partial_failures as { url?: string; type: string; reason: string; potential?: number; extracted?: number }[]).map((f, i) => (
                                <div key={i} style={{ fontSize: '12px', color: 'var(--amber)', marginBottom: '4px', lineHeight: 1.5 }}>
                          ...
                          {selectedJob.scans.length > 0 && (
                            <div style={{ marginBottom: '20px' }}>
                              ...
                              {selectedJob.scans.map((scan, i) => {
                                ...
                                return (
                                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', borderRadius: '8px', background: 'var(--border-light)', border: `1px solid ${BORDER}`, marginBottom: '4px' }}>
                            ...
                            <div style={{ marginBottom: '20px' }}>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '8px' }}>
                                {selectedJob.companies.slice(0, 10).map((c, i) => {
                                  ...
                                  return (
                                    <div key={i} style={{ background: 'var(--border-light)', border: `1px solid ${BORDER}`, borderRadius: '10px', padding: '12px 14px', borderLeft: `3px solid ${tc}` }}>
                            ...
                            <div>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '8px' }}>
                                {selectedJob.speakers.slice(0, 10).map((s, i) => {
                                  ...
                                  return (
                                    <div key={i} style={{ background: 'var(--border-light)', border: `1px solid ${BORDER}`, borderRadius: '10px', padding: '12px 14px', borderLeft: '3px solid var(--purple)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
                              <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                  ...
                                  {job.failed_urls > 0 && (
                                    <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '5px', background: 'var(--red-light)', color: 'var(--red)', border: '1px solid var(--red-border)' }}>
                              ...
                              <div style={{ display: 'flex', gap: '12px', flexShrink: 0 }}>
                                ...
                                <div style={{ textAlign: 'center' }}>
                                  <div style={{ fontSize: '18px', fontWeight: 900, color: 'var(--purple)', lineHeight: 1 }}>{job.speakers_found}</div>
                                  <div style={{ fontSize: '9px', color: MUTED, fontWeight: 700, textTransform: 'uppercase' }}>Speakers</div>
                                </div>
                              </div>
                            </div>
                            {Array.isArray(job.partial_failures) && job.partial_failures.length > 0 && (
                              <div style={{ marginTop: '8px', fontSize: '11px', color: 'var(--amber)', background: 'var(--amber-light)', border: '1px solid var(--amber-border)', borderRadius: '6px', padding: '5px 10px' }}>
          ...
          <div style={{ background: SURFACE, borderRadius: '18px', padding: '28px 32px', maxWidth: '540px', width: '100%', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '22px' }}>
              {dupConfirm.dupes.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '10px 14px', borderRadius: '10px', background: 'var(--amber-light)', border: '1px solid var(--amber-border)' }}>
            ...
            {dupConfirm.fresh.length > 0 && (
              <div style={{ padding: '10px 14px', borderRadius: '10px', background: 'var(--success-light)', border: '1px solid var(--success)', marginBottom: '20px' }}>
                <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--success)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>New URLs (always scan)</div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              {dupConfirm.fresh.length > 0 && (
                <button onClick={() => executeRun(dupConfirm.fresh, false)}
                  style={{ flex: 1, padding: '11px', borderRadius: '10px', border: '1px solid var(--success)', background: 'var(--success-light)', color: 'var(--success)', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                  Scan {dupConfirm.fresh.length} new only
                </button>
              )}
              <button onClick={() => executeRun([...dupConfirm.dupes.map(d => d.url), ...dupConfirm.fresh], true)}
                style={{ flex: 1, padding: '11px', borderRadius: '10px', border: 'none', background: INDIGO, color: 'var(--indigo-light)', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
