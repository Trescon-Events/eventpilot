...
export default function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#F6FFFE', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1E2124', fontSize: '19px' }}>
      Loading campaign…
    </div>
  )
  ...
  if (!campaign) return (
    <div style={{ minHeight: '100vh', background: '#F6FFFE', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
      <div style={{ color: '#1E2124', fontSize: '21px' }}>Campaign not found.</div>
      <Link href="/content" style={{ color: '#00A5A3', fontSize: '19px' }}>Back to Content Hub</Link>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F6FFFE', color: '#1E2124', fontFamily: 'inherit' }}>
      <style>{`
        .cp-btn { padding: 10px 18px; border-radius: 8px; font-size: 21px; font-weight: 700; cursor: pointer; border: none; font-family: inherit; transition: all 0.15s; }
        ...
        .cp-tab { padding: 8px 18px; border-radius: 8px; font-size: 21px; font-weight: 700; cursor: pointer; border: 1px solid transparent; background: transparent; color: #464D53; font-family: inherit; transition: all 0.15s; }
      ...
      <div style={{ borderBottom: '1px solid #C8DFE0', padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', boxShadow: '0 1px 3px rgba(0,165,163,0.08)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: '19px', fontWeight: 700, color: '#1E2124', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '2px' }}>
              {campaign.events?.name ?? 'Content Campaign'}
            </div>
            <h1 style={{ fontSize: '38px', fontWeight: 900, color: '#1E2124', margin: 0, letterSpacing: '-0.02em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {campaign.name}
            </h1>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '16px', flexShrink: 0 }}>
          {[
            ...
            <div key={s.label} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '44px', fontWeight: 900, color: s.color, lineHeight: 1 }}>{s.val}</div>
              <div style={{ fontSize: '19px', color: '#1E2124', letterSpacing: '0.8px', textTransform: 'uppercase' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '32px 40px' }}>
        ...
        {posts.length === 0 && planReady && (
          <div style={{ background: 'rgba(0,165,163,0.05)', border: '1px solid rgba(0,165,163,0.2)', borderRadius: '16px', padding: '28px', marginBottom: '28px' }}>
            <div style={{ fontSize: '19px', fontWeight: 800, color: '#00A5A3', marginBottom: '8px' }}>Ready to build your post schedule?</div>
            ...
            {planWeeks.map(wk => (
              <div key={wk.week} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(0,165,163,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '19px', fontWeight: 800, color: '#00A5A3', flexShrink: 0 }}>{wk.week}</div>
                <div style={{ fontSize: '21px', fontWeight: 700, color: '#464D53', minWidth: 160 }}>{wk.theme}</div>
                <div style={{ display: 'flex', gap: '4px' }}>
                  {wk.roles.map((r, i) => (
                    <span key={i} style={{ fontSize: '19px', padding: '2px 7px', borderRadius: '4px', background: '#FFFFFF', color: '#464D53' }}>{r}</span>
                  ))}
                </div>
              </div>
            ))}
        ...
        {posts.length === 0 && !planReady && !campaign.start_date && (
          <div style={{ textAlign: 'center', padding: '60px 40px', background: '#FFFFFF', border: '1px dashed #C8DFE0', borderRadius: '16px' }}>
            <div style={{ fontSize: '21px', fontWeight: 700, color: '#464D53', marginBottom: '8px' }}>Set a start date to build the post schedule</div>
            <div style={{ fontSize: '21px', color: '#1E2124' }}>Edit the campaign to add a start date, then return here to scaffold posts.</div>
          </div>
        )}
              ...
              return (
                <div key={wkNum} style={{ background: '#FFFFFF', border: '1px solid #C8DFE0', borderRadius: '14px', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid #C8DFE0', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(0,165,163,0.15)', border: '1.5px solid rgba(0,165,163,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '21px', fontWeight: 800, color: '#00A5A3', flexShrink: 0 }}>{wkNum}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '21px', fontWeight: 800, color: '#1E2124', marginBottom: '2px' }}>{wkTheme}</div>
                    ...
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '19px' }}
                    >
                      {anyGen ? (
                        ...
                        <div key={post.id} className="post-card" onClick={() => setActivePost(post)}>
                          ...
                          <div style={{ padding: '10px 12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                              <span style={{ fontSize: '19px', fontWeight: 700, color: pc }}>{post.platform}</span>
                              <span style={{ fontSize: '19px', fontWeight: 700, padding: '2px 7px', borderRadius: '10px', background: sc.bg, color: sc.color }}>{sc.label}</span>
                            </div>
                            <div style={{ fontSize: '19px', color: '#1E2124', marginBottom: '4px' }}>{post.narrative_role}</div>
                            {post.text ? (
                              <div style={{ fontSize: '19px', color: '#464D53', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                                {post.text}
                              </div>
                            ) : (
                              <div style={{ fontSize: '19px', color: '#1E2124', fontStyle: 'italic' }}>No copy yet</div>
                            )}
                            <div style={{ marginTop: '8px', display: 'flex', gap: '6px' }} onClick={e => e.stopPropagation()}>
                              {post.status === 'planned' && (
                                <button className="cp-btn cp-btn-ghost" style={{ fontSize: '19px', padding: '5px 10px' }}
                                  disabled={isg} onClick={() => generatePost(post)}>
                                  Generate
                                </button>
                              )}
                              {post.status === 'generated' && (
                                <button className="cp-btn cp-btn-lime" style={{ fontSize: '19px', padding: '5px 10px' }}
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid #C8DFE0', borderRadius: '14px', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 100px 90px 100px 120px', padding: '10px 16px', borderBottom: '1px solid #C8DFE0', background: 'rgba(0,165,163,0.06)' }}>
              {['Wk', 'Copy', 'Platform', 'Status', 'Role', 'Date'].map(h => (
                <div key={h} style={{ fontSize: '19px', fontWeight: 700, color: '#1E2124', letterSpacing: '0.8px', textTransform: 'uppercase' }}>{h}</div>
              ))}
            </div>
            {posts.map(post => {
              const pc = PLATFORM_COLOR[post.platform] ?? '#888'
              ...
              return (
                <div key={post.id} onClick={() => setActivePost(post)}
                  ...
                  <div style={{ fontSize: '21px', color: '#1E2124', fontWeight: 700 }}>W{post.week_number}</div>
                  <div style={{ fontSize: '21px', color: '#464D53', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: '12px' }}>
                    {post.text || <span style={{ color: '#1E2124', fontStyle: 'italic' }}>No copy yet</span>}
                  </div>
                  <div style={{ fontSize: '19px', fontWeight: 700, color: pc }}>{post.platform}</div>
                  <span style={{ fontSize: '19px', fontWeight: 700, padding: '2px 8px', borderRadius: '10px', background: sc.bg, color: sc.color, alignSelf: 'center', textAlign: 'center' }}>{sc.label}</span>
                  <div style={{ fontSize: '19px', color: '#464D53' }}>{post.narrative_role}</div>
                  <div style={{ fontSize: '19px', color: '#464D53' }}>{fmtDate(post.scheduled_date)}</div>
                </div>
              )
            })}
          </div>
          ...
          <div>
            {pendingApproval.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px', background: '#FFFFFF', border: '1px dashed #C8DFE0', borderRadius: '16px' }}>
                <div style={{ fontSize: '21px', fontWeight: 700, color: '#3D6B00', marginBottom: '6px' }}>All clear</div>
                <div style={{ fontSize: '21px', color: '#1E2124' }}>No posts awaiting approval.</div>
              </div>
            ) : (
                      ...
                      <div style={{ padding: '18px 20px', flex: 1, display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                            <span style={{ fontSize: '19px', fontWeight: 700, color: pc }}>{post.platform}</span>
                            <span style={{ fontSize: '19px', color: '#1E2124' }}>Week {post.week_number} · {post.narrative_role}</span>
                            <span style={{ fontSize: '19px', color: '#1E2124', marginLeft: 'auto' }}>{fmtDate(post.scheduled_date)}</span>
                          </div>
                          <div style={{ fontSize: '19px', color: '#464D53', lineHeight: 1.65, marginBottom: '12px', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                            {post.text}
                          </div>
                          {post.revision_note && (
                            <div style={{ background: 'rgba(255,107,107,0.08)', borderLeft: '3px solid #FF6B6B', padding: '8px 12px', borderRadius: '0 6px 6px 0', fontSize: '19px', color: '#FF6B6B', marginBottom: '10px' }}>
                              Revision note: {post.revision_note}
                            </div>
                          )}
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flexShrink: 0 }}>
                          <button className="cp-btn cp-btn-lime" style={{ fontSize: '19px' }} disabled={busyId === post.id} onClick={() => approvePost(post.id)}>
                            {busyId === post.id ? '…' : 'Approve'}
                          </button>
                          <button className="cp-btn cp-btn-red" style={{ fontSize: '19px' }} onClick={() => { setRejectId(post.id); setRejectNote('') }}>
                            Revise
                          </button>
                        </div>
                      </div>
            ...
            <div className="modal-box" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ padding: '20px 24px', borderBottom: '1px solid #C8DFE0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '21px', fontWeight: 700, color: pc }}>{post.platform}</span>
                  <span style={{ fontSize: '19px', fontWeight: 700, padding: '2px 8px', borderRadius: '10px', background: sc.bg, color: sc.color }}>{sc.label}</span>
                  <span style={{ fontSize: '19px', color: '#1E2124' }}>Wk {post.week_number} · {post.narrative_role}</span>
                </div>
              ...
              <div style={{ padding: '24px' }}>
                ...
                {/* Copy — editable */}
                <label style={{ fontSize: '19px', fontWeight: 700, color: '#1E2124', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Copy</label>
                <textarea
                  value={post.text}
                  onChange={e => updatePostText(post.id, e.target.value)}
                  rows={8}
                  placeholder="No copy yet — click Generate below."
                  style={{ width: '100%', padding: '12px', background: '#FFFFFF', border: '1px solid #C8DFE0', borderRadius: '10px', color: '#1E2124', fontSize: '19px', fontFamily: 'inherit', resize: 'vertical', lineHeight: 1.65, outline: 'none', boxSizing: 'border-box' }}
                />

                {post.revision_note && (
                  <div style={{ background: 'rgba(255,107,107,0.08)', borderLeft: '3px solid #FF6B6B', padding: '10px 14px', borderRadius: '0 8px 8px 0', fontSize: '21px', color: '#FF6B6B', marginTop: '10px' }}>
      ...
      {rejectId && (
        <div className="modal-bg" onClick={() => setRejectId(null)}>
          <div className="modal-box" style={{ maxWidth: '460px' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '24px' }}>
              <div style={{ fontSize: '21px', fontWeight: 800, color: '#1E2124', marginBottom: '6px' }}>Request Revision</div>
              <div style={{ fontSize: '21px', color: '#1E2124', marginBottom: '18px' }}>Describe what needs to change. The post goes back to Generated status.</div>
              <textarea value={rejectNote} onChange={e => setRejectNote(e.target.value)}
                placeholder="e.g. Too generic. Mention the AI in Banking theme specifically. Add a CTA to register."
                rows={4}
                style={{ width: '100%', padding: '11px 14px', background: '#FFFFFF', border: '1px solid #C8DFE0', borderRadius: '10px', color: '#1E2124', fontSize: '19px', fontFamily: 'inherit', resize: 'none', lineHeight: 1.65, outline: 'none', boxSizing: 'border-box' }} />
              <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                ...
                <button onClick={rejectPost} disabled={!rejectNote.trim() || busyId === rejectId}
                  style={{ flex: 1, padding: '10px', borderRadius: '9px', background: '#DC2626', border: 'none', color: 'white', fontWeight: 700, fontSize: '19px', cursor: 'pointer', opacity: rejectNote.trim() ? 1 : 0.4 }}>
                  Send for Revision
                </button>
              </div>
            </div>
