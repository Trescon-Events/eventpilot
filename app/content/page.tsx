...
const STATUS_CFG: Record<string, { color: string; bg: string }> = {
  planning:  { color: 'rgba(255,255,255,0.45)', bg: 'rgba(255,255,255,0.07)' },
...
export default function ContentHubPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080A0B', color: 'white', padding: '40px 48px', fontFamily: 'inherit' }}>
      <style>{`
        .cc-btn { padding: 9px 18px; border-radius: 9px; font-size: 12px; font-weight: 700; cursor: pointer; border: none; font-family: inherit; transition: all 0.15s; }
        .cc-btn-teal { background: #00A5A3; color: white; }
        .cc-btn-teal:hover { background: #00C4C2; }
        .cc-btn-ghost { background: transparent; border: 1px solid rgba(255,255,255,0.12) !important; color: rgba(255,255,255,0.5); }
        .cc-btn-ghost:hover { border-color: rgba(255,255,255,0.25) !important; color: rgba(255,255,255,0.8); }
        .cc-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; overflow: hidden; transition: border-color 0.2s; }
        .cc-card:hover { border-color: rgba(0,165,163,0.3); }
        .filter-btn { padding: 6px 14px; border-radius: 7px; font-size: 11px; font-weight: 700; cursor: pointer; border: 1px solid rgba(255,255,255,0.1); background: transparent; color: rgba(255,255,255,0.45); font-family: inherit; transition: all 0.15s; }
        ...
        .plat-chip { display: inline-flex; align-items: center; gap: 5px; padding: 5px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; cursor: pointer; border: 1px solid transparent; transition: all 0.15s; user-select: none; }
      `}</style>

      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
        <div>
          <Link href="/admin" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '11px', color: 'rgba(255,255,255,0.35)', textDecoration: 'none', marginBottom: '12px' }}>
          ...
          <div style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '2.5px', textTransform: 'uppercase', color: '#00A5A3', marginBottom: '6px' }}>Content Engine</div>
          <h1 style={{ fontSize: '26px', fontWeight: 900, color: 'white', margin: 0, letterSpacing: '-0.02em' }}>Content Campaigns</h1>
      ...
      {showCreate && (
        <div style={{ background: 'rgba(0,165,163,0.04)', border: '1px solid rgba(0,165,163,0.2)', borderRadius: '16px', padding: '28px', marginBottom: '32px' }}>
          <div style={{ fontSize: '11px', fontWeight: 800, color: '#00A5A3', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '20px' }}>New Campaign</div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            {/* Name */}
            <div style={{ gridColumn: '1/-1' }}>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Campaign Name</label>
            ...
            {/* Event */}
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Event (optional)</label>
              <select className="inp" value={form.event_id} onChange={e => setForm(f => ({ ...f, event_id: e.target.value }))} style={{ background: '#0F1215' }}>
            ...
            {/* Phase */}
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Phase</label>
              <select className="inp" value={form.phase} onChange={e => setForm(f => ({ ...f, phase: e.target.value }))} style={{ background: '#0F1215' }}>
            ...
            {/* Start date */}
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Start Date</label>
              <input type="date" className="inp" value={form.start_date} onChange={e => setForm(f => ({ ...f, start_date: e.target.value }))} style={{ background: '#0F1215' }} />
            </div>
            {/* Duration */}
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Duration (weeks)</label>
              <select className="inp" value={form.duration_weeks} onChange={e => setForm(f => ({ ...f, duration_weeks: +e.target.value }))} style={{ background: '#0F1215' }}>
                {[2,3,4,6,8,12].map(w => <option key={w} value={w}>{w} weeks</option>)}
              </select>
            </div>
            {/* Objective */}
            <div style={{ gridColumn: '1/-1' }}>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Objective</label>
          ...
          <div style={{ marginBottom: '14px' }}>
            <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Platforms</label>
              ...
              {Object.entries(PLATFORM_COLOR).map(([p, c]) => {
                ...
                return (
                  <button key={p} className="plat-chip" onClick={() => togglePlatform(p)}
                    style={{ background: sel ? `${c}22` : 'rgba(255,255,255,0.04)', borderColor: sel ? `${c}55` : 'rgba(255,255,255,0.1)', color: sel ? c : 'rgba(255,255,255,0.45)' }}>
                    <span style={{ width: 7, height: 7, borderRadius: '50%', background: sel ? c : 'rgba(255,255,255,0.2)' }} />
                    {p}
                  </button>
                )
              })}
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.8px', textTransform: 'uppercase', display: 'block', marginBottom: '5px' }}>Brand Notes <span style={{ color: 'rgba(255,255,255,0.25)', fontWeight: 400, textTransform: 'none' }}>(optional — AI reads your event brief automatically)</span></label>
          ...
          {saveMsg && <div style={{ fontSize: '12px', color: '#FF6B6B', marginBottom: '12px' }}>{saveMsg}</div>}
      ...
      {(events.length > 0 || campaigns.length > 0) && (
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.3)', letterSpacing: '1px', textTransform: 'uppercase' }}>Event</span>
          ...
          <span style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.3)', letterSpacing: '1px', textTransform: 'uppercase' }}>Phase</span>
      ...
      {/* Campaign grid */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '80px', color: 'rgba(255,255,255,0.3)', fontSize: '13px' }}>Loading…</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 40px', background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '20px' }}>
          ...
          <div style={{ fontSize: '15px', fontWeight: 700, color: 'rgba(255,255,255,0.6)', marginBottom: '8px' }}>No campaigns yet</div>
          <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)', marginBottom: '20px' }}>Create your first campaign to start generating content for your events.</div>
              ...
              <div key={c.id} className="cc-card">
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', marginBottom: '10px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 9px', borderRadius: '20px', background: ph.bg, color: ph.color }}>{ph.label}</span>
                    <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 9px', borderRadius: '20px', background: st.bg, color: st.color, textTransform: 'capitalize' }}>{c.status}</span>
                  </div>

                  {/* Name */}
                  <div style={{ fontSize: '15px', fontWeight: 800, color: 'white', marginBottom: '4px', lineHeight: 1.3 }}>{c.name}</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>{cnt} posts</span>
                  </div>

                  <Link href={`/content/campaigns/${c.id}`}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '9px', background: '#00A5A3', color: 'white', fontSize: '12px', fontWeight: 700, textDecoration: 'none' }}>
