...
| Field | Value |
|---|---|
| Who | Madhu + Claude Code (Sonnet 5) — 24–25 Sep 2026. Built the new **Operations Hub** module: shared Vendor Directory, speaker-licence procurement (ready list + batches), and an isolated external **Vendor Portal** for the licence vendor (own login, captcha + emailed OTP, time-limited ZIP download, licence upload / "approved" confirmation). |
| Date | 2026-09-25 |
| Latest push | **Not pushed yet at the time of writing** — commits are prepared locally on `main`; push needs Madhu's explicit go. If you are reading this and `origin/main` doesn't contain "Operations Hub", the push hasn't happened. |
| DB migrations applied | `supabase/ops_vendors_migration.sql`, `supabase/ops_license_batches_migration.sql`, `supabase/ops_vendor_portal_migration.sql` — all three already applied to production by Madhu via `!` psql (the classifier blocks these). New private bucket `ops-license-files` created by the last one. |
...
| Left alone / known follow-up | The pre-existing security issues found along the way (browser-visible admin code, forged-cookie acceptance, hardcoded secret, anonymous department change, task-profiles leak) are fixed — see "Pre-existing security issues" below. The cookie-decoder sweep is done too. `NEXT_PUBLIC_ADMIN_CODE` was deleted from Railway on 26 Sep 2026 (no redeploy triggered). |
...
2. Security follow-ups: none outstanding from the 25-26 Sep audit (CLAUDE.md cleaned, decoder sweep done). Optional: require a session on the /profile onboarding flow (closes the last anonymous-department-change residual) and stop `verify-staff` POST leaking staff ids by email.
