...
| Handed off to | Madhu (resumes next session). Ops team (Hussain) sends the two draft batches once the EasyGo agreement is signed. |
...
1. **Ops (Hussain) — EasyGo:** complete + sign the EasyGo agreement — the Word draft "Trescon-EasyGo-Data-Processing-Agreement-DRAFT.docx" was generated 26 Sep and is held by Madhu (moved out of the repo, NOT committed — if lost it must be regenerated) (yellow placeholders: EasyGo's legal entity — "EasyGo Hub" vs "EasyGoTicket, LLC" — licence no., storage location/account type/MFA, named staff, contacts; delete the last "drafting notes" page; put on letterhead). **Do not SEND any batch to EasyGo until signed.** Then send Draft **Batch 1** (20 speakers) and **Batch 2** (17) from Operations → Licences: the vendor (EasyGo, contact Khaleel Shaik, assigned to DFFW) needs a Portal login first (Vendors → Portal logins → invite); they accept the terms, acknowledge the delete-by date, download, and complete each batch via the deliberate flow (tick deletion → upload licence/approve → Submit). The two batches are records of what was ALREADY emailed and processed; Adnan Chilwan (only sent a Word file with numbers — nothing stored) and the recent submissions (Albert Saporta, Kubra Nefise Kalkan, Sheikh Muhammad Noman) are in NO batch and go into the next real batch.
