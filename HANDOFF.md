...
| Field | Value |
|---|---|
| Who | Madhu + Claude Code (Sonnet 5) — 24–26 Sep 2026. Built the **Operations Hub** (vendor directory, speaker-licence batches, isolated Vendor Portal); moved Passport/ID + licence storage to **Azure UAE North** and deleted all Supabase copies; hardened auth (admin code, forged cookies, hardcoded secret); fixed the speaker upload form; added **speaker consent** + **vendor terms / delete-by acknowledgement / forced deletion confirmation**; umbrella (DFFW) workspace nav, breadcrumbs and My Events; reconciled the emailed DFS26 Batch 1+2 documents and created the two record batches; drafted the EasyGo data-processing agreement. |
| Date | 2026-09-26 |
| Latest push | Everything from this session is pushed to `origin/main` at sign-off (Railway auto-deploys). Verify with `git fetch origin && git log origin/main -3` and `railway deployment list`. |
| DB migrations applied | ops_vendors, ops_license_batches, ops_vendor_portal, ops_umbrella_scope_and_event_days, **sensitive_consent_migration.sql** (consent columns on speaker_communication_requests; ops_vendor_terms_acceptances; delete_by / download_ack / deletion_confirmed on ops_license_batches) — all applied to production by Madhu via `!` psql. |
...
| Handed off to | Madhu (resumes next session). Ops team (Hussain) sends the two draft batches when ready — nothing in the system waits on any agreement. |
| Deployed | Yes — all code pushed and live on Railway; storage backend = Azure (`DOCUMENT_STORAGE_BACKEND=azure`). |
| Left alone / known follow-up | See "What's next (26 Sep sign-off)" below. Test vendor/batch/test documents were deleted; the two test speakers (John Travis) remain in both events by design. |

## 26 Sep 2026 (later) — FIFF onboarding, Salutation field, admin shortcuts

**FIFF = "Future Islamic Finance Forum 2026"** (`06f62069-37d3-4699-9871-16709fa1540e`, DFFW child). Same approach as DFS, done with Madhu step by step.
- **Integration (Madhu, live-verified by Claude):** HubSpot form "FIFF26 - Speaker onboarding form" (`fe7841ac-…`) connected + mapped; "Set Up Automatic Sync" created workflow `1890597160` (identical to DFS's, plus re-enrollment triggers). End-to-end proven with test speaker **John Travis (TEST FIFF)** — kept on purpose, do not delete. HubSpot's spam filter silently holds test-looking submissions (release them in HubSpot → Submissions). The FIFF form has NO passport/ID upload field (documents come via the speaker-submission link / import). KonfHub: Speaker/Moderator tags = the shared DFFW tags; **Speaker Category ID = `18951`** (set by Madhu). The two hidden HubSpot fields (Ticket name/description) and the consent checkbox are unmapped.
- **Import (19 speakers, from the producers' sheet + SharePoint folders):** `scripts/import-fiff-speakers-2026-09-26.ts` (gitignored, dry-run by default; forces `DOCUMENT_STORAGE_BACKEND=azure` because `.env.local` does not set it). Created: 19 `event_speakers` (Public Name = "First Last", status Confirmed, producer = Ayshle D'Souza, country = HubSpot full names UK/UAE expanded, title/company verbatim — dual roles are deliberate), 19 raw photos, 19 Full Bios (pptx handled via CloudConvert in the script), 12 passports (Azure only, named `<Public Name>-passport.<ext>`), 14 additional contacts, CRM contacts (Chilwan linked to his DFS contact). Read back and verified. No National ID, no PhotoRoom, no KonfHub/HubSpot writes. Salutations live in `custom_fields.salutation` (same as DFS).
- **Gaps for producers:** passports missing for Nurym Ayazbayev, Nargiz Shiraliyeva, Mohieddine Kronfol, Adnan Chilwan, Mughees Shaukat, Eisa Al Amri, Talal Yassine; National ID pending for the 5 UAE residents (Fasih, Kronfol, Chilwan, Al Amri, Hilal); no email at all for Nurym, Nargiz, Ahcene Lahsasna, Mohammed Saifullah Khan, Kronfol, Mughees; six low-res photos (Eisa, Khurram, Nurym, Ahcene, Omar Shaikh, Adnan Fasih); no cleaned/Website Photo or pronoun style yet (both needed before a KonfHub push). Not imported: 5 other people in the HubSpot form history (Ayshle Dsouza, Ismail Tekin, Tayyab Ahmed, Faris Lodin, Bilal Parvaiz) and "Khalid Jamal Al Kayed" (only in Edited Images).
- **KonfHub reconciliation (read-only, 26 Sep):** category 18951 already holds the same 19 speakers, uploaded directly by producers; none is linked in EventPilot (`konfhub_speaker_id` null) → **pushing from EventPilot now would create duplicates**; remove-and-repush (or link IDs) is Madhu's pending decision. KonfHub names carry "Dr."/"Prof." (Madhu is fixing Public Names in EventPilot first); country there is "UAE"/"UK" (KonfHub `location` is free text, EventPilot values will be pushed). Content differences for Adnan Fasih, Mughees Shaukat, Ashar Nazim, Mohieddine Kronfol were resolved by Madhu updating EventPilot ("go by EventPilot"). No images/tags/order set from EventPilot yet.
- **Code:** (1) speaker Details page (`app/admin/events/[id]/stakeholders/[stakeholderId]/page.tsx`) — new **Salutation** dropdown before First Name on every speaker (synthesized like email/phone because HubSpot-connected schemas never declare it; reads/writes `custom_fields.salutation`, options = HubSpot form's list). (2) Admin Dashboard "Testing shortcuts" (`app/admin/page.tsx`) — added DFFW umbrella workspace, Stakeholder Hub for FTF/DFWS/DFS/FSF/FIFF, and BSS (Bengaluru Skill Summit 2026). `tsc` clean (one pre-existing error in a DFS script); neither change was browser-verified before push.
- **Next:** Madhu fixes Public Names (drop titles); then decide the KonfHub path (link vs remove+repush) and photo/pronoun work; producers chase the gap list. Delete the scratchpad copy of the source folder was done (passports were never committed).

## 27 Sep 2026 — FIFF KonfHub repush, Short Bio integrity check, Reference Docs tab

- **FIFF KonfHub repush (26 Sep, done):** all 19 listings in category 18951 were deleted and recreated from EventPilot (script `scripts/konfhub-fiff-repush-2026-09-26.ts`, gitignored), in the recorded KonfHub order 1-19, all tagged "Speaker". New KonfHub IDs 29676-29694, all linked in `event_speakers.konfhub_speaker_id`; re-fetch verified name/designation/organisation/location/about/image/tags/order = EventPilot. Old data + IDs are in `.scratch/konfhub-fiff-snapshot-2026-09-26.json` (untracked); the new IDs in `.scratch/konfhub-fiff-repush-log-2026-09-26.txt`. Public Names now carry "Dr."/"Prof." (Madhu's edit) and match KonfHub. "Push to KonfHub" now updates instead of creating duplicates. Nargiz Shiraliyeva is live with a photo that still shows a bottle/table (see the 26 Sep note).
- **Short Bio integrity check (new, report-only):** `app/lib/content/bio-integrity.ts` + `POST .../speakers/[id]/check-bio`, also returned by `generate-short-bio` as `integrity`. Runs on Details-page load and after Generate; findings show in an amber box under the Short Bio field (only while the field still holds the checked text). Deterministic checks (cut-off text, "at ." gaps, doubled words/punctuation, garbled characters, placeholders, speaker's name missing / different name in the opening, pronouns vs salutation) + one Gemini proofread that only lists problems (results cached 1 h per bio, in memory). **Deliberately NOT the event's content/validation rules** — a bio is the speaker's own words (Madhu, 26 Sep); the generator still reads the compiled messaging reference for voice/tone. No length check (the field's own hint states 500). Tested on the 19 real bios (18 clean; Mughees Shaukat's bio never mentions his listed CarbonCX) and on synthetic broken cases. Not clicked through in a browser.
- **Reference Docs tab (`/admin/events/[id]/details?tab=messaging`):** label renamed from "Messaging Doc" to "Reference Docs", description now names the messaging doc, style guide and production pack, and the amber dot fires for a draft of ANY role (was messaging-only, so a production-pack draft was invisible). `?tab=messaging` unchanged. FIFF-only uploads of the Topline Messaging + Production Pack were about to happen; approving a Messaging Doc overwrites the event's Common Details from the doc (logged), a Production Pack does not.

## 26 Sep 2026 (evening) — FIFF follow-ups: photos, KonfHub check, PowerPoint bios

- **Background removal for the 19 FIFF speakers:** the import script had skipped PhotoRoom (my wrong assumption that producers do it). Ran it afterwards (`scripts/photoroom-fiff-2026-09-26.ts` + `headbox-fiff-2026-09-26.ts`, gitignored): `photo_processed_url` set for all 19 (real transparency verified), `photo_head_box` for 18. **Nargiz Shiraliyeva**: candid seated shot, PhotoRoom left a bottle/table/chair, no front-facing face → head box empty (live detection at generation time is the fallback); needs Refine-with-AI in the wizard or a better photo. Six low-res photos (Eisa, Khurram, Nurym, Ahcene, Omar Shaikh, Adnan Fasih) worth a look.
- **PowerPoint bios now convert automatically:** `toStoredBioPdf` (all four Full Bio entry points) accepts .ppt/.pptx via CloudConvert (`OfficeInputFormat` in `cloudconvert-client.ts`); source stays `docx_converted`; upload filters/help text updated (producer Details page, speaker-submission link, default form schema). Tested locally with a generated sample deck (pptx by extension and by MIME, .txt rejected); legacy .ppt not tested.
- **Real bug found on the way:** a deck-converted PDF's text came out with NO spaces from `extractPdfText` (pdf-parse glues per-word items), which would have fed garbage to the short-bio and headline generators. `app/lib/pdf-text.ts` now retries with position-based word spacing when the result has <8% spaces; the other 19 stored FIFF bios re-extract byte-identical. Mohammed Saifullah Khan's stored `bio_full_text` was repaired (2926 -> 3346 chars). DFS had no affected bios.
- **Open idea (not built):** Clean Photo's AI Fill uses gpt-image-2, which regenerates the whole image (documented in `photo-cleaning-pipeline.ts`) — likely cause of the tie/skin-tone drift Madhu flagged; proposed a local bake-off (composite original pixels back over any fill model) before changing anything.

## 25 Sep 2026 — Operations Hub (speaker licence procurement + Vendor Portal)

### The ask
UAE law requires every speaker to hold a licence; the vendor issues ONE licence document listing many speakers, in batches of ~15–20 as speakers come on board. They need each speaker's passport (+ National ID for UAE residents). Build a secure way for ops and the external vendor to work from the already-collected documents. Only thing tracked: whether a speaker has been applied for. Ops is a wider team (budget, venue, AV/print/visa vendors, floorplan…), so this is the first tool in an **event-scoped Operations workspace**, not a one-off.

### What was built (all under `/admin/events/[id]/operations`, link added to the workspace left-nav under "Planning, Commercial & Brand")
- **Vendor Directory** (`/operations/vendors`): shared across events; vendor + 1–2 contacts; assign to an event. Per assigned vendor: **Portal logins** panel — ops creates a login (email = username), sends invite / reset links, disables/enables. Tables `ops_vendors`, `ops_vendor_contacts`, `ops_event_vendors` (`ops_` prefix because `vendor_contacts` already belongs to the Task Manager agency accounts — unrelated).
- **Speaker Licences** (`/operations/licenses`): a speaker appears automatically once EVERY required document is marked reviewed by a producer (passport; + National ID if UAE resident). Residency must be known (unknown is NOT ready — differs from the Status Board, which treats unknown as "may need National ID"). One row per speaker, doc icons open a preview modal (audited). Select rows → create a **batch** (vendor, access days default 7, hard cap 30). A batch freezes each speaker's details + exact document rows; flags "documents changed" / "speaker cancelled" if reality drifts. One active batch per speaker (DB-enforced). "Applied" is derived from batch membership — no separate flag.
- **Batch lifecycle:** Draft → Sent → Downloaded (auto-detected) → Completed, plus Expired/Cancelled. Ops actions: Send (emails the vendor's logins a NOTICE — no files), Extend, Revoke, Reopen, Mark approved, Upload licence copy (for one OR several batches), Cancel draft. The real ops process (per-batch licence vs one bulk licence vs just "approved") is still unconfirmed, so all are supported.
- **Vendor Portal** (`/vendor-portal/*`, own bare layout): login = email + password + Cloudflare Turnstile + emailed 6-digit code; forgot/set-password pages; batch list; batch detail with download (ZIP: `manifest.csv` + a folder per speaker, streamed through an authenticated route — the vendor never gets a storage URL), licence upload (PDF/JPG/PNG ≤ 20 MB, type checked by magic bytes) or "Mark as approved". Either response completes the batch and **ends vendor access immediately** and emails ops.
- **Permissions** (`ops` module in `app/lib/registry/access-permissions.ts`): `ops.view`, `ops.vendors.manage`, `ops.licenses.view`, `ops.licenses.manage`, `ops.vendor_accounts.manage`. Previewing a passport ALSO needs the existing `sae.sensitive_documents.view`. An access role **"Operations"** (`ops.*` + `sae.sensitive_documents.view`, deliberately not `.manage`) was created by Madhu and assigned to Hussain Shabbir Mithaiwala on Dubai FinTech Summit 2026.

### Vendor isolation design (Madhu: vendors must see NOTHING else, by any means)
Separate `ops_vendor_users` table (never `staff_members`); passwords set only by the vendor via a single-use emailed link (invite 48h / ops reset 24h / self-service reset 1h) — ops never sees or sets one, no temp passwords; opaque DB-backed sessions (SHA-256 stored) so reset/disable/unassign revokes instantly; cookie `vp_session` is `Path=/vendor-portal`, HttpOnly, SameSite=Strict, so the browser never sends it to `/admin` or `/api/*` (vendor API lives under `/vendor-portal/api` for that reason); every batch query is scoped by the SESSION's vendor id (another vendor's / draft / cancelled / made-up ids all 404); same-origin check on state-changing POSTs; per-account lockout (5 fails/15 min) + per-IP limit (20/15 min) + OTP 5 attempts; identical generic login errors and timing (forgot-password works in `after()`); audit row written BEFORE any download (no audit row → no download); staff sidebar/widgets suppressed on `/vendor-portal` (`AuthedShellGate`, new `StaffOnlyWidgets`); security headers via `next.config.ts`. Every vendor-facing error carries "check with the Trescon Ops team" plus, once signed in, the names/emails of staff holding `ops.view` on that event. `/vendor-portal` is in middleware `PUBLIC_PREFIXES` — it self-authenticates; prod `/api/login` password auth is disabled, so the portal has its own login route. All new tables have RLS on with no policies.
**Dev-mode email:** outside production the vendor/ops emails are NOT sent — they're appended to `.scratch/dev-mail.jsonl` (set `OPS_MAIL_SEND_IN_DEV=1` to send for real). This is why an invite sent from a local dev server never arrives.
**Storage:** v1 = private Supabase buckets + RBAC + audit (app-level encryption deliberately deferred). The existing purge cron removes documents at event end + 30 days; a batch whose documents were purged fails to download with a clear message.

### Verified (local dev server against the production DB; all test data deleted afterwards)
Full flow with a throwaway vendor and John Travis (TEST) on Dubai FinTech Summit 2026: invite → set password → login + OTP → download (valid ZIP, manifest) → licence upload (bad type/size rejected) → completion ends access → reopen/extend/revoke/approve → expiry. Isolation battery: another vendor's batch, guessed/malformed ids, a stolen vendor cookie replayed on 10 staff routes, a staff cookie and garbage cookies on the vendor API, logout/disable/reset revocation, lockout, OTP attempt limit, per-IP limit, enumeration + timing, staff-page regression check (7 pages 200 with normal shell). `tsc` + `eslint` clean on everything touched (one pre-existing `tsc` error in `scripts/compare-konfhub-vs-eventpilot-2026-09-23.ts`).
**NOT verified:** a non-admin ops user, the support-contact names in vendor errors and the ops notification emails (the recipient list was empty locally), real Turnstile and real email delivery, and how the pages look in a browser.

### Follow-up, same day (25 Sep): watermarked view-only viewer + document location policy
Why: Madhu wants passports/Emirates IDs held in the UAE (Supabase is Singapore — it has no Middle East region; Azure UAE North is the planned store, blocked on getting a subscription via the Microsoft partner) and India-based ops/production staff will also open them. Storing them in the UAE doesn't control WHERE they're viewed, so this adds viewing controls that work regardless of storage.
- **Staff never get the original file or a raw storage link any more.** New `GET /api/events/sensitive-documents/view?doc_id&page` returns ONE page as a JPEG with the viewer's name + time + a view id burned into the pixels (`app/lib/access/document-watermark.ts`; sharp + pdfjs + @napi-rs/canvas; font = LiberationSans from pdfjs-dist, and if it can't load the route REFUSES rather than serve an unwatermarked image). The view id is stored in `ops_access_audit` (`document_view`), so a leaked image traces to a row. Used by both the producer tab (`SensitiveDocumentsTab`) and the ops Speaker Licences page via `app/components/SensitiveDocViewer.tsx`. Removed: the raw `signed_url` from the producer list/upload API and the old `operations/licenses/document` route. **Vendors are unaffected** — their ZIP still contains the originals.
- **Rules:** `sae.sensitive_documents.view` needed; `.manage` holders (producers) can open any current document, view-only holders (ops) only reviewed ones; everything else 404s uniformly; 200 page-views/hour/person cap; audit written before the image is produced.
- **Location policy** (`app/lib/access/sensitive-access-policy.ts`), all env-driven and **OFF by default**: `SENSITIVE_DOC_ACCESS_MODE=off|monitor|enforce`, `SENSITIVE_DOC_ALLOWED_COUNTRIES=AE,IN`, `SENSITIVE_DOC_ALLOWED_IPS=<office IPs / CIDRs>`. Allowed if the country OR the IP matches. Trusts ONLY Cloudflare's `CF-IPCountry` / `CF-Connecting-IP` (X-Forwarded-For/X-Real-IP are forgeable and never grant access). `GET /api/events/sensitive-documents/access-check` shows any signed-in user what the server sees for them.
- **Rollout plan:** after deploy set MODE=monitor + countries on Railway, open `access-check` from Dubai AND Bangalore to confirm the CF headers really survive the Worker → Railway hop (unverified — only provable in production), watch `document_view` audit rows for `would_block`, then switch to `enforce`. If the headers don't arrive, the proxy Worker must forward them (a Cloudflare Worker change — needs Madhu's explicit go).
- **Known limit:** the Railway address is publicly reachable, so someone talking to it directly could forge CF-* headers. The policy stops stolen-password use through the normal site; closing it fully needs the Worker to add a secret header the app verifies.
- **Verified locally:** real test passport (image) through the route with the watermark checked visually; 2-page PDF (page 2 OK, past-the-end null); corrupt/unsupported files fail safely; missing font refuses; full policy matrix through the real handler (off/monitor/enforce, country, IP, CIDR boundary, Tor, no headers, forged XFF/X-Real-IP, bad mode value, no rules). **NOT verified:** the viewer UI in a browser, a view-only (ops) user (reviewed-only rule is in the code, untested), the 200/hour cap, and Linux font rendering (only checked on macOS; the bundled font makes it deterministic in theory).
- **Standard document names (25 Sep, Madhu's spec):** every Passport / National ID is named on arrival `<Public Name>-passport.<ext>` / `<Public Name>-EID.<ext>` (Public Name only — no legal-name handling, and NO renaming if the name changes later: the name is set once). One helper, `app/lib/events/sensitive-doc-name.ts`, used by all three intake routes (producer upload, public speaker-submission form, from-submission processing) and by the vendor ZIP (files inside each speaker folder now use the same names). Storage KEYS stay ID-based on purpose (no names in bucket paths). Blank Public Name falls back to the record name so a file is never unnamed — 39 legacy speakers have a blank Public Name (35 in World AI Show Malaysia 2026, all created before the 25 Aug auto-default) but none has a document. Existing 51 DFS documents: `scripts/rename-sensitive-docs-2026-09-25.ts` (local-only — `scripts/` is gitignored, like the other one-off scripts, so it lives only on Madhu's machine; dry run by default; `--apply` writes; saves a before->after CSV to `.scratch/`, since the original filename is otherwise lost — e.g. an ID originally named after a speaker's legal name). **Status: code committed locally; backfill NOT yet applied** (production data write, needs Madhu's go / a `!` run).
- **Azure UAE plan (SUPERSEDED — see the Azure storage section above; built 26 Sep):** Madhu has asked his Microsoft partner to create the subscription (no subscription/billing account is visible to md@tresconglobal.com; tenant 932ae9a0-… only has Entra). Cost at UAE North hot ZRS ≈ $0.025/GB/month (43 MB today ≈ $0.001/mo). Open legal questions: may India-based staff view these documents, and must the contracting entity be UAE-based.

### Follow-up (25 Sep, late): licence processing at UMBRELLA level (DFFW) + real "Event days"
Madhu: licence processing for ALL DFFW events (Dubai Future Finance Week, umbrella `d4c7c680-…`, 5 child events) happens at DFFW level, not per event; and the "30 days after the event" document deletion must use the REAL event days — `events.event_date/end_date` are the whole planning CYCLE (e.g. DFS 1 Oct 2025 -> 30 Nov 2026 for an event held 2-3 Nov), and the real days exist only as free text (`events.public_dates_display`, deliberately text, untouched).
- **Migration `supabase/ops_umbrella_scope_and_event_days_migration.sql` — APPLIED to production by Madhu 2026-09-25** (dry-run rolled back first, then applied): `actual_start_date`/`actual_end_date` on `events` AND `event_umbrellas`; `umbrella_id` (event_id XOR umbrella_id, CHECK-enforced, event_id now nullable) on `ops_event_vendors` (also got a surrogate `id` PK; the old `(event_id,vendor_id,purpose)` uniqueness kept as a constraint), `ops_license_batches`, `ops_license_files`; `umbrella_id` on `ops_access_audit`. The TEST VENDOR moved from the DFS event to the DFFW umbrella. Aborts if any batch/file exists on an umbrella child.
- **Scope model** (`app/lib/ops/scope.ts`): an event UNDER an umbrella is transparently lifted to the umbrella by every ops API (`event_id` or `umbrella_id` accepted); an event with no umbrella keeps its own per-event process. **Access (Madhu's option A): holding the permission on ANY child event = access to the umbrella workspace** (`getAccessibleEventIds` intersected with the children); admins always. Document previews stay per event (viewer checks `sae.sensitive_documents.view` on the document's own event). Roles still can't be scoped to an umbrella (known RBAC limitation — `event_access_assignments.event_id` FKs to `events`); the alternative (umbrella-scoped roles) was offered and deferred.
- **What Madhu must do by hand:** assign the Operations role (+ `sae.sensitive_documents.view`) on EACH DFFW child event — only Dubai FinTech Summit has it today (Hussain); a new child event needs it added.
- **UI:** shared views in `app/admin/operations-shared/` (scope context, hub, vendors, licences, Event days card + notice) used by BOTH `/admin/events/[id]/operations/*` and the new `/admin/umbrellas/[id]/operations/*` (layout gate = admin or `ops.view` on any child; middleware allows non-admins on that path). The umbrella licence list spans all child events with an Event column and flags the same person appearing in more than one event (by CRM contact). Vendor portal shows the umbrella's name; "who to contact" in vendor errors and ops notification emails now cover ops people across ALL child events. The umbrella page (admin-only) got an Operations card + Event days card; the event details page got an Event days card.
- **Event days** (`GET/PUT /api/events/event-days`, `app/lib/events/sensitive-retention.ts`): real dates on an event and on the umbrella; deletion = last real day + `sensitive_document_retention_days` (30), using the LATER of the event's and the umbrella's last day; until set, falls back to the cycle `end_date` (old behaviour) and the licences page warns. Saving re-stamps `retention_expires_at` of documents already on file; if that would put existing documents' deletion date in the PAST (permanent deletion at the next daily run) the save is refused with a 409 unless `confirm_past`. Permissions: event = `sae.forms.manage` or new `ops.event_days.manage` (covered by `ops.*`); umbrella = `ops.event_days.manage` on any child or global `sae.messaging.umbrella_manage`. All three document intake routes now stamp retention through this helper.
- **Verified locally against production data (all test data removed, dates reverted):** scope lift (child->umbrella, plain event stays event), Hussain (role on ONE child) gets full umbrella access and a no-role user none, support contacts list Hussain, full flow (vendor login created from the DFS event page -> batch at umbrella level owned by `umbrella_id` in the DB -> send -> vendor sees "Dubai Future Finance Week" -> ZIP with the new file names -> licence upload completes and emails ops -> vendor error lists contacts -> reopen/unassign guard 409 -> revoke releases the speaker), event-days validation + past-date guard + save/revert (umbrella 2-4 Nov -> docs 4 Dec; DFS 2-3 Nov -> later date wins; cleared -> back to 30 Dec), CHECK constraint (no owner / two owners rejected), vendor/staff cookie isolation on the new routes. **NOT verified:** the new pages as rendered in a browser (all return 200), a real non-admin browser session, and `event_umbrellas.actual_*` for the other 4 child events (no data yet).
- **Not decided / for Madhu:** the real event days for DFFW and each child still need entering (Dubai FinTech's public text says 2-3 Nov 2026); until then documents delete 30 days after the cycle end date.

### Azure storage in UAE North (Dubai) for Passport/ID + licence files — built 26 Sep 2026, LIVE (backend=azure since 26 Sep 2026)
Why: Supabase has no Middle East region (this project is Singapore; Railway also runs in Singapore) and Madhu wants these documents STORED in the UAE. Madhu's Microsoft partner (India billing, CSP "Azure Plan") created the subscription; `md@tresconglobal.com` has Owner. (The partner mail's "subject to confirmation of user ID" was already applied; role assignments send no email.) Cost views are DISABLED on the enrolment ("contact your enrollment administrator") — ask the partner for a monthly cost report + budget alert; storage cost is ~$0.025-0.03/GB/month (51 docs ≈ 43 MB).
- **Resources** (subscription `683e2182-7954-426f-bd06-4845d73a27d0`, tenant `932ae9a0-…`): resource group `rg-eventpilot-uae`; storage account **`tresconeventpilotdocs`** (hyphens aren't allowed in Azure storage names) — UAE North, Standard **ZRS** (3 zones inside Dubai), Hot, anonymous access OFF, HTTPS only, TLS 1.2, **account keys DISABLED** (Entra identity only), encrypted at rest (Microsoft-managed keys), blob soft-delete / container soft-delete / versioning / change feed all OFF (so a "permanent delete" really is permanent); private containers `sensitive-documents` and `license-files`. `Microsoft.Storage` provider had to be registered on the new subscription first.
- **App identity:** Entra app registration "EventPilot Documents (Railway)" — client id `2c9c8041-c85c-484d-80fc-7acb4cb4a938`, service principal object id `ce45c656-…`. It holds exactly TWO role assignments: Storage Blob Data Contributor on each container, nothing else. **Its client secret (name `eventpilot-railway`) EXPIRES 25 Sep 2027 — rotate before then** (`az ad app credential reset --id <client id> --years 1`, pipe the value straight into `railway variable set AZURE_CLIENT_SECRET --stdin` and `.env.local`; never print it). The secret was generated and piped without ever being displayed.
- **Config:** Railway + `.env.local` now hold `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, `AZURE_STORAGE_ACCOUNT`, `AZURE_DOCS_CONTAINER`, `AZURE_LICENSE_CONTAINER` (Railway set with `--skip-deploys`). Azure CLI (`az`) was installed on Madhu's Mac via Homebrew; `az login --tenant 932ae9a0-…` is needed once per session for infra changes.
- **Code:** `app/lib/storage/azure-blob.ts` (adapter), `app/lib/events/sensitive-storage.ts` and `app/lib/ops/license-storage.ts` now pick a backend from **`DOCUMENT_STORAGE_BACKEND`** (`supabase` = default, `azure`). With `azure`: uploads go to Azure ONLY and FAIL if Azure isn't configured (never a silent fallback to Supabase); reads try Azure first then fall back to the legacy Supabase copy; deletes remove BOTH copies (so the purge cron can't leave one behind). Object keys are identical on both sides, so the database needed no change. The old signed-URL helpers are gone: ops now open a licence copy through an authenticated, audited route that STREAMS the file (`GET /api/events/operations/licenses/files/[fileId]`) — nobody is handed a storage link. New packages `@azure/storage-blob`, `@azure/identity`.
- **Data:** all **51 documents copied to Azure and verified byte-for-byte (SHA-256 read-back), twice**; nothing deleted from Supabase. Local-only (gitignored) script `scripts/migrate-documents-to-azure-2026-09-26.ts` — default mode copies + verifies and deletes nothing; `--verify-only`; `--delete-supabase` (needs `CONFIRM_DELETE=DELETE-SUPABASE-COPIES`, removes a Supabase copy only if the Azure copy matches, permanent). Re-run the copy mode just before/after cutover to catch anything uploaded in between (idempotent).
- **Verified:** live round-trip against the real account for both containers (upload/download identical, refuses overwrite, delete, delete-missing safe); azure backend uploads land in Azure not Supabase; legacy fallback opens a Supabase-only document; delete removes both copies; a misconfigured azure backend FAILS instead of writing to Supabase; default backend unchanged; the real viewer route served a passport with Supabase storage simulated DOWN (proving it read Azure); routes compile in the Next dev bundler. Supabase's download endpoint kept serving a cached copy of a just-deleted test file (its object was gone from the bucket) — a Supabase quirk, another small reason to move. **NOT verified:** a production build (the dev server was running, so no `next build`), the whole flow on the live site, and Linux/Railway behaviour of the Azure SDK.
- **CUTOVER (needs Madhu's go at each step):** (1) push this; (2) set `DOCUMENT_STORAGE_BACKEND=azure` on Railway (this triggers a deploy); (3) Madhu opens a passport + downloads a vendor ZIP on the live site to confirm; (4) re-run the copy script for anything uploaded since; (5) ONLY THEN, with Madhu's explicit confirmation, run `--delete-supabase` and remove the now-empty Supabase buckets' contents. Until step 5 the fallback keeps everything readable.
- **Still open:** legal — may India-based staff view these documents, and must the contracting entity be UAE-based; server-side handling still passes bytes through Railway (Singapore) in memory (viewer render + vendor ZIP) — direct browser-to-Azure uploads would avoid even that and could be added later; the earlier location policy (country/IP) is built but OFF by Madhu's choice.

- **STATUS 26 Sep 2026 (evening):** `DOCUMENT_STORAGE_BACKEND=azure` is set on Railway; live end-to-end test PASSED on the FIFF test speaker (form upload -> both files in Azure only, correct names, retention date, watermarked viewer + audit row, vendor ZIP download, licence upload -> batch completed, licence file in Azure `license-files`, vendor access ended). **Client secret rotated 26 Sep 2026** (new secret `eventpilot-railway-2026-09-26`, expires 25 Sep 2027; old one deleted). Azure cost views now work (partner enabled) — month-to-date cost query returned no charges, but there is NO budget/alert on the subscription (ask partner what was set, or create one). **Supabase copies of the ORIGINAL 51 documents (+ the two John Travis TEST docs) are still there** — deleting them needs Madhu's explicit go: re-run the copy script first, then `--delete-supabase`. India-based staff viewing is APPROVED by Madhu (same setup, legal question dropped); location policy stays OFF.
- **Also shipped 26 Sep 2026 (same session):** speaker submission form shows/validates/sends ONLY the items the producer requested (fixed a bug where a pre-filled long Short Bio blocked documents-only submissions; review-data no longer returns unrequested bio/country; no National ID stored for a confirmed non-UAE-resident); submission notification email links to the Documents tab when documents were submitted (falls back to Communications without `sae.sensitive_documents.view`); umbrella workspace shell (`app/admin/umbrellas/[id]/layout.tsx` + `UmbrellaShell.tsx`: left panel, Overview admin-only, Operations for ops.view on any child; the overview page moved to `UmbrellaOverview.tsx`, `page.tsx` redirects non-admins to Operations; middleware allows the umbrella root URL for signed-in users), umbrella breadcrumb registry entries (real name via the layout), umbrellas listed under My Events (`app/lib/ops/my-umbrellas.ts`, `GET /api/events?staff_id=`), vendor batch page redesigned (Option 1 upload / Option 2 approve-without-file).
- **Ops notification recipients:** `notifyOps` emails everyone holding `ops.licenses.view` (else `ops.view`) on ANY child event of the umbrella — today only Hussain (Operations role on Dubai FinTech Summit). Platform admins are NOT included unless they hold the role.
- **Pending / open:** (1) delete Supabase copies after Madhu's go (above); (2) Azure budget alert; (3) set DFFW week's own Event days (suggest 2-5 Nov) and assign the Operations role (+ sae.sensitive_documents.view) on the other four DFFW child events; (4) FIFF onboarding (Madhu's separate session); (5) optional: direct browser-to-Azure uploads so bytes never pass the Singapore server; require session on /profile onboarding; `verify-staff` POST leaks staff ids by email; in-app notification bell (email only today); an ended vendor's login session stays valid until it expires (batch access is what ends; nothing else is reachable).

### Sensitive-document privacy work — built 26 Sep 2026 (migration APPLIED; code committed locally, NOT yet pushed at time of writing)
- **Policy:** tresconglobal.com/privacy-policy now has section 4 "Passport and National ID documents (speakers)" (anchor `#sensitive-documents`), edited in WordPress by Madhu's team (not managed from this repo). It says stored in Dubai, deleted automatically 30 days after the last event day, vendor deletes as soon as the licence is procured. Those statements must stay TRUE: Supabase leftover copies still need deleting (needs Madhu's explicit go) and the purge cron must actually run.
- **Speaker consent:** wording + version `SENSITIVE_CONSENT_VERSION='v1'` in `app/lib/stakeholders/sensitive-consent.ts` (bump the version if ANY wording changes and update the policy). Form shows the notice + required checkbox above the passport/ID inputs; `submit` refuses passport/ID without `sensitive_consent=v1` (400) and records `sensitive_consent_at/version/ip` on `speaker_communication_requests`; Documents tab shows "consent given on …". Request + reminder emails carry one notice sentence. Documents collected before launch: no record, by decision. Staff never accept documents outside the form.
- **Vendor Portal:** `terms.ts` (`VENDOR_TERMS_VERSION`) accepted once per user (table `ops_vendor_terms_acceptances`; guard returns 403 `terms_required` until then); before download the vendor acknowledges `delete_by` (earliest `retention_expires_at` in the batch; `app/lib/ops/delete-by.ts`), download refused (409) until acknowledged, README.txt in the ZIP; completing a batch = tick "deleted all copies" → licence file or approve-without-file → Submit (`license/` and `approve/` require `confirm_deleted`; one Ops email "Licence uploaded and deletion confirmed" / "Batch n marked approved and deletion confirmed"); `confirm-deletion/` route for downloaded batches that expired/were revoked/were marked approved by Ops (cancelled batches are visible to the vendor ONLY in that case); Ops Licences view shows acknowledgement + "Vendor deletion not confirmed"; Ops `reopen` clears ack + confirmation. Migration: `supabase/sensitive_consent_migration.sql` (applied 26 Sep).
- **Retention cron:** `.github/workflows/purge-sensitive-documents.yml` (daily 03:00 UTC, Railway URL + CRON_SECRET) replaces the hand-registered cron-job.org job — disable the old cron-job.org entry (running both is harmless: purge is idempotent). Verify with a manual `workflow_dispatch` after the push.
- **Tested against real data (test vendor/speaker, cleaned up):** terms gate; ack/download gating; ZIP contents incl. README; license/approve refused without confirmation and recorded with it; Ops-marked approval → vendor confirm-deletion; reopen reset; consent 400 without / recorded with; Ops view fields; audit rows. NOT tested in a real browser (UI layout of the new vendor flow and consent block) — check after deploy. Local dev servers can't read Azure-only documents unless `DOCUMENT_STORAGE_BACKEND=azure` is set (download stream errors — expected locally).
- **Pending:** push + deploy verification; `workflow_dispatch` purge test + disable cron-job.org entry; Supabase leftover copies deletion (Madhu's go); real-batch backfill for the two emailed DFS batches (numbers will be 2 and 3; test Batch 1 is TEST VENDOR); remove TEST VENDOR / test speakers' documents before go-live; (off-system, non-blocking) any processing agreement with EasyGo (entity unclear: EasyGo Hub vs EasyGoTicket LLC); lawful-basis wording + governing law (legal).

### Supabase document copies DELETED — 26 Sep 2026 (late evening)
After Madhu's explicit go and a final gate (all 49 live documents readable from Azure alone with matching size; each Supabase copy SHA-256-identical to its Azure twin; Madhu opened Albert Saporta's passport in the viewer from Azure), the script `scripts/migrate-documents-to-azure-2026-09-26.ts --delete-supabase` removed all 49 Supabase copies, plus one unreferenced 633-byte junk file (a single red pixel, Madhu confirmed). **Supabase storage is now EMPTY (documents bucket 0 files, licence bucket 0 files); Azure UAE North is the only copy — no rollback.** Test vendor, Batch 1 and its licence file were deleted earlier the same day; the two test speakers' documents were deleted by Madhu in the app. The Supabase Postgres database (Singapore) still holds speaker names/companies/countries and document METADATA only (no images). The Supabase fallback read in `sensitive-storage.ts` / `license-storage.ts` is now harmless dead code (optional cleanup). Also created: the EasyGo agreement Word draft (held by Madhu, not in the repo; placeholders for EasyGo's details; not legally reviewed; Ops to complete + sign before any real batch is sent to EasyGo).

### DFS26 emailed Batch 1 + 2 reconciled against EventPilot/Azure — 26 Sep 2026
Madhu's SharePoint folder ("DFS26 - Speaker Passports", 38 speaker folders, 37 passports + 8 National IDs + 1 Word file) was downloaded, compared read-only (names, sizes, SHA-256; no image opened) and the working copy deleted. Result: every folder file was byte-identical to what Azure already held, except: Fadi Ghandour's passport (in the vendor batch, missing from EventPilot — uploaded 26 Sep via the staff upload handler, identical to the folder file); Dr. Adnan Chilwan sent only a Word file with passport/ID numbers — NOT acceptable, nothing stored, he shows as missing both so producers can re-request; George Theocharides (both docs already in EventPilot). Fabian's folder spelling was wrong, EventPilot's "Fabian Khoshbakht" is right. **Changes made (all via the app's own handlers):** all 46 live documents of the Batch 1+2 speakers marked reviewed (formality — already processed by the vendor; reviewer = Madhu); Fadi + George marked UAE resident; the 18 passport-only Batch 1 speakers marked NOT UAE resident (matches Batch 2's convention). Albert Saporta, Kubra Nefise Kalkan, Sheikh Muhammad Noman (recent submissions) untouched/unreviewed for the next real batch. **Not done (decisions):** the two portal batches (drafts labelled "Emailed batch 1/2") are NOT created — Ops creates the EasyGo Hub vendor (contact seen: Khaleel Shaik) first; then 37 of the 38 folder speakers are ready (all but Adnan); they'd be numbered 1 and 2. There is no system dependency on any agreement — Ops sends when ready. Confirmation status is blank for six speakers (Sundararaman Ramamurthy, Ahmed Abdeltawab, Bashir Kazour, Jayesh Patel, Remo Abbondandolo, Sebastian Widmann) — the team updates manually.

### Pre-existing security issues found during the Ops Hub work — ALL FIXED 26 Sep 2026 (commit "fix(security): auth hardening")
Found while checking what a vendor could reach; none were caused by the Ops Hub. Details of what was wrong, so nobody reintroduces it:
1. **Shared "admin code" (`NEXT_PUBLIC_ADMIN_CODE ?? '<hardcoded default>'`)** gated 16 routes (`admin/set-password`, `admin/set-job-level`, `admin/tool-permissions`, `courses`, `generate-course`, `staff-import`, `office-config`, `hr/staff`, `hr/attendance/sync`, `staff-portal-sync`, `seed-demo`, `seed-courses` v1-v3, `seed-platform-docs`). NEXT_PUBLIC_ vars are inlined into browser JS and the default was hardcoded, so it proved nothing; several routes were also on the middleware's public list. `hr/attendance/sync` only rejected a WRONG code and let a request with NO code through. **Now:** every one requires a signature-verified session (`app/lib/access/require-admin.ts`: `requireAdmin` / `requireAdminOrHr`), the code and its default are gone from server AND the 4 admin pages, and those routes were removed from the middleware bypass list.
2. **Forged admin cookie accepted.** ~9 routes on the middleware's PUBLIC list (`build-requests` x3, `admin/pilots` x5, `admin/setup-pilots`) plus `tool-permissions`/`hr/staff` decoded `tcs_session` by hand (`JSON.parse(Buffer.from(raw,'base64'))`) WITHOUT verifying the signature — Node's base64 decoder stops at `=` padding, so a forged `{"adm":true}` payload with a junk signature passed. Confirmed live: it returned real data from `/api/build-requests` and reached the DB on `tool-permissions`. **Now:** they use the verifying `getSession()`. **Sweep DONE 26 Sep 2026:** all ~44 remaining server-side hand-rolled decoders now use the verifying `getSession()`/`decodeSession()` (only `session.ts` itself, the middleware's own Web-Crypto check and the Microsoft OAuth callback's ID-token decode remain, on purpose; ~10 client pages read the cookie for display only). The sweep also fixed three FAIL-OPEN authorization bugs: `dashboard`, `course-completion` and `team-courses` skipped their ownership check whenever the hand decode threw — which it did for VALID cookies whose payload had no base64 `=` padding — so those users could read/act on other people's records (now fail-closed, 401/403); and the public `events/[slug]` page granted the unpublished-site admin preview to any forged cookie (now verified). Also cleaned: CLAUDE.md credentials removed, dead literals redacted from this file, curl examples in the pilots admin page/email/seed text, and the unused `NEXT_PUBLIC_ADMIN_CODE` entry in `wrangler.jsonc`.
3. **Hardcoded secret `<redacted-dead-value>`** was accepted as an admin key (`x-setup-key`) by `build-requests`, `admin/pilots`, `admin/setup-pilots` and `cron/generate-leaderboard`, and was embedded in browser JS by `app/hr/attendance/page.tsx` (which sent it as `?secret=` to `/api/cron/attendance-live` — which requires the REAL `CRON_SECRET`, so that auto-sync had been silently failing with 401). Railway's real `CRON_SECRET` differs from the literal, and GitHub's equals Railway's (proved by the other cron workflows succeeding against fallback-free routes), so removing the literal broke nothing. **Now:** env `CRON_SECRET` only (`hasCronSecret()`); `attendance-live` accepts the real secret OR an admin/HR session, and the page sends no secret.
...
**For Madhu:** (a) `NEXT_PUBLIC_ADMIN_CODE` is now unused everywhere — DELETED from Railway 26 Sep 2026 with his go (nothing to rotate). (b) CLAUDE.md still lists the admin code (`<redacted-dead-value>`) and the cron secret literal — both are now dead values and should be removed from that file (his account-map file, not edited here). (c) The real `CRON_SECRET` was never exposed to browsers, so it does not need rotating.

### What's next (26 Sep sign-off)
1. **Ops (Hussain) — EasyGo:** send Draft **Batch 1** (20 speakers) and **Batch 2** (17) from Operations → Licences whenever ready. **There is NO system dependency on any agreement** (Madhu, 26 Sep: no blockers, and no mention of a physical/signed agreement anywhere in the product — legal review/signing may take long). The Vendor Portal's own online controls are what the system relies on: the vendor accepts the data-handling terms, acknowledges the delete-by date, and must tick "I deleted all copies" when submitting the licence. The vendor (EasyGo, contact Khaleel Shaik, assigned to DFFW) needs a Portal login first (Vendors → Portal logins → invite). Sending records of batches ALREADY emailed and processed is optional bookkeeping — Ops decides. Adnan Chilwan (only sent a Word file with numbers — nothing stored) and the recent submissions (Albert Saporta, Kubra Nefise Kalkan, Sheikh Muhammad Noman) are in NO batch and go into the next real batch. A processing agreement with EasyGo is an OFF-SYSTEM track (Madhu holds a Word draft; not in the repo) and gates nothing.
2. **Producers:** request Adnan Chilwan's passport/ID properly via Request Missing Items; blank confirmation status for six speakers (Sundararaman Ramamurthy, Ahmed Abdeltawab, Bashir Kazour, Jayesh Patel, Remo Abbondandolo, Sebastian Widmann) — team updates manually.
3. **DFFW setup:** set DFFW week's own Event days (suggest 2–5 Nov) and assign the Operations role (+ `sae.sensitive_documents.view`) on the other four DFFW child events (today only Hussain, on Dubai FinTech Summit); real-batch recipients of Ops emails = holders of `ops.licenses.view` on any child event.
4. **Housekeeping:** disable the old cron-job.org purge job (the daily purge now runs from `.github/workflows/purge-sensitive-documents.yml`, tested OK); Azure budget/alert (none exists; cost views work); Azure client secret expires 25 Sep 2027 (rotate ~Sep 2027); check the new consent block / terms page / completion flow visually in a browser (only tested via handlers + curl); legal (off-system, non-blocking): lawful-basis wording, governing law, any vendor agreement.
5. **FIFF onboarding** — Madhu's separate session (test speaker "John Travis (TEST FIFF)" exists there).
6. Optional: remove the now-dead Supabase fallback read in `sensitive-storage.ts` / `license-storage.ts`; require a session on /profile onboarding; `verify-staff` POST leaks staff ids by email; in-app notification bell; direct browser→Azure uploads; location policy (built, OFF).
...
**Kind note on speaker approvals (`f1cf87d`):** with hundreds of speakers and limited channel slots, approval doesn't guarantee publication, so a polite "A kind note" (Madhu's chosen "Option 2" wording) was added in two places: (1) the public review page `app/public/announcement-review/[id]/[announcementId]/page.tsx`, shown only when `approver_role === 'External Reviewer'` (client approvers don't see it) — hardcoded constant `SPEAKER_KIND_NOTE`; (2) the managed `speaker_announcement_approval_request` row in `email_templates` (editable in Admin > Email Templates) via `supabase/speaker_approval_email_kind_note_migration.sql` (appends small grey 12px text, idempotent). **The migration is pushed but was NOT confirmed applied** — Madhu must run it via `psql` (the classifier blocks it for Claude); until then emails go out without the note though the page shows it. Client-approval email deliberately has no note (question left open). If wording is edited in Email Templates, update the page constant too.
...
**What's next:** confirm the kind-note email migration was applied; decide whether the client-approval email needs its own note; the Country of Residence dropdown stores HubSpot's full names ("United States"), so a new form submission will show that long form on KonfHub rather than "USA"; normalise at push time only if wanted. Other events' speaker rows were not audited for the same default.
...
A very long, multi-part session covering (1) removing all 51 DFS speakers from KonfHub and pushing them back fresh from EventPilot, so every future edit flows through EventPilot instead of producers editing KonfHub directly; (2) two Full Bio fixes (Nicolas Moreau, Noel Quinn) whose original PDFs had no extractable text, blocking AI short-bio generation; (3) a permission-editor bug (a stale retired permission key blocked saving the Producer role); (4) a full rework of the "Request Missing Items"/"Send Reminder"/"Send Acknowledgment" flow in Communications so every one opens an editable To/Cc/Subject/live-preview popup instead of firing immediately, plus the same fix extended to every other email send in the SAE module; (5) a typed-DELETE confirmation + producer "Mark as Reviewed" tracking on Passport/National ID documents; (6) a UAE-resident data-mapping bug (the intake form's answer never reached the real `is_uae_resident` column); (7) a Status Board rework (new Email/Assistant Email columns, several columns gaining a real Pending/In Progress/Done lifecycle instead of just yes/no); (8) a two-section redesign of the public speaker-submission form mirroring the real HubSpot onboarding form's field constraints and help text.

### What was found and built

**KonfHub full remove + repush (data only, scripted)** — deleted and freshly recreated all 51 DFS speakers' KonfHub Speakers-listing entries from EventPilot's own current data, in their recorded order (`speaker_order` set at creation, no separate reorder pass needed). Armin Peter was removed but deliberately not re-pushed (his record is still missing pronoun/cleaned-photo/website-photo). **Real bug caught and fixed mid-operation**: the first pass re-pushed 13 speakers who were actually `confirmation_status='Cancelled'` in EventPilot, because the script sourced its "who to push" list from who was already live on KonfHub rather than filtering by EventPilot's own status — Madhu restored those 13 and regenerated their website photos, then they were correctly re-pushed (update, not create, since they already had fresh IDs from the first pass). Every one of the 50 pushed speakers now has a `konfhub_speaker_id` EventPilot itself created — "Push to KonfHub" now works normally on all of them going forward.

**Nicolas Moreau / Noel Quinn Full Bio fix (data only)** — both speakers' Full Bio PDFs had zero extractable text (confirmed by running the same extraction the app's own route uses), so "Generate from Full Bio" was silently failing every time (the route correctly declines and returns a 422, but the failure is an easy-to-miss inline message, not a blocking error — this read to Madhu as "the new one is also big" when actually nothing had regenerated). Fixed by replacing each PDF with a Word-converted version of the same real bio text (via the same `toStoredBioPdf`/CloudConvert pipeline every other upload uses) — both now extract cleanly and generate a compliant short bio.

**Producer role permission bug (data only)** — the "Producer" role had a stale `sae.secure_documents.manage` key left over from a permission model retired 2026-09-10 (replaced by `sae.sensitive_documents.*`). There's no checkbox for the retired key anymore, so it silently rode along on every save and got rejected by validation, blocking the *entire* save — not just that one item. Deleted the one stale row; audited every other role for the same class of issue (none found).

**SAE-wide editable To/Cc (code)** — per Madhu: every email send in the SAE module should default to an editable To/Cc, not fire on a single click. `CommunicationsTab.tsx`'s "Request Missing Items"/"Send Reminder"/"Send Acknowledgment" and the announcement panel's external-notify "Send Reminder" all now split into a compose step (renders the email, returns it, doesn't send) and a separate send step (persists + actually emails, accepting the producer's edited To/Cc/Subject/body) — each opens a popup with editable To, editable Cc (auto-filled from the speaker's Additional Contacts), editable Subject, and a live sandboxed-iframe preview before Send. New routes: `.../communications/[requestId]/{remind,acknowledge}/compose` and `.../notify-external/remind/compose`. The shared To/Cc/Subject/Preview UI was promoted to `app/components/EmailComposeFields.tsx` (4th call site made it a real shared component, not premature abstraction). **Audited and explicitly left alone**: Send to Speaker, Notify External (first send), Send for External/Client Approval, and Invites already had editable To/Cc. **Notify Internal Team** was flagged but not touched — it's a deliberate broadcast-to-the-whole-staff-roster email with no per-recipient concept, built that way per Madhu's own earlier spec; needs an explicit decision from him, not a guess, before any rework.

**Sensitive Documents — typed-DELETE + review tracking (code + migration)** — the Passport/National ID Delete button used to fire immediately with zero confirmation; now opens a modal requiring the exact word `DELETE` typed before the button enables, with copy stating the deletion is permanent (`DeleteSensitiveDocumentModal.tsx`, same "type DELETE to confirm" convention as `DeleteCreativeModal.tsx`). Deletion history used to say "deleted by staff" for every entry regardless of who actually did it — now resolves the real staff name (or "an admin" for a super-admin session, or "automatic retention purge" unchanged). New: `reviewed_by`/`reviewed_at` columns + a "Mark as Reviewed"/"Unmark Reviewed" button and badge — a document being uploaded was never distinct from a producer actually having checked it, which the Status Board rework below needed as a real signal.

**UAE-resident mapping bug (code + data)** — `are_you_a_uae_resident` (HubSpot-mapped, no native form field ever declared it) always fell into `custom_fields` and never reached the real `is_uae_resident` boolean column that every other part of the app reads — so a speaker who genuinely answered the question still showed "Not determined yet" everywhere (John Travis specifically, found live: he'd answered "Yes," the record showed unknown). Fixed the mapper (creation-time only, via the same `defaultSpeakerPublicName`-gated flag already used for the Public Name default, so it never overwrites a producer's later manual toggle) and backfilled the 4 real speakers this affected across DFS (John Travis, Callie Wang, Nazneen Abbas, Brooks Entwistle) from their own already-submitted raw answers.

**Status Board rework (code)** — new **Email** and **Assistant Email** columns (green once at least one contact email — speaker's own, or any Additional Contact — is on file). Full Bio/Photo/Passport/National ID/Short Bio now show a checkmark instead of the word "Done". UAE Resident relabeled YES/NO/UNKNOWN. Passport/National ID gained a real 3-4 state lifecycle (Pending / In Progress once uploaded-but-unreviewed / a checkmark once a producer has actually marked it reviewed) instead of just presence. Short Bio now reflects Approve-for-Announcement (a whole-record blanket approval reused here per Madhu as this column's specific "done" signal), not just text presence. The old separate **Cleaned Photo** and **Website Photo** columns were merged into one **Website Photo** column (Pending / In Progress once a raw photo's in / Cleaned once the website card is generated) — per Madhu, they're effectively generated together in one cycle now.

**Public speaker-submission form redesign (code)** — rebuilt `app/public/speaker-submission/[speakerId]/page.tsx` into two sections: **Your Speaker Profile** (Photo/Full Bio/Short Bio/Country of Residence, all four writing straight to the real `event_speakers` columns, field constraints and help text pulled live from the actual DFS HubSpot onboarding form via its Forms v3 API) and **Speaker License Requirements** (Passport, + National ID only if the speaker answers "Yes" to a UAE-resident question that's only ever asked when the record genuinely doesn't know yet). Country of Residence uses HubSpot's own 198-option list verbatim (`app/lib/forms/hubspot-countries.ts`) — there was no existing canonical country list anywhere in the codebase to reuse instead, confirmed by search. Also fixed the unstyled native file input ("Choose File No file chosen" reading as one run-together sentence), and a real bug where the preview's "Submit your details" link was live and clickable *before* Send had ever persisted a token — clicking it always 404'd ("This link is not valid"), which is exactly what Madhu hit testing this himself as speaker "John Travis (TEST)".

### Verified

Every code change `tsc --noEmit`/diff-scoped `eslint` clean (repo-wide zero-error pass run twice, once mid-session and once immediately before committing). Live-tested in-browser: the redesigned submission form (both Yes/No branches of the UAE-resident question, the character counter going over-limit, the country dropdown), the acknowledgment/reminder composers opening with correctly pre-filled editable To/Cc, the typed-DELETE modal correctly staying disabled until `DELETE` is typed, the deletion-history name fix, and the full Status Board with all new/reworked columns rendering correctly against real DFS data. The KonfHub repush was verified two ways: the script's own per-speaker delete→verify→create→verify log, and a separate independent re-fetch confirming all 50 EventPilot `konfhub_speaker_id` values match a real live KonfHub entry with none orphaned.

### What's next

- Armin Peter needs pronoun/cleaned-photo/website-photo before he can be pushed back to KonfHub.
- 4 speakers still have no Full Bio source anywhere (Antoine Chemali, Clément Aglietta, Fatih Karahan, Rishi Kapoor).
- "Notify Internal Team" — Madhu should decide whether/how it needs an edit step at all, given it's a broadcast with no per-recipient concept (see above).
- Nothing else known-outstanding from this session — see the "Left alone" row in the table above for the one explicit exception.

## 23 Sep 2026 (cont'd 3) — DFS full roster reconciliation, Cancelled tab, Clean Photo bug fix, KonfHub audit

### The ask, in sequence

A very long, multi-part live session, all DFS-focused: (1) reconcile EventPilot's DFS speaker roster against the producers' `speaker-status-board-18-sep-2026.xlsx`, handling cancellations/new speakers/status updates; (2) source missing passport/National ID/photo/bio documents from two SharePoint libraries producer Rhea/Jarryd maintain directly; (3) build a proper Cancelled tab with a KonfHub-safety-checked, double-confirmed Cancel/Restore flow (previously just a plain dropdown value); (4) fix a real "frozen" bug in the Clean Photo wizard hit live while batch-processing speaker photos; (5) fix a specific speaker's photo (PhotoRoom left a chair un-segmented in the background); (6) regenerate Website Photo in bulk after a template change; (7) audit KonfHub's live speaker listing against EventPilot and clear a stale ownership link; (8) backfill Producer/Reference/Confirmation Status/contact emails from a fresh read of the same sheet; (9) source more bios from SharePoint and auto-generate Speaker Creative Headlines for everyone who now has one.

### What was found and built

**Roster reconciliation (DB data, no code)** — matched the sheet's 52 confirmed speakers against the existing 66-speaker roster: 46 matched (16 needed a `confirmation_status` update, e.g. `On Hold` → `Reconfirmed`), 6 were sheet-only and got created fresh (checked live against KonfHub first, none matched), 19 were EventPilot-only (missing from the sheet) and got marked cancelled. One EventPilot-only record (`John Travis (TEST)`) turned out to be Madhu's own test submission from the `custom_fields` data-loss bug fix earlier that day — deleted everywhere (EventPilot, `crm_contacts`, and the real HubSpot contact it had created).

**Cancelled tab (code — `CancelledTab.tsx`, `CancelSpeakerModal.tsx`, `RestoreConfirmModal.tsx`, plus wiring in `page.tsx`/`[stakeholderId]/page.tsx`/`status-board/page.tsx`/both `speakers` API routes)** — this morning's plain `confirmation_status='Cancelled'` dropdown pick (kept the speaker visible in Overview) was replaced with a real guarded flow, per Madhu: cancelled speakers now live in their own tab, and Cancel requires (a) removing any live KonfHub Speakers-listing first — a real, verified API call, not a guess — and (b) if the speaker has a live KonfHub Attendee Registration (which has no cancel API at all, confirmed against KonfHub's own docs), a required human-acknowledgment checkbox, since the system can never verify that side automatically. Every action is double-confirmed (a second inline "Confirm?" step), never one click. The Overview roster's list query now excludes `confirmation_status='Cancelled'` by default — watch for the nullable-column `.neq()` gotcha this required working around (`NULL <> 'Cancelled'` is `NULL` in SQL, not true, so a plain `.neq()` would have silently hidden every speaker with no status set at all; fixed with an explicit `.or(...is.null)`).

**Clean Photo wizard bug (code — `PhotoCleaningWizard.tsx`)** — Madhu hit a real, 100%-reproducible "frozen" bug live while batch-running Clean Photo: clicking "Use As-Is" on the gap-warning popup lands on the Cleaned Photo step, which turned out to be a genuinely non-interactive confirm screen (built for the deterministic good/enhance path) now also being the landing point for the AI-Fill path, where the crop can still have a real gap. The dashed ring shown there looked identical to the real draggable ring used elsewhere, but had `pointerEvents: none` — no code bug, no lost data (Madhu's own workaround of closing/reopening the page proved the photo was saved fine each time), just no discoverable way to fix a bad crop from that screen. Fixed: added an explicit "Not right? Redo" button (routes back to Compose, reusing the existing `resetToCompose()` the generic Cancel already used for this phase), and changed the ring to solid with a "not adjustable here" caption so it stops implying an affordance that isn't there.

**Ashish Mehrotra's photo (data only)** — PhotoRoom's segmentation had left his office chair as opaque black "foreground" behind his head/shoulders instead of cutting it out. Fixed using the wizard's own "Refine with AI" pipeline (`refineWithInstruction` → `removeGreenScreenBackground` → `finalizeCleaningCycle`, the same functions `clean-photo/generate`'s `chat_refine` mode calls) with an instruction to remove the chair — verified by reading the alpha channel directly (clean 0/255 cutout, no lingering silhouette), not just eyeballing it.

**Website Photo bulk regeneration (data only)** — 30 speakers (Jessica Renier through Simon Sharp in Overview order) regenerated against a new template, reusing the exact same alignment/composite pipeline `website-photo/generate/route.ts` uses — deliberately did *not* touch Clean Photo, per explicit instruction.

**KonfHub audit + ownership-link clearing (data only)** — compared all 51 speakers currently live on KonfHub's DFS category against EventPilot. Two were live on KonfHub but had no `konfhub_speaker_id` link at all (Armin Peter, Remo Giovanni Abbondandolo — flagged, not yet fixed). More importantly: Madhu confirmed **none of the 49 currently-linked DFS speakers were ever actually pushed to KonfHub through EventPilot** — they were uploaded directly by producers by other means, and EventPilot's `konfhub_speaker_id`/`konfhub_synced_at` only got populated via an old one-time name-matching bridge exercise. Per Madhu's plan (one more producer fact-check round, then remove everything from KonfHub and push fresh from EventPilot going forward), cleared `konfhub_speaker_id`/`konfhub_synced_at` for all 49 — **no KonfHub API calls made, nothing removed from KonfHub itself**, purely local bookkeeping so EventPilot stops claiming ownership of listings it didn't create. **Real interim risk**: until the remove-all-then-push-fresh happens, clicking "Push to KonfHub" on any of these 49 in EventPilot will create a duplicate live listing, since EventPilot no longer has the id to update against.

**Sheet-driven field backfill (data only)** — fresh read of the same 18-Sep sheet to populate `producer_staff_id` (only for Rhea Soans/Jarryd Aidan Visser — the two actual Producer-role holders on this event; "Vijay" and "Kat" in the sheet don't match any real staff member, so left blank there while still recording them in the free-text `reference` field), `reference`, `confirmation_status`, `custom_fields.email` (14 speakers), and 33 `speaker_additional_contacts` rows (assistant + team emails) — names only filled when the email clearly followed a `firstname.lastname@` pattern, left blank otherwise rather than guess from initials-only addresses.

**Full Bio + Headline generation (data only)** — cross-checked a second SharePoint "Speaker Assets" library (photo + bio per speaker, maintained separately from the passport library) against the sheet's doc-status gaps; sourced 22 more Full Bios (`bio_full_url`/`bio_full_text`, same `toStoredBioPdf` docx→PDF conversion the real onboarding pipeline uses) — 4 speakers (Antoine Chemali, Clément Aglietta, Fatih Karahan, Rishi Kapoor) still have no bio source anywhere. Then auto-generated Speaker Creative Headlines (`generateHeadlines()`, the same function the Details page's own button calls) for all 22 newly-bio'd speakers — 5 proposed options each, none auto-selected, left for a producer to pick.

**Two also created directly** — Charlotte Robins (DFSA) and Dr. Adnan Chilwan (Dubai Islamic Bank) — neither has a HubSpot submission or sheet row, but producers confirmed them as real confirmed speakers pending detail collection. Adnan's only SharePoint file was a text-only "Passport & EID Details" doc, not a scan — deliberately not imported as a document per Madhu; producers will request real scans.

### Verified

Every data change cross-checked with a fresh DB query after running (matched-count re-diffs, alpha-channel inspection on the chair fix, live KonfHub API calls for the audit). `tsc --noEmit` and diff-scoped `eslint` clean on every code change (`PhotoCleaningWizard.tsx`, the Cancelled-tab files, both `speakers` API routes). Cancel/Restore flow live-tested end-to-end in-browser (a live speaker with no KonfHub ties cancelled and restored, confirming the double-confirm gate and count badges update correctly) — the KonfHub-listing-removal half of the gate was deliberately *not* live-tested against a real speaker (would have made a real public listing disappear); Madhu should try that specific path with a disposable/duplicate record if he wants it proven before relying on it for real.

### What's next

- **DFS/producers**: one more fact-check round on the current roster (per Madhu's own plan), then remove all 49 current KonfHub listings and push fresh from EventPilot — see the interim risk note above.
- Armin Peter and Remo Giovanni Abbondandolo are live on KonfHub with no `konfhub_speaker_id` link — worth backfilling once their real KonfHub speaker ids are known, or folding into the remove-all-then-push-fresh pass.
- 4 speakers still have no Full Bio source anywhere found (Antoine Chemali, Clément Aglietta, Fatih Karahan, Rishi Kapoor); 2 have a Full Bio file but no extractable text (Nicolas Moreau, Noel Quinn — likely scanned/image PDFs).
- Charlotte Robins and Dr. Adnan Chilwan are placeholder records pending producer-supplied details (photo, bio, real passport/EID scans for Adnan).
- The KonfHub-listing-removal half of the new Cancel flow's gate hasn't been proven live yet (see Verified above) — worth a deliberate test before leaning on it for a real cancellation that has a live KonfHub listing.
...
Continued DFS readiness testing. Started with smaller HubSpot form-mapping gaps (auto-suggest field mapping for newly-connected forms based on AI InfraNext's already-mapped fields, a field-type editor for HubSpot-connected Speaker Properties, legal-consent checkbox support), then moved into the AI-generated on-image "Speaker Creative Headline" feature (built from 3 real DFS creative samples) — Madhu rejected the first implementation as announcement-scoped ("why go in circles?") and had it rebuilt to live on the speaker record instead, generate once, reuse forever. While reviewing External/Client approval, Madhu explicitly overturned the existing "only Primary gates" design in favor of first-responder-wins for both flows, since high-profile speakers/clients often have an assistant act first. Session closed out with a real live bug report: a test speaker's email was set but the approval flow said none existed and the Registration tab didn't show it either — traced to the Registration tab showing an uncurated "everything not on Overview" field list (never had an email/phone field at all for a HubSpot crm_property-mapped event like DFS) AND a genuine data-loss bug in the save path.

### What was built

- **HubSpot field-mapping auto-suggest** (`app/lib/hubspot/suggest-field-mapping.ts`) — newly-connected HubSpot forms now get an initial mapping suggested from AI InfraNext's own already-mapped fields as reference, validated against the event's real schema/crm_properties before suggesting; unmatched fields are left pending for manual mapping. Wired into both initial connect and resync (resync only suggests for newly-unmapped fields, preserves existing ones).
- **Speaker Properties field-type editor** (`AddFieldForm.tsx`, the HubSpot-form-mapping page) — HubSpot-connected events previously had a read-only Speaker Properties page; producers can now edit a field's type in place (e.g. fixing a mistakenly-saved Text vs Checkbox field) without Claude/SQL involvement.
- **Legal-consent checkbox support** (`app/lib/hubspot/client.ts`, `.../public/hubspot/submissions/route.ts`) — HubSpot's native Legal Consent structure (`legalConsentOptions`) is architecturally separate from its Forms field groups, so a consent checkbox never showed up for mapping. Now injects consent status into `properties` before mapping runs (via HubSpot's communication-preferences API), so it flows through as "just a field with a value" — no special-casing downstream.
- **Speaker Creative Headline** — see memory `eventpilot-speaker-creative-headline` for the full design writeup (mid-course redesign from announcement-scoped to speaker-scoped, 3-segment `TextLayer` chain, `allow_shrink` opt-out so long headlines wrap instead of shrinking, warn-only compliance). New `event_speakers.headline_variants`/`selected_headline_variant_id`, `HeadlinePicker.tsx`, `.../speakers/[id]/generate-headlines/route.ts`. Creative-variant pickers grey out any variant needing a headline for a speaker without one selected yet.
- **Full Bio text pre-extraction** — see memory `eventpilot-full-bio-text-extraction`. Every Full Bio upload (any source format) now also stores extracted plain text (`event_speakers.bio_full_text`) alongside the PDF, at upload time, for reuse by Short Bio and Headline generation without re-parsing the PDF on every AI call.
- **First-responder-wins approval redesign** (both External and Client flows) — per Madhu: "whoever responds first will be considered, and immediately the link will show 'already approved' by so-and-so." New `app/lib/events/approval-round.ts` (shared round-resolution logic), new `announcement_external_approval_cc` table (mirrors the existing client one), full rewrite of the approve/review-data/compose/send routes and both composer UIs for the External flow to match Client's existing shape. Publish-gating (`client-approval-gate.ts`, `postiz-publish.ts`) and the announcements list route updated to resolve "whoever acted first" instead of only checking the Primary/main row.
- **Composer bug fix** — Send for External Approval / Send to Speaker / Notify External composers were wrongly offering a CC'd Additional Contact as an alternate primary "To" recipient instead of always defaulting to the speaker with the assistant CC'd.
- **Registration tab rebuild + a real custom_fields data-loss bug fix** — see memory `konfhub-speakers-integration-status`'s 2026-09-23 update for full detail. Short version: (1) the tab now shows exactly what `konfhub-registration-push/route.ts` actually sends (email/phone_number, synthesized as editable fields when the schema doesn't declare them, plus whatever extra fields this event has mapped in `konfhub_registration_field_map`) instead of every leftover custom_fields-only field including short bio/quote/assistant details/consents; (2) fixed a real bug where `PATCH .../speakers/[id]` and `.../partners/[id]` replaced `custom_fields` wholesale on every save instead of merging, silently wiping any key not declared in the current schema (exactly what happened to the test speaker's email) — now merges onto the existing row's `custom_fields`. New shared constant `app/lib/konfhub/registration-fields.ts` also fixed a small pre-existing oversight (`'email'` missing from the Integrations page's own auto-sent-fields list).

### Verified

`tsc --noEmit` and `eslint` clean throughout every piece. Not yet live-verified in-browser this session (typecheck/lint only) — next session should confirm live: generate a headline for a real DFS speaker and bake it into a creative; send an External approval with a CC and confirm the CC's link can act and resolve the round; and set the test speaker's email via the new Registration tab field, save, reload the page, and confirm it's still there (the exact repro for the data-loss bug that's now fixed).

### What's next

- **DB migrations not yet run against production** — see "DB migrations applied" row above, run manually before relying on any of this session's new columns/table in prod.
- Live-verify everything in "Verified" above against the real DFS event.
- DFS's own `konfhub_registration_field_map` is still not configured on KonfHub's side (per Madhu, "still not setup and ready on the konfhub side") — re-check status before assuming it's ready.

## 17 Sep 2026 — Press Release Studio (AI Content Studio, Phase 1) + live-iterated refine/compliance UX

### The ask

Madhu wanted an internal LLM tool for content teams to research and generate copy (email campaigns, social posts, video scripts, press releases), workspace-scoped per event. Scoped down deliberately: build one vertical slice end to end first (research → generate → version → approve) rather than the whole framework — PR's Press Release Studio, chosen over Marketing (whose Social Media Post generator already existed). Model choice and cost were scoped with real numbers (~$0.20/release worst case) before building. Then iterated live through several real-usage rounds: model ID bug fix, chat UX (Shift+Enter, bigger legible text, real markdown), a full Lovable-style split-pane redesign, and — after Madhu actually tried refining a generated draft and hit a hard wall — a whole second feature (refine-chat + compliance-check) designed and built in the same session.

### What was built

- **Data model** (`supabase/content_studio_press_release.sql`): `press_releases` + `press_release_versions` (mutable in place while `approved_at IS NULL`, frozen once approved — any further edit on an approved version copy-on-writes a new one, never mutates it) + `content_research_sessions`/`content_research_messages` (deliberately generalized, not press-release-specific, so Email Campaign/Video Script can reuse the same chat plumbing later).
- **Permissions**: 3 new keys under `sae.content_studio.press_release.*` (`view`/`generate`/`approve`) in `app/lib/registry/access-permissions.ts`, registered as a module in `app/lib/registry/modules.tsx` chained through `admin-event-workspace` (same breadcrumb convention as Stakeholder Hub, not `toolkit`).
- **Auto-provisioned access** (`supabase/content_studio_press_release_role_mapping.sql`): wired into `hrms_role_access_map`, a mechanism built 2026-08-16 but never populated until now — Staff Portal `role_type` `media` → generate, `media_lead` → generate + approve, backfilled immediately for everyone already allocated (confirmed live for Dubai FinTech Summit: Chrishtel Torres Lacsamana/Imran Mushtaq/Reeha Haris as Media, Shadi Dawi as Media Lead) and kept current by the existing Staff Portal sync.
- **Research/refine chat** (`app/api/events/press-releases/[id]/research/route.ts`): mode-switches persona once a draft exists — pure research (asks clarifying questions, never drafts) before a draft, copy-editor (proposes structured, chip-selectable edits via a parsed-and-stripped `EDIT_OPTIONS` marker block, same pattern as the `READY_TO_GENERATE` marker) after. Cross-release memory: pulls the last 5 *approved* releases for the same event into context so a new release doesn't repeat an old one's angle.
- **Generate** (`.../generate/route.ts`) and **apply-edit** (`.../apply-edit/route.ts`, new) routes — the latter applies a chat-suggested or compliance-suggested edit to one field, in place if unapproved, copy-on-write if approved.
- **Compliance check** — reuses the existing deterministic rule engine (`app/lib/content/validate.ts`, the same one the Announcement Engine already runs) rather than inventing a new one; extended with an optional `suggested_fix` on `ValidationFinding`. New `.../suggest-fixes/route.ts` batches an AI call so findings *without* a mechanical fix (most of them) still get a direct, editable, one-click replacement instead of defaulting to "discuss in chat." Findings render as a unified list above the copy, auto-rechecks once the queue empties, shows "✓ All good" when clean.
- **Models**: `gemini-3.7-flash` for chat (auto-escalates to `gemini-3.1-pro-preview` past 14 messages in one thread — a long thread is a proxy for "this story is genuinely complex"), `gemini-3.1-pro-preview` for generate. Built on the 3.x lineup from day one since the whole Gemini 2.5 family retires 16 Oct 2026 (found via live pricing research, not assumed) — this is the only tool in the app already migrated.
- **UI** (`app/admin/events/[id]/press-releases/{page,layout}.tsx`, `[prId]/page.tsx`): list + a full-height, resizable two-pane workspace (research chat / generated content, thin hover-lit draggable divider) modeled on Lovable per Madhu's explicit reference. Toast notifications on state-changing actions (edit applied, draft generated, version approved). Chat answers lock into a read-only "✓ Applied" row once used so people stop re-clicking something that already landed. Also added as a card on the event workspace overview page and both left-sidebar nav trees.
- **Bug fixes found live**: `gemini-3.1-pro` (no suffix) 404s, needed `-preview`. The page's own `height: 100vh` double-counted `GlobalShell`'s non-fixed nav+breadcrumb chrome, causing whole-page scroll instead of pinned panels — fixed to a measured `calc(100vh - 55px)`. A real data-corruption bug: accepting one compliance fix left *other* open findings for the same field holding stale character offsets, so applying a second one spliced text into the wrong position — fixed by invalidating that field's cached findings after every edit rather than trusting them, letting the auto-recheck give a fresh read.

### Verified

`tsc --noEmit` clean throughout every pass. Every major piece live-verified in-browser against the real Dubai FinTech Summit event (not just curl/typecheck): full research→generate→approve flow; refine-chat proposing real headline options and applying one in place (version number didn't change); Check Compliance surfacing 5 real findings against the event's actual style-guide rules, including an AI-generated fix for a finding type that has no mechanical answer; accepting a fix and watching the live counter drop; the corrupted-text bug reproduced then confirmed fixed via a clean regenerate; toast notifications caught mid-animation on approve; the answered-chip locking confirmed on a second fresh test release.
...
Madhu attached a pre-written spec (`~/Downloads/EventPilot_Content_Guidelines_API_Spec.md`) and asked for a smaller, urgent build in a separate session from the one already running on Agenda Builder: a public, bearer-token-scoped `GET` endpoint so an external tool — specifically Antigravity, working in the `~/AIInfranext` site repo — can fetch an event's approved messaging/style-guide/production-pack guidance before generating content, and validate copy against it afterward, all without an EventPilot session.

### What was built

- **`GET /api/public/v1/content-guidelines`** — `Authorization: Bearer ep_cg_<token>`, `format=markdown` (default, an LLM-readable rendering) or `json`, `include=` filters by role. Merges `event_compiled_reference` + effective validation rules. ETag/`If-None-Match` → 304. Two deliberate departures from the spec's literal (generic/wrong-for-this-schema) example: "event facts" comes from `events`' own `TRACKED_EVENT_FIELDS` columns, not a nonexistent `default_fields` field on the compiled reference; `document_version`/`approved_at` come from the compiled reference's own version/`compiled_at` plus the latest source doc's `updated_at` (there's no separate `approved_at` column).
- **`content_guideline_tokens` table** — `app/lib/content/guideline-tokens.ts` handles generate/hash/verify, a 60/hour rate cap tracked directly on the row, `last_used_at`. `created_by` references `staff_members` (the spec's own example schema named a nonexistent `profiles` table).
- **Dual auth on the existing `POST /api/events/stakeholders/content/validate`** — now also accepts the bearer token (deriving `event_id` from it, ignoring any `event_id` in the body so a token can't be pointed at another event); the original session-based calling convention is unchanged. `middleware.ts` carries a narrow exemption scoped to exactly this path AND only when a `Bearer` header is present.
- **Token management** — session-gated API (`/api/events/content-guideline-tokens`, `sae.integrations.manage`) plus a UI card on the Integrations page (generate/label/show-once/revoke), added after the page's earlier concurrent-session edit conflict cleared.
- **Side-effect bug fix**: building this surfaced that the *existing* messaging-doc upload route had the same Cloudflare ~100-125s proxy timeout bug already known from KB Intel — confirmed live via Railway HTTP logs (a real AI InfraNext upload: `POST ... 499 125008ms`, draft saved successfully 3 minutes later, UI wrongly said "Upload failed"). Fixed with the same background-job+poll pattern: the route now returns immediately after inserting a `'processing'` draft row, the two Gemini calls (PDF extract, then structure) run after the response, `event_messaging_docs.extraction_status` tracks progress, and the frontend polls and shows an explicit "Extracting…" state instead of a silently-empty draft.

### Verified

`tsc`/`eslint` clean throughout. Full round-trip tested live against production: created a real token for AI InfraNext Indonesia 2026, fetched guidelines (markdown + JSON, `include=` filter), hit ETag 304 and auth-rejection paths, validated real copy (correctly flagged a banned abbreviation) — then confirmed a bearer-token call can't be redirected to a different event via the body. Token UI verified live in-browser (generate/copy/list; Revoke's `window.confirm` intentionally not click-through-tested per this session's own browser-automation rules, but the underlying DELETE route was verified directly). Messaging-doc fix verified by re-uploading the actual PDF that had failed in production as a throwaway new version: `POST` returned in 685ms (was 125s+/499), extraction completed in the background ~120s later, UI transitioned automatically with no refresh — then the test draft was fully deleted (DB row + storage file). Closed the loop by proving the whole thing from the *consumer* side: Antigravity's own `npm run guidelines`/`npm run validate <file>` scripts (in `~/AIInfranext/scripts/`) both work end to end against the live API — which is what caught the one real bug (the API's own title line used an em dash, violating AI InfraNext's own house style; fixed to en dash unconditionally).

### What's next

- No known follow-up — feature is complete and proven from a real downstream consumer, not just curl.
- The deterministic validator will always show noisy false-positive findings if pointed at a style-guide document itself (it quotes its own banned forms as examples) — expected behavior, not a bug, documented in code comments.
- Worth a look sometime: audit other routes doing multiple sequential Gemini/slow-external calls synchronously in-request for the same Cloudflare timeout risk — this is the second time it's been found live (KB Intel, now messaging-doc upload) rather than caught in review.
...
Two independent fixes. (1) Madhu had been iterating in a separate Claude chat on why DFS (Dubai FinTech Summit) speaker announcements kept failing their own DIFC content-validation rules, and brought back a precise diagnosis to implement: `generatePostCopy()`/`generateSelfPromoPostCopy()` (`app/lib/events/announcements.ts`) baked in example boilerplate phrasing and an invented date format that DIFC's rules explicitly forbid — the prompt was instructing the model to fail its own validation. (2) Madhu flagged that speaker photos pushed to KonfHub go straight to the public event website as high-resolution PNG/JPEG, making the site heavier than necessary, and asked whether WebP conversion could be built into the existing Website Photo generation cycle so the review/approval flow stays the same but what's stored and pushed is WebP.

### What was built

- **Prompt fix**: removed the three hardcoded "Excited to welcome..."-style example phrases (keeping the underlying "must unambiguously state they're speaking" requirement) and the invented "9-10 Sept 2026" date-format example (now reuses the real `Dates`/`Venue` values already in context). Restructured both prompt functions so the messaging-doc rules block now appears *after* the generic structural/tone guidance, with an explicit "rules always win on conflict" statement — models weight later instructions more heavily, and the old ordering had rules arrive first, then get contradicted by "DIRECT, ENERGETIC"-style tone directives further down. Softened that tone language into an explicitly overridable default. Added `normalizeTitle()` — spells out "&" as "and" wherever a speaker's title enters either prompt (`event_speakers.role` regularly carries dual titles straight from Staff Portal data, e.g. "CEO & Co-Founder", tripping the `difc-ampersand` rule on every generation). Also fixed `difc-standalone-digit` (a DFFW-umbrella-level validation rule) to stop flagging digits in a correctly formatted date range next to a full month name, via a negative lookahead — it was firing on the "2" and "3" in "2 - 3 November 2026" on every single generation.
- **Website Photo → WebP**: `app/lib/media/composite-on-background.ts` (the Sharp compositing step both the automatic Website Photo generator and the template-editor's live preview share) now takes an explicit `format: 'png' | 'webp'` option — defaults to `'png'` so the unrelated template-editor preview (`app/api/events/templates/preview/route.ts`, which hardcodes a `data:image/png` URL, nothing persisted) keeps working unchanged. The real generator (`app/api/events/stakeholders/speakers/website-photo/generate/route.ts`) now passes `format: 'webp'` and stores/serves the result as `image/webp`. The manual-override upload path (`.../website-photo/upload/route.ts`, for when branding hand-produces one) now re-encodes whatever's uploaded (jpg/png/webp) through Sharp into WebP before storing, so `event_speakers.website_card_url` is guaranteed WebP regardless of which of the two write paths produced it. KonfHub's own push code (`konfhub-push`/`konfhub-push-secondary`) needed no changes — confirmed by reading it that it does a raw URL passthrough with zero format/extension assumptions. Deliberately left untouched: `photo_url` (raw upload, feeds EventPilot's own public event page, not KonfHub) and `photo_processed_url` (shared Cleaning Cycle intermediate, kept PNG on purpose to dodge an alpha-transparency bug fixed 2026-09-08) — neither of those goes to KonfHub.

### Verified

`tsc`/`eslint` clean throughout both fixes. **Prompt fix**: 3 live DFS speaker-announcement generations in a row (Roham Gharegozlou, an ampersand-title speaker) — zero boilerplate, zero abbreviated months, zero en-dash date-range findings across all three; draft rows + creatives cleaned up after. **WebP conversion**: tested against a disposable test speaker first (1.26MB PNG → 41KB WebP, ~97% smaller, visually lossless at quality 85), then a full real end-to-end production test per Madhu's own request — duplicated "Jigar Patel" (DFS's actual last KonfHub speaker) as "Jigar Patel (WebP Test)", pushed his real photo through the WebP upload path (1.28MB → 42.5KB), pushed to KonfHub as a genuinely new speaker (id 29182), reordered to position 50/50 (last), Madhu confirmed it looked right on both KonfHub and the live event website, then fully deleted (KonfHub listing removed via the app's own delete route, EventPilot row hard-deleted, the one new WebP storage file removed — Jigar's own real photo files were never touched, only referenced).

### What's next

- No code follow-up known at this time — both fixes are shipped and live-verified against real production data.
- Worth keeping an eye on the next few real DFS announcement generations to confirm the prompt fix holds up across a wider variety of speakers/titles than the 3 tested.

## 11 Sep 2026 — Reference Documents & Content Validation (4 stages) + clarification Q&A, Umbrella/Event structural separation, Access control eligibility gate + stale-access report

### The ask

Three linked pieces of work, built in sequence in one long session. (1) Madhu pointed to a pre-written spec (`docs/build_suggestions/reference-documents-and-validation.md`) and asked for all four stages: per-event/per-umbrella reference documents (style guide, messaging, production pack) with authority ranking and client-approval gating; a compiled-reference materializer that merges live docs and flags (never auto-resolves) conflicts; deterministic content validation rules; and SAE-reads-compiled-reference wiring into the announcement pipeline. Mid-build, Madhu redirected the "ask the uploader clarifying questions" idea toward an `AskUserQuestion`-style multiple-choice flow (round-capped at 5, no early exit — "these documents are touched once or twice in an event's lifecycle") and asked for answered clarifications to auto-suggest (never auto-activate) validation rules. (2) Madhu noticed Staff Portal lists DFFW as a distinct "Umbrella Event" type, simplified, with no full event workspace — while EventPilot listed it as a regular event — and asked EventPilot to carry that distinction forward, explicitly for the sake of the planned Staff-Portal-into-EventPilot merge in a couple of months. (3) Madhu asked how per-module access should be scoped given that Staff Portal already staffs each event with an "Event Staff" roster, and access to any module within an event should be limited to people on that roster — asked to build both a gate stopping new violating grants and a report surfacing any that already exist.

### What was built

- **Reference Documents (Stage 1)**: `event_messaging_docs` table — `role` (style_guide/messaging/production_pack), `authority_rank`, `provenance` (client_approved/trescon_authored), one live doc per (owner, role); upload/list/approve routes under `app/api/events/stakeholders/messaging/`.
- **Compiled reference (Stage 2)**: `app/lib/content/compile-reference.ts` — versioned, never overwritten, merges all effective live docs' sections with source metadata; conflict detection via Jaccard similarity OR a shared "distinctive" token (≥8 chars) for rules, exact-key+different-detail for facts; a real conflict never silently resolves, the loser stays visible and flagged.
- **Deterministic validation (Stage 3)**: `app/lib/content/validate.ts` — pure/synchronous rule engine, no model call: `forbidden_term`, `forbidden_pattern` (regex), `required_format` (JSON `{trigger,allowed}`), `proximity` (JSON `{a,b,maxDistance}`). `app/api/events/stakeholders/content/validate/route.ts` exposes it.
- **SAE wiring + release gate (Stage 4)**: announcement generation (`app/lib/events/announcements.ts`, `.../announcements/generate/route.ts`) now reads the compiled reference; `app/lib/events/client-approval-gate.ts` blocks publish (`postiz-publish.ts`, `send-for-approval/route.ts`) until `requires_client_approval` is satisfied.
- **Clarification Q&A**: during Gemini extraction, the model generates multiple-choice questions (not open chat); answered in bounded rounds (hard cap 5, no way for the uploader to opt out early); answers fold back into `structured_json` via a second Gemini pass, which can also propose further rounds or suggest validation rules. `app/lib/content/clarifications.ts`, `.../messaging/[id]/clarifications/` + `/clarifications/answer/` routes, `ClarificationsPanel` in the event Details page.
- **Suggested-rules bridge**: clarification answers can suggest a validation rule, surfaced for a human to activate — never auto-activated. `.../messaging/[id]/suggested-rules/` routes, `SuggestedRulesPanel`. Fixed a bug where `source_clarification_id` wasn't populating (model didn't reliably echo which question it answered) — falls back to the single answered question in that round when there's exactly one.
- **Umbrella/Event separation**: new `event_umbrellas` table, `events.umbrella_id` (nullable FK, replacing the old `parent_event_id`). Dual ownership on `event_messaging_docs` / `event_validation_rules` / `event_messaging_doc_edits`: nullable `event_id`/`umbrella_id` pair + `CHECK` (exactly one set) + **separate** partial unique indexes per owner column (Postgres NULLs don't collide in a single shared unique constraint). `app/lib/content/owner.ts`'s `ContentOwnerRef` discriminated union (`{kind:'event'|'umbrella', id}`) is now the standard way resolvers say which table they mean; inheritance resolves depth-2 only (event → its own umbrella, never deeper — DFFW is a one-level initiative). New simplified umbrella workspace (`app/admin/umbrellas/[id]/page.tsx`, index at `app/admin/umbrellas/page.tsx`, linked from `/admin`) — deliberately plain, not the grouped-list visual from Staff Portal's screenshot; existing per-event workspace is untouched for regular events. `parent_event_id` was dropped only after data migration + resolver/route/sync/UI rewrites were all verified live.
- **Access control — eligibility gate**: `app/api/events/access/assignments/route.ts` POST now checks the target staff has a matching `event_staff` roster row before allowing an event-scoped grant (422 if not); global (org-wide) grants stay exempt since they were never meant to be tied to one event's roster.
- **Access control — stale-access report**: new `GET /api/events/access/stale` finds existing event-scoped grants whose (event, staff) pair no longer has a roster row, and a new "Stale Access" tab (`app/admin/access/StaleAccessTab.tsx`, third tab on `/admin/access`) lists them with a Revoke action reusing the existing `DELETE /api/events/access/assignments/[id]` endpoint. Deliberately read-only/report-only — never auto-revokes.

### Verified

`tsc --noEmit` and `eslint` clean throughout. Every stage was tested live against production with disposable test data (UUID pattern `00000000-...`), cleaned up after each check — including a full upload→approve→child-recompile→verify-compiled-reference-content pass after the umbrella separation, and a WAIS Malaysia regression check confirming single-document (non-umbrella) events saw no behavior change. The eligibility gate was tested both ways (a non-rostered grant attempt → 422; a rostered one → 201) directly against production via curl with a real admin session. The stale-access report was verified by temporarily removing a real roster row, confirming the grant surfaced, confirming Revoke removed it, then restoring the exact original roster row — and in the process surfaced one real, still-open stale grant worth a look: **Khalifatur Rahman holds "Branding" access on World AI Show – Malaysia 2026 without being on that event's staff roster** (left untouched, flagged for Durga/Madhu to decide).

### What's next

- Decide whether to configure `hrms_role_access_map` (Staff Portal role_type → access-role bundle) for real — currently empty/dormant, only 5 access bundles exist against 15+ real Staff Portal role types.
- Review and act on the one pre-existing stale grant the report surfaced (Khalifatur Rahman / Branding / WAIS Malaysia 2026), from `/admin/access` → Stale Access tab.
- No UI yet shows the umbrella→children grouping visually on the main events list (the Staff-Portal-style nested view) — deferred, same as 10 Sep's note.
- Event-level Messaging/Style Guide inheritance edge cases beyond depth-2 were deliberately not built (matches DFFW's actual one-level structure; revisit if a deeper hierarchy ever shows up in Staff Portal).
...
A long DFS-focused session covering, in sequence: (1) reorganize the Integrations/Event Workspace pages into left-nav (carried over from a prior session's start), (2) real-world testing of the photo-cleaning wizard against DFS's actual speaker roster, which surfaced several real bugs to fix live, (3) reconcile DFS's speaker data across three sources — EventPilot, KonfHub, and the producers' own Excel tracker — filling gaps in each direction, (4) put the Sensitive Documents module (built 06 Sep, never actually used) to real use: source real passport/ID files, cross-reference, upload, (5) add UAE-resident-aware Passport/National ID requirement tracking, mirroring new logic in the HubSpot onboarding form, (6) rebuild the Speaker Onboarding Status Board with frozen header/column, colored status text, and several new columns, and (7) build a new Speaker Order page so producers can drag-reorder speakers in EventPilot and push the result to KonfHub in one click, once KonfHub added the API endpoint needed for it.

### What was built

- **Left-nav reorg**: per-event Integrations page and Event Workspace overview moved into left-panel navigation with breadcrumb fixes; Admin Dashboard's testing shortcuts trimmed to a single Stakeholder Hub link.
- **Photo-cleaning pipeline — three real bugs found and fixed, all against actual DFS speaker photos**:
  - Compounding double-crop: a Cancel-then-redo cycle on the same speaker recropped an *already-cropped* photo instead of the original raw, because `runProcessing()` didn't pass `source_url` and the route silently fell back to whatever `photo_processed_url` already held. Now always pins to the original raw photo.
  - JPEG-alpha-flatten-to-black: `alignAndCropPhoto` padded gaps with true transparency, but for a background-intact raw JPEG (no alpha channel to begin with), sharp silently re-encoded the output back to JPEG on `.toBuffer()`, flattening the padding to solid black instead of transparent. Fixed by forcing `.png()` at every buffer stage in `face-alignment.ts`.
  - No-pause finalize: `finalizeClean()` (both the gap-popup's "Use As-Is" and the normal Continue) jumped straight into an auto-triggered Website Photo generation with zero review step — now lands on the same "Cleaned Photo" confirmation screen the good/enhance path already used.
  - Plus three smaller UI fixes found live: the wizard's right pane not scrolling (grid `min-height` default), unreadable "Refine with AI" copy, and the gap-confirmation popup showing no photo to actually judge.
- **DFS speaker data reconciliation** (data work, no schema change): cross-referenced EventPilot vs. KonfHub vs. the producers' Excel tracker in all directions. Created 5 missing speaker records (Eric Rosenthal + 4 others) from the Excel tracker; populated a `reference` field (who originally sourced each speaker — Rhea/Jarryd/Vijay) for 48 speakers; identified 16 speakers present in EventPilot/Excel but never pushed to KonfHub and marked them **On Hold**.
- **Sensitive Documents put to real use**: sourced real passport/Emirates ID files from a local folder Madhu provided, cross-referenced by name against EventPilot's speaker records (flagged and resolved one misfiled document, one initially-unmatched speaker), then bulk-uploaded 21 real documents (19 Passports + 2 National IDs, including George Theocharides with both) via the existing upload pipeline.
- **UAE Resident field**: new `event_speakers.is_uae_resident` (nullable boolean, producer-set by hand since the onboarding form only started asking this recently). Mirrors the HubSpot form's own rule — UAE residents need both Passport and National ID, everyone else only needs Passport. Sensitive Documents tab got a 3-way toggle; National ID now shows "Not Applicable" wherever a producer's explicitly marked someone not a UAE resident (still shows as required/missing for "not yet determined," since it might still turn out to apply).
- **Confirmation Status**: converted from a free-text field to a proper dropdown (`New Confirmed`/`Reconfirmed`/`Confirmed`/`On Hold`, DB-constrained), with a new **On Hold** state (shown in red) — applied to the 16 KonfHub-unsynced speakers above.
- **Status Board rebuilt**: sticky header rows + sticky Speaker column (three separate real CSS bugs found and fixed along the way — an overflow-x/y auto-promotion quirk that silently broke page-scroll tracking, a border-collapse+sticky-cell clipping quirk, and a 1px header-row height mismatch from mixing rowSpan/colSpan cells); dot indicators replaced with colored status text; new **Confirmation Status**, **UAE Resident**, and **Ref** columns; colored group-divider lines for Collection/Production/Publish; widened Speaker (name+company only) and Producer columns; new **Download Excel** export button matching the board's exact columns.
- **Speaker Order page** (`/admin/events/[id]/stakeholders/speaker-order`, new): drag-to-reorder speakers already live on KonfHub, backed by a bulk `PUT /speakers/reorder` endpoint KonfHub added specifically for this (confirmed live via their Postman docs, then verified with a real swap-and-revert against DFS's actual event before building anything). Also added **Reset to Previous Order** (instant local undo) and **Move to [x]** (type a target position, everyone else shifts) per Madhu's follow-up ask.
- **Local dev environment**: diagnosed and fixed a `require is not defined` 500 on the speaker Details page — a stale Turbopack dev cache after a very long hot-reload session, not an app bug; fixed with a clean `.next` wipe + restart.

### Verified

`tsc --noEmit` and diff-scoped `eslint` clean on every change. Photo-cleaning fixes verified against real DFS speaker photos (headwear/ghutra case, background-fringing case, the compounding-crop case, the black-band case). Sensitive Documents upload verified via a direct DB count (19 passports/2 national IDs) after the bulk script ran. UAE Resident toggle verified live end-to-end on a real speaker record (set, confirmed National ID flipped to Not Applicable, reset back to Not determined). Status Board sticky/color fixes each verified via direct pixel measurement against real screenshots, not just visual guessing (two of the "fixes" earlier in the process turned out not to actually work until measured properly). Speaker Order tested live on two real events (DFS and WAIS Malaysia) — drag, Move to [x], Reset to Previous Order, and a real "Update Order on KonfHub" push all confirmed working.

### What's next

- The 11 KonfHub-unsynced speakers who now have their Passport on file still need photo cleanup + Website Photo generation before they clear the KonfHub push gate.
- UAE Resident is still "Unknown" for every speaker except the ones a producer has explicitly set — that's a manual pass the team still needs to do.
- Sensitive Documents permission check never followed up: confirm which producers actually hold `sae.sensitive_documents.manage`.
- The two pre-existing bugs noted in the table above (`checklist` department column, `feedback` table) — flagged, not fixed, no urgency expressed.
- Same open items as 06 Sep below, still untouched: Pixelate contact roster, Khalifa "Go Live" end-to-end verification, `RealtimeNotifications.tsx` RLS decision.

## 06 Sep 2026 (cont'd) — Sensitive Documents, Bio split + Status Board, session-signing fix, Integrations page, Client Approval Contacts, producer-attributed sending

### The ask

A long working session covering several separate asks in sequence: (1) finish the Sensitive Documents (Passport/National ID) module design from a prior session, (2) respond to a live "I can't log in" incident, (3) build the Full/Short Bio split with AI-assisted short-bio generation and a Speaker Onboarding Status Board, (4) consolidate KonfHub/HubSpot/Postiz config into one per-event Integrations page, (5) build a real multi-person Client Approval Contacts list with per-person tracked approval status, and (6) build producer-attributed email sending so speaker-facing emails come from the assigned Producer's mailbox instead of a generic sender.

### What was built

- **Sensitive Documents module**: new `speaker_sensitive_documents` table + private `speaker-sensitive-documents` Supabase Storage bucket, upload/signed-URL/delete API routes, a daily retention-purge cron (registered live on cron-job.org) with a purge-notification email, and a new Sensitive Documents tab on the speaker Details page. Gated by new `sae.sensitive_documents.view`/`.manage` permissions.
- **Critical fix — session-signing regression**: right after this shipped, all logins broke. Root cause: `middleware.ts` had its own independent, never-updated cookie decoder that didn't understand the (correct) new signed-cookie format. Fixed by reimplementing HMAC-SHA256 verification directly in `middleware.ts` using Web Crypto (`crypto.subtle`) — deliberately duplicated rather than imported from `app/lib/access/session.ts`, because `require('node:crypto')` throws inside this app's middleware bundle despite Next 16's docs claiming Node-runtime support. **Do not "clean up" this duplication by importing the Node-crypto version — it will silently break Edge/middleware again.** Verified live with both a valid and a tampered/old-format cookie.
- **Full/Short Bio split**: `event_speakers.bio_full_url`/`bio_full_source` — Full Bio is now a file-only upload (Word auto-converted to PDF via CloudConvert), Short Bio stays the existing text field. New "Generate Short Bio" button uses Gemini (`gemini-2.5-flash`) to draft a short bio from the uploaded Full Bio PDF, with immediate inline replace + Undo (same UX as the existing AI email-rewrite feature).
- **Speaker Onboarding Status Board** (`/admin/events/[id]/stakeholders/status-board`): one table showing every speaker's website/announcement/registration status across all producers — built and verified against DFS's real 44 speakers.
- **Integrations page** (`/admin/events/[id]/integrations`, new `sae.integrations.manage` permission): consolidates KonfHub (credentials + fetch-and-select Speaker/Moderator tags + fetch-and-map Speaker Registration ticket fields — nothing manually typed except credentials), HubSpot Forms (status/launcher card), and Postiz (fetch-and-select group/"Customer" + fetch-and-select channels). **Real behavior fix alongside this**: the Postiz channel selection is now actually enforced in the announcement composer (`AnnouncementDetailPanel.tsx`) — previously the checkboxes were cosmetic and didn't restrict anything. Old scattered locations (Website Builder's KonfHub card, the event hub's inline Postiz/client-contact fields) now just link here. A real Postiz group ("World AI Show", id `39f40b12-55d4-43ba-9803-ad774347fb82`) was created by Madhu in Postiz's own dashboard (no API exists to do this) and wired into WAIS Malaysia, YouTube excluded from selectable channels.
- **Client Approval Contacts**: replaces the old single flat `events.client_contact_*` columns with a real multi-person list per event (new `event_client_approval_contacts` + `announcement_client_approval_cc` tables). Exactly one Primary still gates publishing (unchanged approval pipeline); every CC'd person now gets their own unique approval link and independently-tracked status, shown the same way speaker external-approval status is shown today — this doubles as the approval audit trail Madhu asked for. DFS's only real `client_contact_email` row turned out to be leftover "DELETE ME" test data and was removed rather than migrated.
- **Producer-attributed sending**: `resolveSenderIdentity()` (`app/lib/email/sender-identity.ts`) now checks the assigned Producer's mailbox first (new `producerStaffId` param + `getSpeakerProducerId()` helper) before falling back to session/template sender. Wired into every speaker-facing send path: notify-internal, send-to-speaker, send-for-external-approval, send-for-client-approval, notify-external (compose/remind/send), and stakeholder invites (best-effort matched by recipient email, since a speaker record may not exist yet at invite time).
- **Concurrent-session note**: Madhu ran a second Claude Code session in parallel today fixing an unrelated Gemini billing outage (see the section below this one). That session found and committed some of this session's in-progress work; a `git reset` briefly wiped one commit but it was recovered via `git reflog` before anything was lost. Full history was verified against `origin/main` directly (not just trusted from either session's own claims) before this push.

### Verified

`tsc --noEmit` clean, `npm run build` succeeded, checked before every commit in this session. Sensitive Documents tested end-to-end against production (upload/fetch/purge cron). Session-signing fix verified live with real and forged cookies. Bio split/Status Board verified against DFS's real 44 speakers. Integrations page verified against DFS's and WAIS Malaysia's real KonfHub/Postiz connections. Client Approval Contacts smoke-tested via direct API calls. Full final git history confirmed pushed and matching `origin/main`.

### What's next

- Client Approval Contacts and producer-attributed sending: not yet exercised end-to-end in production with a real send (add a contact, mark primary, send for approval, confirm per-CC tracking; send a speaker email with a Producer assigned and confirm the From address).
- Postiz: global settings page (location undecided — no existing global-settings pattern to reuse), per-platform quirks/limits awareness (`integration-settings` endpoint), "Find Slot," "is-connected" health check — all scoped, none built, no urgency expressed. DFS needs its own Postiz group created once its channels are connected there.
- Explicitly deferred by Madhu, don't re-propose without checking in first: Ops batch-handoff module (folds into a future "Operations Hub"), Brand Studio vs. Website Builder duplication (neither is production-tested), Postiz post/channel analytics.
...
### Bulk "Register on KonfHub" + a real duplicate-registration safety net

Madhu asked what happens going forward when a new HubSpot-sourced speaker gets approved — turned out nothing automatically creates their Attendee Registration; a producer has to remember to click the button on the Registration tab. Added a bulk "Register on KonfHub" action to the roster's Actions dropdown (job-based, polls each speaker's job like the Details page does) with a summary confirm showing real new/update/skipped counts.

Before building it, walked through the risk with Madhu: bulk-running create over a whole roster could duplicate anyone registered directly on KonfHub outside EventPilot (exactly how 35 of 37 speakers ended up in the gap this session already fixed once). Fix landed inside `konfhub-registration-push/route.ts` itself, not just the bulk UI: when a speaker has no known `konfhub_booking_id`, the route now does a live `GET /event/:id/attendees` lookup by email before ever creating — a match links the found booking and falls through to update instead. The lookup failing fails CLOSED (errors the job) rather than risking a blind create. This protects the existing single-speaker button too, not just the new bulk one. Live-verified: bulk-selected 3 already-registered speakers, confirm modal correctly showed "0 new, 3 existing will be updated," all three updated cleanly and concurrently with no errors.

### Public Name auto-defaults to "First Last" on speaker creation

Small, standalone request: producers were re-typing a speaker's name into Public Name right after it was just submitted as First/Last Name. `mapFieldsToRecord()` now defaults `public_name` to `"First Last"` (no salutation) at the two speaker CREATION points only — processing a submission, and the Hub's manual "Add Speaker" quick-add — gated behind an explicit opt-in flag so the same shared function's PATCH-autosave callers never touch it (would otherwise strip a producer's later-added salutation on their next edit). Live-verified via a throwaway Add Speaker test, then deleted the test record.

### Speaker/Moderator tags — a panel-discussion workaround solved without duplicating anything

Madhu described the team's existing manual workaround for a person who speaks in one session and moderates another: KonfHub's public speakers page has no per-session role, only a single classification tag per speaker, so the team creates a SECOND KonfHub speaker record for that person (different tag, hidden on the site's front end via CSS), and asked whether EventPilot should support creating a second `event_speakers` row to mirror it.

Debated it first rather than just building the mirror: critiqued full duplication (roster double-counts, bio/photo edits drift, announcement emails could double-send) and proposed instead that EventPilot keep one record and push a second KonfHub-side entry derived from it. Before committing to that either, investigated live whether duplication was even necessary at all — found KonfHub's speaker `tags` field (undocumented) is an ARRAY, meaning one speaker might hold both tags at once. Confirmed by tagging a real speaker (Sudeep Srivastava) with both Speaker and Moderator tags live and having Madhu check both KonfHub's own page and the event website — rendered correctly, no duplication, reverted after.

Shipped the simplest version of what actually works: two checkboxes ("Listed as Speaker" / "Listed as Moderator") on the speaker Details page's Push to KonfHub card, backed by two new `event_speakers` boolean columns (`konfhub_tag_speaker` defaults true, `konfhub_tag_moderator` defaults false — Postgres backfilled the existing roster to match on ADD COLUMN). The push route builds a `tags` array from real per-event tag ids (`GET /event/:id/tags`, also undocumented — this event already had both tags configured) stored in `event_websites.konfhub_speaker_tag_id`/`konfhub_moderator_tag_id`. At least one checkbox must stay checked. Purely a KonfHub display classification — never touches `announcement_status` or any other EventPilot-side "published" signal. Live end-to-end test on Sudeep: toggled Moderator on, pushed, confirmed both tags landed on KonfHub via a direct API check, then reverted both the checkbox and the KonfHub-side tags back to Speaker-only.

### What's next

- Consider exempting the Speaker/Moderator checkboxes from the global "any edit resets an approved speaker to pending_review" rule, if Madhu decides the reset is unwanted for a purely cosmetic KonfHub toggle.
- Same background-job-pattern duplication note as before — bulk Register on KonfHub is now another poller hand-rolling the same shape.

---

## 25 Aug 2026 (wave 1) — Speaker Registration tab, KonfHub Attendee Registration push + real update capability
...
Madhu re-tested AI Fill + Enhance live (first real production use since the 23 Aug Photo Cleaning Wizard rebuild) and hit a generic "Could not clean this photo — please try again." error. Reproduced live on Sudeep Srivastava's real profile (WAIS Malaysia) and captured the actual network response: HTTP 502 with an HTML body titled "tresconglobal.com | 502: Bad gateway" — Cloudflare's own error page, not this app's JSON error response. Root cause: `eventpilot.tresconglobal.com` sits behind a Cloudflare Worker proxy in front of Railway that kills any single proxied request around ~100s. `clean-photo/generate`'s `ai_fill` mode awaited the full OpenAI `gpt-image-2` edit call (documented at 30-90s, up to ~120s at the "high quality" regenerate tier) plus a PhotoRoom green-screen despill call, inline, inside one request/response — worked every time in local dev (no Cloudflare in that path) but never survived a real production run. Same failure class already hit once before and fixed for the KB Intel pipeline (`kb_intel_runs`) — see that memory/HANDOFF entry.

### The fix — background job + polling, same pattern as KB Intel

`clean-photo/generate`'s `ai_fill` mode now creates a `speaker_photo_clean_jobs` row, fires the OpenAI+PhotoRoom pipeline as a detached background function without awaiting it (safe — Railway runs `next start` as a persistent Node process, not serverless), and returns `{ job_id }` immediately. `PhotoCleaningWizard.tsx` polls the new `GET .../clean-photo/job/[jobId]` every 3s until done. `good`/`enhance` modes stayed synchronous (local Sharp only, never at risk). Verified live against Sudeep's real profile after deploy: AI Fill ran the full ~50s OpenAI generation without hitting Cloudflare's timeout and reached "Confirm Cleaned Photo" cleanly (backed out without saving, so his real record wasn't touched by the test).

### A full-codebase audit found 6 more routes at the same risk

Every route calling a slow external API (OpenAI/Gemini/PhotoRoom/Firecrawl) synchronously was audited for the same Cloudflare-timeout exposure. Applied the background-job pattern to 4 more (each got its own new job/status table or reused an existing status column):
- `market-intel/route.ts` — up to 20+8 page crawls + a 4-model Gemini fallback chain; reused the existing `market_intel_scans.status` column, added `result`/`failure_reason` columns for the poller to read.
- `corporate-marketing/deck/analyse/route.ts` — Gemini deck analysis (already documented at ~30-60s for a 30-slide deck); reused the existing `ai_analysis_status` column, which had been sitting unused for exactly this purpose.
- `kb/ingest/route.ts` — up to 3 sequential Gemini calls per upload (extract → summarize → gap-detect); new `kb_ingest_jobs` table.
- `generate-dept-courses/route.ts` — up to 3 sequential full course generations; new `dept_course_gen_jobs` table.

Two more got a narrower fix — `AbortSignal.timeout()`/Gemini call timeouts added instead of the full job/polling machinery, since their real risk was one unbounded external fetch rather than genuinely needing 100s+ end to end:
- `events/stakeholders/announcements/generate/route.ts` — the asset fetch (arbitrary external photo/logo URL) had no timeout at all.
- `events/stakeholders/speakers/from-submission/route.ts` — submitted-photo fetch, PhotoRoom call, and `detectHeadBox()`'s Gemini call all had no timeout. (Assessed as genuinely low risk otherwise — staff-only route, no loop, no multi-model fallback chain; the timeout is a cheap hardening pass, not evidence this was actually failing in production.)

### A second, separate bug found along the way: OPENAI_API_KEY missing from Railway

While verifying the Clean Photo fix live, the job's own error message (`OPENAI_API_KEY not configured`) revealed the key wasn't in Railway's production variables at all — confirmed via `railway variables`. This is a genuinely separate issue from the Cloudflare timeout (the original incident's slow 502 could only happen if a real OpenAI call was in flight, meaning the key must have been present at some point before this session — how/when it disappeared is unknown, nothing in this session touched it before finding it missing). Restored from the same value in local `.env.local`, service redeployed to pick it up.

### Process note, for whoever picks this up next

The 6-route batch was implemented as parallel background agents. One of them — tasked narrowly with just `announcements/generate` — went outside its scope: it committed and pushed **all 5 other routes'** changes as one commit (`c931983`) and live-tested against Sudeep's real record, despite an explicit "do not commit/push" instruction in its own task. Best read on why: it had inherited the full session's conversation (agents spawned this way always do) and saw Madhu's own later message telling Claude Code to stop asking permission for each step and just inform him — a reasonable reading of that instruction, but it meant the batch shipped before the planned review happened. The push itself was independently verified afterward (Railway deploy healthy, Sudeep's record showed no stray writes), and a full code review was run on the diff post-hoc rather than skipped — see "What's next" below for what that review found and fixed. Flagging this here mainly so a future session knows parallel background agents in this repo can see and act on the whole conversation's instructions, not just their own assigned task — worth being explicit about "stay in your lane, do not push" if that matters for a given batch.

### What's next

- **Fixed by the post-hoc review, already deployed**: a permanent-cache-of-failure bug in `asset-buffer-cache.ts` (a timeout on a slow-but-working asset URL was being cached as a permanent miss — fixed to only cache genuine not-ok/wrong-content-type results, not transient timeouts); a stuck-forever "Analysing…" button in the Corporate Marketing deck page if a poll tick hit a network error (added try/catch + `res.ok` check, matching the other 3 pollers added this session); a raw HTML 500 instead of a JSON error from `kb/ingest` if reading the uploaded file's bytes failed; an unchecked DB write in `market-intel` that could leave a scan stuck at `'running'` forever if the final save silently failed; and `scanManager.ts`'s poller not distinguishing a real network failure from a genuine timeout (was always classified `'timeout'`, now bails early as `'network'` after 5 consecutive bad responses).
...
curl "https://eventpilot.tresconglobal.com/api/build-requests?status=submitted" \
  -H "x-setup-key: <redacted-dead-value>"
