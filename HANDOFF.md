...
| Left alone / known follow-up | Two **pre-existing security issues** found (not caused by this work, not fixed by the Ops Hub commits) — see the section below: routes gated only by a browser-visible "admin code", and `GET /api/task-profiles` returning all staff task profiles with no login. |
