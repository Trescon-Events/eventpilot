...
| Field | Value |
|---|---|
| Who | Madhu + Claude Code (Sonnet 5) — 11–14 Aug 2026, one continuous thread |
| Latest push | 2026-08-14 — everything below (Corporate Brand + SAE + HubSpot Forms integration from 11 Aug, the Event Details Page + HubSpot mapping UX from the 11 Aug continuation, and the 13 Aug Event Details tabs redesign + Staff Portal/HRMS date-integrity fixes) is now on `main` in one push. Merged onto a parallel Durga + Claude Code (Opus 4.7) session from 11 Aug — see below — which had already landed `66ebbe8` (CM-002.1 Statistics Repository final slices), `dc4bb6d` (Nic batch), `a381b1f` (Thulasi 2 quick fixes), `696ed08` (CM-002.1 Slices 1–3) on `main` first. |
| DB migrations applied | 11 Aug: `access_rbac`, `email_templates`, `stakeholder_invites`, `event_form_schemas`, `event_stakeholder_custom_fields`, `form_schema_defaults`, `hubspot_forms`, `hubspot_submissions`, `secure_documents`, `corporate_brand_assets`, `corporate_brand_guidelines`, `event_public_details`, `event_details_page`. 13 Aug: `events_on_hold_status` (adds `on_hold` to the `events.status` CHECK constraint). All applied directly against production Supabase via the pg pooler. Parallel session: ✅ `supabase/cm_statistics_repository.sql` (applied by Durga via Studio SQL Editor). |
| Handed off to | Durga |
| Deployed | ✅ Yes — pushed to `main` this session, Railway auto-deploys on push. Verify `eventpilot.tresconglobal.com` reflects the latest commit. |
