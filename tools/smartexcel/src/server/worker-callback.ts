// HTTP endpoint the Python worker calls when a sample/full run finishes (PRD
// §6.5, §7.1). Authenticated by WORKER_SHARED_SECRET. Invoked directly from the
// Worker fetch handler (src/worker.ts) — it is machine-to-machine, not a
// session/SSR route. Records run results, output artifacts, state, notification.
...
export async function handleWorkerCallback(request: Request): Promise<Response> {
  ...
  try {
    ...
    // Log details server-side for debugging; don't leak parse errors over the wire.
  ...
  // Update the run row (latest pending/running for this stage) and the job state.
  ...
  const target: JobStatus = !succeeded
    ? "failed"
