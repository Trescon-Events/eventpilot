// Gemini orchestration layer (PRD §6.3, §7.4). The AI clarifies, classifies, and
// plans; deterministic processing stays in the Python worker. Model routing uses
// abstract tiers so we never hardcode one model everywhere.
...
// One AI call per chat turn that does both routing and response:
//   mode="refine"  — the user changed what we should do. Update understanding.
//   mode="reply"   — the user asked a question, requested suggestions, or is
//                    just chatting. Don't touch the understanding; just reply.
// Single call instead of classifier + handler so latency stays low.
export async function chatTurn(
  ...
  // Trim to last ~12 messages to bound token cost. The full chat history is
  // already reflected in `current` (the durable understanding) so the AI
  // doesn't lose context from older turns.
  ...
  if (result.mode === "refine") {
    if (!result.understanding) {
      // The AI claimed refine but didn't return an understanding — degrade
      // gracefully to a plain reply so we don't drop the message.
