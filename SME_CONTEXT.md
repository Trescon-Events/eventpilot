...
| `/admin/events/[id]/integrations` | Integrations (5/6 Sep 2026) — consolidated external-integration config: KonfHub credentials/tags/fields, HubSpot Forms status, Postiz group/channels, Client Approval Contacts, Site Registry/Health Checks, and (16 Sep 2026) Content Guidelines API tokens for external tools — see §7's bearer-token pattern. Gated by `sae.integrations.manage` |
| `/admin/events/[id]/stakeholders` | Stakeholder Hub — speaker/sponsor/partner onboarding, RBAC-gated (`sae.*` permission keys). Forms are managed in HubSpot, not built in-app — see `.../stakeholders/hubspot-form/[formType]` to connect a form and map its fields. **Caution when connecting a form**: nothing validates that the HubSpot form's own content matches the category (form_type) you connect it under — pick the wrong one and the field-mapping dropdown will show the wrong entity's fields (this bit DFS's Speaker form, which got connected under Sponsor by mistake). New **Cancelled** tab (23 Sep 2026, speakers-only) alongside Deleted — a speaker missing from a producer's confirmed-list re-check moves here via a guarded, double-confirmed Cancel action (checks/clears any live KonfHub Speaker-listing first; KonfHub has no cancel API for Attendee Registration, so that half is a required human-acknowledgment checkbox instead) rather than the old plain `confirmation_status='Cancelled'` dropdown pick, which is now read-only once set. **Communications tab** (24 Sep 2026) — every email send (Request Missing Items, Send Reminder, Send Acknowledgment) now opens an editable To/Cc/Subject/live-preview popup before sending, via a compose-then-send route split; same pattern applied to the announcement panel's external-notify reminder, shared UI in `app/components/EmailComposeFields.tsx`. **Sensitive Documents tab** (24 Sep 2026) — Passport/National ID Delete now requires typing `DELETE` to confirm; deletion history shows the real staff name; new producer "Mark as Reviewed" tracking (`reviewed_by`/`reviewed_at`) feeds the Status Board's Passport/National ID columns. **Status Board** (24 Sep 2026) — new Email/Assistant Email columns; Passport/National ID/Short Bio gained a real Pending/In Progress/Done lifecycle (reviewed-document and Approve-for-Announcement signals, not just presence); Cleaned Photo + Website Photo merged into one Website Photo column. **Public speaker-submission form** (`app/public/speaker-submission/[speakerId]`, 24 Sep 2026) redesigned into two sections (Speaker Profile; Speaker License Requirements with a conditional UAE-resident question), field constraints/help text mirrored from the real HubSpot onboarding form, Country of Residence dropdown sourced from `app/lib/forms/hubspot-countries.ts` (HubSpot's own 198-option list — no other canonical country list exists in this codebase). **Country & dial code are never defaulted** (24 Sep 2026): `event_speakers.country` and `dial_code` have no DB default, the Add Speaker modal opens empty, and `country` reaches KonfHub only as the Speakers-listing `location` when a person/form actually supplied it (omitted when empty). The KonfHub Registration push omits `country`/`country_code`/`dial_code` when empty or unmapped — never falls back to UAE/+971, and never guesses a country code. **Approval kind note** (24 Sep 2026): speaker (External Reviewer) approval emails end with a small grey "A kind note" that publication isn't guaranteed (limited channel slots) — it lives in the `speaker_announcement_approval_request` email template (editable in Admin > Email Templates) and, as a hardcoded copy, on the public review page for External Reviewers only. **Speaker Details page** (26 Sep 2026) now leads with a **Salutation** dropdown before First Name (stored in `custom_fields.salutation` for HubSpot-connected events). FIFF (Future Islamic Finance Forum 2026, a DFFW child) was bulk-imported from the producers' sheet the same way DFS was |
| `/admin/events/[id]/agenda` | Agenda Builder (16 Sep 2026) — real tracks/sessions with a speaker-picker (FK to `event_speakers`), replacing the old free-text agenda tab still used by `/admin/events/[id]/website`'s Content tab for events that haven't opted in. Gated by `event_websites.agenda_source`: `konfhub_authoritative` events (DFFW-family) get structure only from a KonfHub fetch/reconcile flow on the Integrations page (no "+ Add Stage"); `eventpilot_native` events (everything else) create stages directly here |
...
| `/admin/umbrellas/[id]/operations` | **Umbrella-level Operations** (25 Sep 2026) — for an umbrella such as Dubai Future Finance Week, speaker-licence processing happens here for ALL its child events (one list with an Event column, one vendor set, batches numbered across the umbrella). An event under an umbrella lifts to it automatically. Access = the Operations role on ANY child event. Also hosts the "Event days" card. |
...
| `ops_license_batches` / `ops_license_batch_items` (owned by event_id XOR umbrella_id; same for `ops_event_vendors`, `ops_license_files`) | event_id, vendor_id, batch_number, status, access_days, expires_at; speaker_id + frozen speaker fields + document ids | Licence batches (status draft/sent/downloaded/completed/expired/cancelled) and their frozen speaker snapshots; a speaker is in at most one ACTIVE batch |
...
| `ops_license_files` / `ops_license_file_batches` / `ops_access_audit` | storage_path (Supabase bucket `ops-license-files` today; Azure container `license-files` once `DOCUMENT_STORAGE_BACKEND=azure`); file↔batch link; actor_type/action/target | Licence copies (one file may cover several batches) and the append-only access audit |
...
**External/public API pattern (bearer token, no session)** — 16 Sep 2026, first used by `GET /api/public/v1/content-guidelines`. For a route an *external tool* needs to call (not a browser with a session cookie): put it under `/api/public/`, which `middleware.ts` already exempts from the session gate; auth it with a per-purpose bearer token table (hash stored, plaintext shown once at creation — see `content_guideline_tokens`) rather than a shared secret, so access is scoped and individually revocable. If an *existing* session-gated route also needs to accept the same external caller, add the bearer check inside that route and give it a narrow `middleware.ts` exemption keyed on the `Authorization` header being present — not a blanket path exemption — so the route's original session-based calling convention keeps working unchanged.
...
|---|---|---|
| `AppShellRoot` | `app/components/nav/AppShellRoot.tsx` | **Persistent shell as of the 17–18 Aug 2026 nav rebuild** — replaces `GlobalShell` as `AuthedShellGate`'s mount. Left `AppSidebar` (registry-driven, replaces the old top nav bar) + a slim breadcrumb strip (unchanged — "where am I", while the sidebar answers "where can I go") + Help/Sound/Profile. Also mounts `CommandPalette` (cmd+k). **A new page never renders this itself and never unmounts/remounts it** — just build page content; the shell is already there. |
| `AppSidebar` / `AppSidebarSection` / `EventsSidebarSection` | `app/components/nav/` | The persistent left sidebar itself — collapsible sections, derived from the module registry's new `sidebar` field (not a hardcoded list). `EventsSidebarSection` is the one section that's per-event-scoped rather than static, resolving access via `NavDataContext`. `useSidebarCollapse.ts` persists collapse state. |
| `CommandPalette` | `app/components/nav/CommandPalette.tsx` | Cmd+K quick-jump, indexed from `app/lib/nav/paletteIndex.tsx` (registry-derived, same source as the sidebar — no separate hardcoded list). |
| `PageHeader` | `app/components/PageHeader.tsx` | The page-level title/description/actions row every new page should render at its own top — `<PageHeader title="..." description="..." actions={<button/>} />`. Do not add a new page-owned logo, back-link, or nav bar; `AppShellRoot` + the breadcrumb already cover that. |
| `ModuleSidebar` | `app/components/ModuleSidebar.tsx` | Optional *secondary* sidebar for a module with 2+ sub-pages (Finance, HR, Knowledge Base, DocuHub, Smart Data are the reference implementations) — nests inside the persistent `AppSidebar`, doesn't replace it. Add a `layout.tsx` for the module rendering `<ModuleSidebar groups={...}>{children}</ModuleSidebar>` alongside a flex wrapper. Skip this for a single-page tool. |
| `ReviewWidget` | `app/components/ReviewWidget.tsx` | Floating "Report Issue" trigger, opened from the Help menu in `AppShellRoot` (not its own floating button anymore) |
| `GlobalShell` / `PlatformMenu` | `app/components/GlobalShell.tsx` / `PlatformMenu.tsx` | **Retired as of the 17–18 Aug 2026 nav rebuild** — `PlatformMenu`'s slide-in tile grid was a third parallel menu system alongside the old top nav and each page's hardcoded tile list; the registry-driven sidebar replaces it rather than sitting alongside it. Both files are left in place as inert dead code (not yet deleted — a couple of exported pieces, `HelpMenu`/`SoundToggle`/`ProfileMenu`, still live in the old `NavBar.tsx` and are reused by `AppShellRoot`). Don't render either in a new page. |
| `NavBar` / `AppShellNav` | `app/components/NavBar.tsx` / `AppShell.tsx` | **Retired as of the 15–16 Jul 2026 nav overhaul** — every page that used to render one of these now renders `PageHeader` instead. Don't use either in a new page; they're kept only because `HelpMenu`/`SoundToggle`/`ProfileMenu` are reused by `AppShellRoot`. |

Do not rebuild navigation from scratch, and do not hand-maintain a new list of "what modules exist" anywhere — add the module to `app/lib/registry/modules.tsx` once (including a `sidebar` entry if it should appear in the persistent sidebar) and every surface (sidebar, command palette, Toolkit hub, page badges, breadcrumbs) picks it up automatically. Breadcrumbs (`app/lib/nav/breadcrumbs.ts`) are derived purely from the registry + current URL — a static `href` needs no extra work; a page whose path includes a dynamic segment your registry entry can't express as its own `href` (an event-scoped tool, for example) needs `breadcrumbPattern` (a `/path/:param` template) + `breadcrumbParent` (the ancestor registry key) on its `ModuleDef` entry instead.

**Module registry & access control** (`app/lib/registry/`): `modules.tsx` is the single source of truth for every module's key/label/icon/color/href/access rule — read it before assuming a module doesn't exist or building a new nav list. `access.ts` (server-only) exposes `requireModuleAccess(moduleKey)` — call this at the top of a `layout.tsx` for any page that should be gated server-side; client-side-only tile hiding is not sufficient (this was a real production security gap, closed 14 Jul 2026). As of 17–18 Aug 2026, `checkAccess()` also supports an `event_permission` kind (with `ctx.eventId` and an optional `.*` wildcard suffix) so a registry entry can gate on per-event RBAC permissions from `app/lib/access/event-access.ts`, not just legacy `module_access`.
